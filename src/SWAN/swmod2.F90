...
!
      REAL(rkind), SAVE    :: ACTOT
      REAL(rkind), SAVE    :: EDRKTOT
      REAL(rkind), SAVE    :: EKTOT
      REAL(rkind), SAVE    :: ETOT1
      REAL(rkind), SAVE    :: ETOT2
      REAL(rkind), SAVE    :: ETOT4
      REAL(rkind), SAVE    :: KM_WAM
      REAL(rkind), SAVE    :: KM01
      REAL(rkind), SAVE    :: SIGM_WAM
      REAL(rkind), SAVE    :: SIGM_10
      REAL(rkind), SAVE    :: SIGM01
!
...
!
      REAL(rkind), SAVE, ALLOCATABLE :: SIGPOW(:,:)
!
...
!  longer filenames for output requests
   ...
   TYPE OPSDAT
      ...
      REAL(rkind)                  :: OPR(5)                     ! real coefficients
      INTEGER               :: MIP                        ! number of points
      REAL(rkind), POINTER         :: XP(:), YP(:), XQ(:), YQ(:) ! point coordinates
   ...
   TYPE ORQDAT
      ...
      REAL(rkind)                   :: OQR(2)   ! real coefficients
      INTEGER, POINTER       :: IVTYP(:) ! type of output variable
      REAL(rkind), POINTER          :: FAC(:)   ! multiplication factor of block output
...
!     LAMBDA: Contains the values for lambda in the MDIA formulation.
   ...
   REAL(rkind), PUBLIC, SAVE, ALLOCATABLE :: AF11(:)
   REAL(rkind), PUBLIC, SAVE, ALLOCATABLE :: CNL4_1(:)
   REAL(rkind), PUBLIC, SAVE, ALLOCATABLE :: CNL4_2(:)
   REAL(rkind), PUBLIC, SAVE, ALLOCATABLE :: LAMBDA(:)

   TYPE RIAMDAT
      INTEGER                :: II(3), JJ(3)
      REAL(rkind)                   :: SSS, DI(3), DJ(3)
...
!               4: directional distribution coefficient
   ...
   TYPE BSPCDAT
      ...
      REAL(rkind), POINTER          :: BSPDIR(:), BSPFRQ(:)
...
!     YCRP    : y-coordinate of corner point
   ...
   TYPE OBSTDAT
      INTEGER                :: TRTYPE
      REAL(rkind)                   :: TRCOEF(3)
      REAL(rkind), POINTER          :: TRCF1D(:), TRCF2D(:,:)
      INTEGER                :: RFTYP1, RFTYP2, RFTYP3
      REAL(rkind)                   :: RFCOEF(6)
      INTEGER                :: NCRPTS
      REAL(rkind), POINTER          :: XCRP(:), YCRP(:)
...
!      REAL(rkind)   , SAVE, ALLOCATABLE :: AC2(:,:,:)
!      REAL(rkind)   , SAVE, ALLOCATABLE :: XCGRID(:,:), YCGRID(:,:)
      REAL(rkind)   , SAVE, ALLOCATABLE :: XCGRID(:), YCGRID(:)
      REAL(rkind)   , SAVE, ALLOCATABLE :: SPCSIG(:)  , SPCDIR(:,:)
      REAL(rkind)   , SAVE, ALLOCATABLE :: DEPTH(:) , FRIC(:,:)
      REAL(rkind)   , SAVE, ALLOCATABLE :: UXB(:,:)   , UYB(:,:)
      REAL(rkind)   , SAVE, ALLOCATABLE :: WXI(:,:)   , WYI(:,:)

      REAL(rkind)   , SAVE, ALLOCATABLE :: WLEVL(:,:) , ASTDF(:)
      REAL(rkind)   , SAVE, ALLOCATABLE :: MUDLF(:)
      REAL(rkind)   , SAVE, ALLOCATABLE :: NPLAF(:) , TURBF(:)
      REAL(rkind)   , SAVE, ALLOCATABLE :: LAYH(:), VEGDIL(:), VEGDRL(:),&
     &                              VEGNSL(:)
...
!     SWREAL(rkind)  : MPI datatype for reals
!     SWSUM   : MPI collective summation
!
   INTEGER INODE !JL Mute use schism NPRO , NPROC
...
!
   ...
   REAL(rkind)    XCLMAX, XCLMIN, YCLMAX, YCLMIN
   ...
   REAL(rkind)   , SAVE, ALLOCATABLE :: XGRDGL(:), YGRDGL(:)
!
!  8. Subroutines and functions used
!
!     ---
...
!     DIFPARDY: derivative of DIFPARAM in y-direction
   ...
   REAL(rkind), ALLOCATABLE :: DIFPARAM(:)
   REAL(rkind), ALLOCATABLE :: DIFPARDX(:)
   REAL(rkind), ALLOCATABLE :: DIFPARDY(:)
