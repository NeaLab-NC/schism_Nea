...
subroutine SwanTranspAc ( amat  , rhs   , leakcf,                 &
                          cgo   , cax   , cay   , cad   , cas   , &
...
!   real(rkind), dimension(MDC,MSC,nverts), intent(in) :: ac2    ! action density at current time level
    real(rkind), dimension(MDC,MSC,5), intent(out)     :: amat   ! coefficient matrix of system of equations in (sigma,theta) space
    ...
    real(rkind), dimension(MDC,MSC), intent(in)        :: cad    ! wave transport velocity in theta-direction
    real(rkind), dimension(MDC,MSC), intent(in)        :: cas    ! wave transport velocity in sigma-direction
    ...
    real(rkind), dimension(MDC,MSC), intent(out)       :: leakcf ! leak coefficient
    ...
    real(rkind), dimension(MDC,MSC), intent(out)       :: rhs    ! right-hand side of system of equations in (sigma,theta) space
...
!    call SwanTranspX ( amat   , rhs  , ac2   , ac1   , cax   , cay   , &
    call SwanTranspX ( amat   , rhs                  , cax   , cay   , &
                       rdx    , rdy  , obredf, idcmin, idcmax, isslow, &
                       isstop , trac0, trac1 )
    
!    if(myrank==0)print*,'SwanTranspX INDEX,INDEX_GL,rhs',KCGRD(1),iplg(KCGRD(1)),rhs(:,MSC)  
    !
    ! add GSE correction, if appropriate
    !
    if ( WAVAGE > 0._rkind ) &
!    call SwanGSECorr ( rhs, ac2, cgo, spcdir, idcmin, idcmax, isslow, isstop, trac0 )
    call SwanGSECorr ( rhs     , cgo, spcdir, idcmin, idcmax, isslow, isstop,trac0 ) 
!    if(myrank==0)print*,'SwanGSECorr rhs',rhs(:,MSC)
...
!                    ac2        , isstop     , anybin     , leakcf , &
                                 isstop     , anybin     , leakcf , &
                    trac0      , trac1      )
!       if(myrank==0)print*,'STRSD rhs',rhs(:,MSC)
       !
    endif
...
!                        amat(1,1,3), anybin, rhs        , ac2        , &
                        amat(1,1,3), anybin, rhs                     , &
                        iscmin     , iscmax, iddlow     , iddtop     , &
                        trac0      , trac1 )
!          if(myrank==0) print*,'STRSSI rhs',rhs(:,MSC)
          !
