...
!
    type geomtype
      real(rkind) :: det                               ! determinant
      real(rkind) :: dx1 , dx2                         ! two components of covariant base vector a_(1)
      real(rkind) :: dy1 , dy2                         ! two components of covariant base vector a_(2)
      real(rkind) :: rdx1, rdx2                        ! first component of contravariant base vector rdx(b) = a^(b)_1
      real(rkind) :: rdy1, rdy2                        ! second component of contravariant base vector rdy(b) = a^(b)_2
      real(rkind) :: th1 , th2                         ! direction of faces pointing to present vertex
    end type geomtype

    type facetype
      integer :: atti(MAXFACEATTI)
      real(rkind)    :: attr(MAXFACEATTR)
    end type facetype
