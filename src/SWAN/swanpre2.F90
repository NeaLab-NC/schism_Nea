...
!
        IF (ONED) THEN                                                    !32.02
          CALL MSGERR (2,  &                                              !32.02
     &    ' Illegal keyword (FRA) in combination with 1-computation')     !32.02
          GOTO 800                                                        !32.02
        ELSE                                                              !32.02
!         ver 30.20: names of input variables changed, order of data changed
     ...
     &          '[alpfr] must be 0 with spherical coordinates')           !33.09
          ...
          OPSTMP%OPR(5) = PI2_W * (ALPK/360.-NINT(ALPK/360.))
...
!
        IF (ONED) THEN                                                    !32.02
          CALL MSGERR (2, &                                               !32.02
     &     'Illegal keyword (GROUP) in combination with 1D-computation')  !32.02
          GOTO 800                                                        !32.02
...
!            *****  of the boundary point in the four sides ********
             DO 50 I=1,4
               INTE=MXN
               ALON=XNLEN
               ANGLE=ANG+PI2_W*(90.*REAL(I-1))/360.
...
!                ***** store number of points *****
             ...
             OPSTMP%OPR(5) = PI2_W * (ALPCN/360.-NINT(ALPCN/360.))       !40.31
...
!# if defined (MULTIPROCESSOR)
              IF(.NOT.SERIAL)THEN
               ...
               WRITE(FILENM(ILPOS+1:ILPOS+4),99) myrank
  33           FORMAT('-',I3.3)               
              ENDIF
!            IF ( PARLL ) THEN                                            !40.30
...
!
  ...
  70      CALL SVARTP (IVTYPE)
     ...
     &                   'netCDF output not allowed for '//TRIM(FILENM)) !41.52
                ENDIF                                                    !41.52
             ENDIF                                                       !41.52
             CALL INREAL ('UNIT', DFAC, 'STA', -1._rkind)
...
!# if defined (MULTIPROCESSOR)
            IF(.NOT.SERIAL)THEN
             ILPOS = INDEX ( FILENM, ' ' )-1
             WRITE(FILENM(ILPOS+1:ILPOS+4),33) myrank
            ENDIF
!          IF ( PARLL ) THEN                                              !40.30
!             ILPOS = INDEX ( FILENM, ' ' )-1                             !40.30
!             WRITE(FILENM(ILPOS+1:ILPOS+4),33) INODE                     !40.30
...
!
      ...
      USE schism_glbl, ONLY : rkind,skind
      ...
      !USE MOD_UTILS
      USE VARS_WAVE,ONLY : NESTING, LBINTER !OBC_HS,OBC_DIR,OBC_TPEAK,NESTING
      USE VARS_WAVE,ONLY : NESTING_FILE,NESTING_TYPE_WAVE 
      USE VARS_WAVE,ONLY : WBAC,WBACOLD,WBACNEW,DSPEC
      USE VARS_WAVE,ONLY : WBSPPARM,IOBCN_W
...
!
      REAL(rkind)    XCGRID(MXC,MYC),    YCGRID(MXC,MYC)                        !30.82
!
...
!
      ...
      REAL(rkind)      CRDP, CRDM, SOMX, SOMY
      REAL(rkind)      XP,YP,XC,YC,RR,DIRSI,COSDIR,SINDIR,DIRSID,DIRREF
      REAL(rkind)      RLEN1,RDIST,RLEN2,XC1,YC1,XC2,YC2,W1
...
! SWAN in SCHISM : local vars
      ...
      REAL(rkind),  ALLOCATABLE :: HSC1_TMP(:,:),DIR1_TMP(:,:)
      REAL(rkind),  ALLOCATABLE :: TPEAK_TMP(:,:)
      REAL(rkind),  ALLOCATABLE :: TIMES(:)
      INTEGER :: I
      REAL(rkind) :: NTMP,F_TMP
      REAL(skind) :: ftmp
!
!  9. Subroutines calling
!
...
! JL : SWAN in SCHISM : NESTING or UNIFORM SPECTRA 
! JL : By using calls like 'SCAN_INCSTR' 'SCAN_INREAL ...' 
! JL ! We use the FVCOM NML Scanning method, not the SWAN Ocean Pack
! input reading system

      CALL SCAN_INCSTR('SCBOU BOUND_CHOICE',BOUND_CHOICE,'UNC',' ')

!      print*,'INSIDE SWBOUN, BOUND_CHOICE=',BOUND_CHOICE

      IF(BOUND_CHOICE == 'UNIFORM' .OR. BOUND_CHOICE == 'CONSTANT')THEN

!         IF(NESTING_ON_WAVE)THEN
!           IF(MSR)WRITE(IPT,*) "BOUND_CHOICE =",BOUND_CHOICE,&
!                          "NESTING_ON_WAVE = ",NESTING_ON_WAVE
!           CALL FATAL_ERROR( & 
!           "The parameter NESTING_ON_WAVE in ***_run.nml should be .", &
!           "or BOUND_CHOICE in INPUT should be NESTING.")
!         END IF

        CALL SCAN_INCSTR('SCBOU SHAPESPEC', SHAPESPEC,'UNC',' ')

        IF(SHAPESPEC == 'JON')THEN
           FSHAPE = 2
           CALL SCAN_INREAL('SCBOU GAMMA', ftmp, 'STA', 3.3)
           PSHAPE(1) = MyREAL(ftmp)
           WRITE(PRINTF,*) 'GAMMA JONSWAP = ', PSHAPE(1)
        ...
        ELSE IF(SHAPESPEC == 'GAUS')THEN
            FSHAPE = 4
            CALL SCAN_INREAL('SCBOU SIGFR', ftmp, 'STA', 0.01)
            SIGMAG = MyREAL(ftmp)
        ...
        CALL SCAN_INCSTR('SCBOU CHAR_WAVE_PERIOD',CHAR_WAVE_PERIOD,'UNC',' ')
        IF(CHAR_WAVE_PERIOD == 'MEAN')THEN
            FSHAPE = -FSHAPE
        ENDIF

        !    directional distribution given by DEGR or by POWER
        CALL SCAN_INCSTR('SCBOU DSPR',DSPR,'UNC',' ')
        IF(DSPR == 'DEGREES')THEN
          DSHAPE = 1
        ELSE IF(DSPR == 'POW')THEN
          DSHAPE = 2
        ELSE
          CALL parallel_abort("SCBOU DSPR: Wrong Choice, DEGREES or POW")
        ENDIF

        CALL SCAN_INREAL ('SCBOU HSIG', ftmp, 'REQ', 0.)
        SPPARM(1) = MyREAL(ftmp)
        CALL SCAN_INREAL ('SCBOU PER', ftmp, 'REQ', 0.)
        SPPARM(2) = MyREAL(ftmp)
        CALL SCAN_INREAL ('SCBOU DIR', ftmp, 'REQ', 0.)
        SPPARM(3) = MyREAL(ftmp)
        IF (DSHAPE == 1) THEN
           CALL SCAN_INREAL ('SCBOU DD', ftmp, 'UNC', 30.)
           SPPARM(4) = MyREAL(ftmp)
           IF (SPPARM(4) > 360. .OR. SPPARM(4) < 0.) THEN
              CALL MSGERR (2,'Directional spreading is less than '// &
                        '0 or larger than 360 degrees')
           END IF
        ELSE
           CALL SCAN_INREAL ('SCBOU DD', ftmp, 'UNC', 2.)
           SPPARM(4) = MyREAL(ftmp)
           WRITE(PRINTF,*) 'DD = ',SPPARM(4)
           IF (SPPARM(4).LE.0._rkind) THEN
              CALL MSGERR (2,'Power of cosine is less or equal to zero')
           END IF

           IF (SPPARM(4)*DDIR**2/2. > 1.) THEN
              CALL MSGERR (2,'distribution too narrow '// &
...
142        FORMAT (' Advise: choose Dtheta < ', F8.3, ' degr')
      ...
      ELSE IF (KEYWIS ('NE').OR.KEYWIS('NESTING')) THEN

       NESTING = .TRUE.

      ELSE
        IF(myrank==0) &
           WRITE(PRINTF,*)"SWBOU, Nesting Unknown, skipping now"

        CALL parallel_abort("SWBOU, Nesting Unknown, skipping now")
!JL Skip Now
      ENDIF

!      print*,'INSIDE SWBOUN, NESTING=',NESTING

!JL : SWAN in SCHISM : Boundary forcing using file with wave parameters or spectral density
!=====================================================================!
...
!
       IF(myrank==0) WRITE(PRINTF,*)&
          "BOUND_CHOICE = NESTING",&
          "INFO! allocate wave boundary arrays"

       IF(.NOT.ALLOCATED(WBAC)) &
          ALLOCATE(WBAC(MDC,MSC,IOBCN_W), stat=istat)
       IF (istat/=0) CALL parallel_abort('swanpre2, allocate error 5')
       WBAC = 0._rkind

!      IF (LBINTER) THEN
       IF(.NOT.ALLOCATED(WBACOLD)) &
          ALLOCATE(WBACOLD(MDC,MSC,IOBCN_W))
       IF(.NOT.ALLOCATED(WBACNEW)) &
          ALLOCATE(WBACNEW(MDC,MSC,IOBCN_W))
       WBACOLD = 0._rkind
       WBACNEW = 0._rkind

       IF(.NOT.ALLOCATED(WBSPPARM)) &
          ALLOCATE( WBSPPARM(8,IOBCN_W))
        WBSPPARM = 0._rkind

       IF(.NOT.ALLOCATED(DSPEC)) &
          ALLOCATE(DSPEC(MDC,MSC,IOBCN_W))
       DSPEC   = 0._rkind
!      ENDIF

       IF(myrank==0) WRITE(PRINTF,*)&
          "BOUND_CHOICE = NESTING",&
          "INFO!reading the boundary forcing from a wave nesting file!"

        CALL SCAN_INCSTR('SCBOU NESTING_TYPE_WAVE',NESTING_TYPE_WAVE,&
     &                  'UNC',' ')

!        IF(myrank==0) print*,'NESTING_TYPE_WAVE',NESTING_TYPE_WAVE

        IF( TRIM(NESTING_TYPE_WAVE) /= 'PARAM' .and.     &
            TRIM(NESTING_TYPE_WAVE) /= 'WAMPARAM' .and.  &
            TRIM(NESTING_TYPE_WAVE) /= 'SPEC') THEN          
            IF(myrank==0) WRITE(PRINTF,*)&
            "THE NESTING_TYPE_WAVE SHOULD BE ",&
     &      "'PARAM' or 'WAMPARAM' for wave parameters or ",&
     &      "'SPEC' for spectral density"
            CALL parallel_abort("SCBOU NESTING_TYPE_WAVE Error")
        ENDIF

        CALL SCAN_INCSTR('SCBOU NESTING_FILE',NESTING_FILE,&
     &                  'UNC',' ')
...
!        CALL parallel_abort("SWBOU, Nesting done")
...
!         set CCOORD to False to indicate that no locations are defined  !40.03
        ...
        IF (CCOORD) THEN
          ...
          DO IBOUNC = 1, NBOUNC
            ...
            ! JEROME DBG
            PRINT*,'(BCFILE) B. spectrum ', IBOUNC, XP+XOFFS, YP+YOFFS
...
!           read frequency in Hz and convert to radians/sec
            READ (NDSD, *) FREQHZ
            BSPFIL%BSPFRQ(IFRE) = PI2_W * FREQHZ                         !40.31
          ENDDO
          READ (NDSD, '(A)') HEDLIN
          IF (ITEST.GE.60) WRITE (PRTEST,11) HEDLIN                      !40.00
          NHEDF = NHEDF + 2 + NFRE
...
!         read boundary point coordinates from file
          READ(NDSD,*) XLON, XLAT, DDATE, EMEAN, THQ, FMEAN
...
!             transform to local Cartesian coordinates
              XP1 = XPC + LENDEG*COS(PI_W*XLAT0/180.)*(XPWAM(IBNC1)-XLON0) !40.13
              YP1 = YPC + LENDEG*(YPWAM(IBNC1)-XLAT0)                    !40.13
              XP2 = XPC + LENDEG*COS(PI_W*XLAT0/180.)*(XPWAM(IBNC2)-XLON0) !40.13
...
!             check whether direction of (RX,RY) corresponds to ALPC + k * 90 degr
              PHI = ATAN2(RY,RX)
              DPHI = MOD(PHI-ALPC+1.25*PI_W,0.5*PI_W)-0.25*PI_W
...
!
      ALLOCATE(BSPFIL%BSPFRQ(NFRE))                                      !40.31
      DO IFRE = 1, NFRE
        BSPFIL%BSPFRQ(IFRE) =  FRQ_ARRAY(IFRE)*2*PI_W                    !40.31
      ENDDO
!
      IF (ITEST.GE.60) THEN
...
!
      DIR_ARRAY(:) = PI_W*DNORTH/180 - DIR_ARRAY(:)                      !40.15
      ...
      DO IANG = 1, NANG
        ...
        ELSE
          IF (DIR_ARRAY(IANG).LT.DIRRD1) THEN
            BSPFIL%BSPDIR(IANG) = 2*PI_W+DIR_ARRAY(IANG)                 !40.31
...
!
          ...
          ENDIF
!
          XP2 = XPC + LENDEG*COS(PI_W*XLAT0/180.)*(XLON-XLON0)
...
!#     endif
      ...
      USE schism_glbl, ONLY : ipgl,iplg
...
!JL add-on 
        CALL MSGERR (3, 'POINTS XY not accepted, only POINTS IJ')
...
! JL transform from Global to Local :
!#  if defined (MULTIPROCESSOR)
!        LXDMP = NLID(LXDMP)
!# endif
...
! JL transform from Global to Local :
          IF(.NOT.SERIAL) THEN
            ...
            IF (ipgl(LXDMP)%rank==myrank) THEN
              LXDMP = ipgl(LXDMP)%id
            ELSE
              LXDMP = 0
            END IF
...
!
     ...
     &          LYDMP.GE.0 .AND. LYDMP.LE.MYC-1) THEN
      ...
      ENDIF                                                              !40.80
!
  40  CALL MSGERR (1, 'test point is not active in that subdomain')      !40.80
...
!# if defined (MULTIPROCESSOR)
!        ILPOS = INDEX ( FILENM, ' ' )-1 
!        WRITE(FILENM(ILPOS+1:ILPOS+4),99) MYID
!  99       FORMAT('-',I3.3) 
!# endif
         IF(.NOT.SERIAL) THEN
          ILPOS = INDEX ( FILENM, ' ' )-1
...
!# if defined (MULTIPROCESSOR)
!        ILPOS = INDEX ( FILENM, ' ' )-1
!        WRITE(FILENM(ILPOS+1:ILPOS+4),99) INODE
!# endif
        IF(.NOT.SERIAL)THEN
         ILPOS = INDEX ( FILENM, ' ' )-1
         WRITE(FILENM(ILPOS+1:ILPOS+4),99) myrank
...
!PUN        FILENM = TRIM(LOCALDIR)//DIRCH2//TRIM(FILENM)                     40.95
 ...
 210       CONTINUE                                                      !40.00
        ...
        DO 220 IS = 1, MSC                                               !40.00
          WRITE (IFS1D, 214) SPCSIG(IS)/PI2_W                            !40.00
...
!PUN        FILENM = TRIM(LOCALDIR)//DIRCH2//TRIM(FILENM)                     40.95
 ...
 310       CONTINUE                                                      !40.00
        ...
        DO 320 IS = 1, MSC                                               !40.00
          WRITE (IFS2D, 214) SPCSIG(IS)/PI2_W                            !40.00
 320    CONTINUE                                                         !40.00
!       full 2-D spectrum
     ...
     &                      'spectral Cartesian directions in degr'      !40.00
        ...
        DO 330 ID = 1, MDC                                               !40.00
          WRITE (IFS2D, 324) SPCDIR(ID,1)*180./PI_W                      !30.82
