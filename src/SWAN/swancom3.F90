...
!     &                   PLWNDA,PLWNDB,SPCDIR,ITER            )          !32.06
     &                   SPCDIR,ITER, PLWNDA,PLWNDB    ) ! JL change
!
!****************************************************************
!
      USE SWCOMM3                                                        !40.41
...
!
      ...
      REAL(rkind)  :: SWIND_EXP, SWIND_IMP    ! explicit and implicit part of wind source
!
!        INTEGERS:
!        ---------
...
!
      REAL(rkind)     WIND10,THETA ,THETAW,EDML  ,ARG1  ,ARG2  ,       &
...
! JL it is wrong, ICMAX != MCIMAX !!!
      REAL(rkind)  :: KWAVE(MSC,ICMAX)
      REAL(rkind)  :: PLWNDA(MDC,MSC,NPTST)                                     !40.00
      REAL(rkind)  :: PLWNDB(MDC,MSC,NPTST)
      REAL(rkind)  :: GENC0(MDC,MSC,MGENR)                                      !40.85
      REAL(rkind)  :: GENC1(MDC,MSC,MGENR)                                      !40.85
...
! JL For Charnock 
!      REAL(rkind)  ::  AALPHA    !Real  Public   Minimum value of charnock parameter
!
      INTEGER  IDCMIN(MSC)           , &
     &         IDCMAX(MSC)
!
...
!     Value of ITEST changed from 10 to 60 to reduce test output         !40.13
      IF ( TESTFL .AND. ITEST .GE. 60 ) THEN                             !40.13
        WRITE(PRINTF,*)
        WRITE(PRINTF,*)'subroutine WINDPAR vert.=',iplg(KCGRD(1))
...
!     8. REMARKS
!
!        IWIND = wind generation model
!              = 1 or 2 first and second generation wind wave model
!              = 3      Snyder/Komen wind input
!              = 4      Janssen wind input
!              = 5      Yan/Westhuysen wind input
!
!        IDRAG = Wind Drag Law
!              = 1 Wind drag is Wu
!              = 2 Wind drag is fit
!              = 3 Wind drag is swell
!
!     9. STRUCTURE
!
!     ------------------------------------------------------------
...
!
      REAL(rkind)         WIND10 ,THETAW ,              &                       !30.70
     &             UFRIC  ,FPM    ,CDRAG  ,SDMEAN                        !30.70
!
      REAL(rkind)         UREF   ,UTL    ,PP, QQ, RR                            !41.33
      PARAMETER (UREF=31.5, PP=0.55, QQ=2.97, RR=-1.49)                  !41.33
      REAL(rkind)         UREF1, UTL1, NSL1, NSH1, CS, CA,CB,CC, NA,NB,NC,&     !41.33
     ...
     &           NSL2=45.,NSH2=55.)                                      !41.33
      REAL(rkind)         A, B, C, D, E                                         !41.33
      REAL(rkind)         ETOTS ,EEX   ,EEY   , EAD   ,SIGMA1,                & !41.33
     &             COSDIR,SINDIR,DETOT , FAC   ,DSPR                     !41.33
!
      REAL(rkind)         WX2(MCGRD)   , &
     &             WY2(MCGRD)   , &
     &             UX2(MCGRD)   , &                                      !30.70
     &             UY2(MCGRD)                                            !30.70
!     REAL(rkind)         AC2(MDC,MSC,MCGRD)                                    !41.33
!
      LOGICAL      ANYWND(MDC)
!
      REAL(rkind)         AWX, AWY, RWX, RWY                                    !30.70
      SAVE IENT
      DATA IENT/0/
...
!     move ThetaW to the right interval, shifting + or - 2*PI            !20.64
      ...
      IF (THETAW .LT. SDMEAN - PI_W) THETAW = THETAW + 2.*PI_W
      IF (THETAW .GT. SDMEAN + PI_W) THETAW = THETAW - 2.*PI_W
!
      IF ( (THETAW - 0.5 * PI_W) .LE. SPCDIR(1,1) ) THEN                   !30.82
        IF ( (THETAW + 1.5 * PI_W) .GE. SPCDIR(MDC,1) ) THEN
          IDWMIN = 1
        ELSE
          IDWMIN = NINT ( (THETAW + 1.5*PI_W - SPCDIR(1,1)) / DDIR ) + 1   !30.82
        ENDIF
      ELSE
        IDWMIN = NINT ( (THETAW - 0.5*PI_W - SPCDIR(1,1)) / DDIR ) + 1     !30.82
      END IF
!
      IF ( (THETAW + 0.5 * PI_W) .GE. SPCDIR(MDC,1) ) THEN                 !30.82
        IF ( (THETAW - 1.5 * PI_W) .LE. SPCDIR(1,1) ) THEN                 !30.82
          IDWMAX = MDC
        ELSE
          IDWMAX = NINT ( (THETAW - 1.5 * PI_W - SPCDIR(1,1)) / DDIR ) + 1 !30.82
        ENDIF
      ELSE
        IDWMAX = NINT ( (THETAW + 0.5 * PI_W - SPCDIR(1,1)) / DDIR ) + 1   !30.82
      ENDIF
!
...
!
      IF ( TESTFL .AND. ITEST .GE. 50 ) THEN
        WRITE(PRINTF,*)'subroutine WINDP1 vert.=',iplg(KCGRD(1))
!        WRITE(PRINTF,6050) KCGRD(1), MDC, MCGRD, IWIND                   !30.21
! 6050   FORMAT(' WINDP1:INDEX MDC MCGRD IWND:',4I5)
        WRITE(PRINTF,6050) KCGRD(1), iplg(KCGRD(1)), MDC, MCGRD, IWIND
 6050   FORMAT(' WINDP1:INDEX INDEX_GL MDC MCGRD IWND:',5I8)
        WRITE(PRINTF,6052) THETAW,WIND10,WDIC,U10
 6052   FORMAT('       : THAW WIND10 WDIC U10 :',4E12.4)
        WRITE(PRINTF,6054) GRAV_W, PI_W, DDIR, VARWI
...
!****************************************************************
!
      SUBROUTINE WINDP2 (IDWMIN  ,IDWMAX  ,FPM     ,            &
     &                   ETOTW   ,                              &
!     &                   AC2     ,SPCSIG  ,                     &        !40.00
     &                            SPCSIG  ,                     &      !40.00
     &                   WIND10                                      )   !30.70
...
!
     ...
     &         IDDUM   ,ID      ,IS      ,ISFPM
!
      REAL(rkind)     ETOTW   ,FPM     ,SIG     ,ATOTD, WIND10
...
!
      IF ( TESTFL .AND. ITEST .GE. 70 ) THEN
        WRITE(PRINTF,*)
        WRITE(PRINTF,*)'WINDP2 vert.=',iplg(KCGRD(1))
...
!      SUBROUTINE WINDP3 (ISSTOP  ,ALIMW   ,AC2     ,            &
      SUBROUTINE WINDP3 (ISSTOP  ,ALIMW   ,                     &
...
!         WRITE(PRINTF,*)'WINDP3 TESTFL ITEST',TESTFL,ITEST 

!
!     *** test output ***
!
      IF (TESTFL .AND. ITEST .GE. 50) THEN
        WRITE(PRINTF,*)'subroutine WINDP3 vert.=',iplg(KCGRD(1))
...
!     &                   UFRIC   ,FPM     ,PLWNDA  , &
!     &                   IMATRA  ,SPCDIR  ,GENC0   )
      SUBROUTINE SWIND0 (SPCSIG ,SPCDIR ,UFRIC ,FPM    ,ISSTOP, & 
...
!
      REAL(rkind)     FPM     ,UFRIC   ,THETA   ,THETAW  ,  &
...
!
      ...
      STW = SIN(THETAW)                                                  !40.41
      FPM =  GRAV_W / ( 28.0 * UFRIC )
      TEMP1 = PWIND(31) / ( GRAV_W**2 * 2. * PI_W )                          !7/MAR
!      WRITE (PRTEST,*) 'SWIND0 PWIND(31) UFRIC',PWIND(31),UFRIC
      DO IS = 1, ISSTOP
        SIGMA  = SPCSIG(IS)                                              !30.72
!
!       ****            ARGU   =  FPM / SIGMA                     ***
...
!       **** with some computers to handle small numbers          ***
        ...
        DO IDDUM = IDCMIN(IS), IDCMAX(IS)
          ...
          IF ( ANYWND(ID) .AND. SIGMA .GE. (0.7 * FPM) ) THEN
            THETA  = SPCDIR(ID,1)                                        !30.82
            COSDIF = SPCDIR(ID,2)*CTW + SPCDIR(ID,3)*STW                 !40.41
!            WRITE (PRTEST,*)'SWIND0 STW CTW', STW,CTW
!            WRITE (PRTEST,*)'SWIND0 SPCDIR2 SPCDIR3', & 
!     &              SPCDIR(ID,2),SPCDIR(ID,3)
            TEMP3  = ( UFRIC *  MAX( 0. , COSDIF))**4                    !40.41
!            WRITE (PRTEST,*)'SWIND0 TMP2 TMP3 FILTER',TEMP2,TEMP3,FILTER
...
!
      IF (ITEST.GE. 60.AND.TESTFL) THEN
        WRITE(PRINTF,*)'subroutine SWIND0 vert.=',iplg(KCGRD(1))
        WRITE(PRINTF,400) KCGRD(1), THETAW*180./PI_W
...
!     &                   FPM     ,PLWNDA  ,ISSTOP  ,SPCDIR  ,ANYWND)
      ...
      SUBROUTINE SWIND3 (SPCSIG  , SPCDIR , UFRIC, FPM, ISSTOP,    &
     &                   THETAW, KWAVE   ,IMATRA  ,GENC0   ,       &
     &                   IDCMIN  ,IDCMAX  ,ANYWND, PLWNDA)
!
!****************************************************************
!
      USE SWCOMM3                                                        !40.41
...
!     SPCSIG: Relative frequencies in computational domain in sigma-space!30.72
!
      REAL(rkind), INTENT(IN) :: SPCDIR(MDC,6)                                 !30.82
...
!***********************************************************************
!
      INTEGER :: IENT, IDDUM ,ID    ,IS
!
      REAL(rkind)    :: THETA , SIGMA , SWINEB, TEMP1,   &
     &           CTW , STW ,COSDIF,               &               !40.41
     &           TEMP2 ,TEMP3 ,CINV
!
!      REAL(rkind)    AC2(MDC,MSC,MCGRD)   ,
...
!     &         PLWNDA(MDC,MSC,NPTST)                                     !40.00
      ...
      REAL(rkind), INTENT(IN)     :: FPM, UFRIC, THETAW
      REAL(rkind), INTENT(IN OUT) :: IMATRA(MDC,MSC)
      REAL(rkind), INTENT(IN OUT) :: PLWNDA(MDC,MSC,NPTST)
      REAL(rkind), INTENT(IN OUT) :: GENC0(1:MDC,1:MSC,1:MGENR)   ! generation coeff.       !40.85
      REAL(rkind), INTENT(IN)     :: KWAVE(MSC,ICMAX)                       !  40.22

      INTEGER, INTENT(IN)  :: IDCMIN(MSC) , IDCMAX(MSC)
!
      LOGICAL, INTENT(IN)  :: ANYWND(MDC)
!
!/T      LOGICAL  IMP_EXP
!
      SAVE IENT
...
!JL Bypass
      ...
      IF (ITEST.GE. 80.AND.TESTFL) THEN                                  !40.00
        WRITE(PRTEST,6000) KCGRD(1), THETAW*180./PI_W
...
#else
!JL PRINTF instead PRTEST
      IF (ITEST.GE. 80.AND.TESTFL) THEN
!40.00
        WRITE(PRINTF,*)'subroutine SWIND3 vert.=',iplg(KCGRD(1))
        WRITE(PRINTF,6000) KCGRD(1), THETAW*180./PI_W
 6000   FORMAT(' SWIND3: POINT  THETAW        :',I5,E12.4)
        WRITE(PRINTF,6100) TEMP1, FPM, UFRIC
 6100   FORMAT(' SWIND3: TEMP1 FPM UFRC     :',3E12.4, /, &
     &         '  IS ID1 ID2       Wind source term')
        DO IS = 1, MSC
          WRITE(PRINTF,6200) IS, IDCMIN(IS), IDCMAX(IS),  &
     &    (PLWNDA(ID,IS,IPTST), ID=IDCMIN(IS), IDCMAX(IS))
 6200     FORMAT(3I4, 600e12.4)
        ENDDO
        WRITE(PRINTF,*)
...
!     &                   SPCDIR  ,ANYWND  ,IT               )
     ...
     &                   SPCDIR  ,ANYWND  ,IT , PLWNDA  )    ! JL Change
!
!******************************************************************
!
      USE SWCOMM2                                                        !40.41
      ...
      USE schism_glbl, ONLY : skind,rkind,iplg
      USE VARS_WAVE, ONLY : AC2
!
      IMPLICIT NONE
!
...
!    !40.85, Aug. 08: store wind input for output purposes
!
!    ! JL April 2017 : CHarnock Coeficient computation added
!
!  2. Purpose
!
!     Computation of the source term for the wind input for a
...
!
      REAL(rkind)     THETA  ,THETAW ,DD     ,SWINEB ,WIND10 ,                &
...
!
      ...
      REAL(rkind) ZTEN,  RATIO,  BETAMX,  TXHFR,  TYHFR,  CW2,  X1,  X2,      &
...
!     JL : For Charnock Computation 
      REAL(skind), PARAMETER :: ALPHA0 = 0.0095_skind ! Bidlot 2012, Minimum value of charnock parameter):   
      REAL(skind), PARAMETER :: ZZWND = 10.0_skind    ! Wind measurement level, 10 m
      REAL(skind), PARAMETER :: KAPPA = 0.40_skind
      REAL(skind) :: SQRTCDM1
!
      SAVE IENT
      DATA IENT/0/
...
!
        DO IS = 1, MSC-1
          ...
          CW1    = SIGMA1 / WAVEN1
          CW2    = SIGMA2 / WAVEN2
          ZCN1   = MyLOG ( GRAV_W * ZE / CW1**2 )
          ZCN2   = MyLOG ( GRAV_W * ZE / CW2**2 )
...
!
  ...
  106         FORMAT(' SW4: ZOLG1-2 BETA1 BETA2:',4E12.4)
              ...
              ELSE                                                       !40.31
                TAUDIR = 0.                                              !40.31
              ENDIF                                                      !40.31
              TAUDIR = MOD ( (TAUDIR + 2. * PI_W) , (2. * PI_W) )            !40.31
              WRITE(PRINTF,107) TAUWX, TAUWY, TAUDIR*180./PI_W
...
!
        ...
        DO J=1, 50
          SIGHF2 = XIS * SIGHF1
          DS     = SIGHF2 - SIGHF1
          ZCNHF1 = MyLOG ( ZE * SIGHF1**2 / GRAV_W )
          ZCNHF2 = MyLOG ( ZE * SIGHF2**2 / GRAV_W )
          AUX    = 0.0
          SIGHF1 = SIGMAX                                                !40.61
          CW1    = GRAV_W/SIGHF1                                           !40.61
          CW2    = GRAV_W/SIGHF2                                           !40.61
          DO IDDUM = IDWMIN, IDWMAX
            ID = MOD ( IDDUM - 1 + MDC, MDC ) + 1
            THETA  = SPCDIR(ID,1)                                        !30.82
            COSDIF = SPCDIR(ID,2)*CTW + SPCDIR(ID,3)*STW                 !40.41
...
!
            FACHFR = SIGMAX**6 * AC2(ID,MSC,KCGRD(1)) * COS2 / GRAV_W**2   !30.21
...
# else
! JL Change
        SQRTCDM1  = MIN(WIND10/UFRIC,100.0)
        ZO  = ZZWND*EXP(-KAPPA*SQRTCDM1)
        ZE     = ZO / SQRT ( 1. - TAUW / TAUTOT ) 
# endif
!
        USTAR(KCGRD(1)) = UFRIC
        ZELEN(KCGRD(1)) = ZE
! JLefevre : Charnock Computation
!       CHARN(KCGRD(1)) = ALPHA / SQRT ( 1. - TAUW / TAUTOT )

# if 1
! JLefevre : Charnock Computation 

! See SUBROUTINE CALC_USTAR in WW3/w3src4md.ftn
...
!
      ...
      !  END IF
        ...
        IF (UFRIC.GT.0.001) THEN
          CHARN(KCGRD(1)) = MAX(GRAV_W*ZO/UFRIC**2,ALPHA0)
          ! Max Charnock constant 0.035 from Garratt 1977
          CHARN(KCGRD(1)) = MIN(CHARN(KCGRD(1)),0.035_skind)
        ELSE
          CHARN(KCGRD(1)) = ALPHA0
        ENDIF

        IF ( ITEST .GE. 10 .AND. TESTFL ) THEN
          WRITE(PRINTF,208) KCGRD(1),iplg(KCGRD(1)),CHARN(KCGRD(1))
 208      FORMAT(' SW4: Charn in point :',2I7,1E12.4)
          WRITE(PRINTF,*)
        ENDIF
! End JLefevre 
!
...
!
 ...
 9001   FORMAT(' SW4: POINT IDWMIN IDWMAX :',3I5)
        WRITE(PRINTF,6053) WIND10,UFRIC,THETAW*180./PI_W
...
!
      INTEGER  IENT, IDDUM  ,ID     ,IS     ,ISSTOP
!
      REAL(rkind)     UFRIC  ,THETA  ,THETAW ,SIGMA  ,SWINEB , TEMP3,  &
     &         CTW    ,STW    ,COSDIF ,                         &        !40.41
...
!
      IF (ITEST.GE. 60.AND.TESTFL) THEN
        WRITE(PRINTF,6000) KCGRD(1), THETAW*180./PI_W, UFRIC
