...
!      SUBROUTINE SBOT (ABRBOT  ,DEP2    ,ECOS    ,ESIN    ,AC2     ,& ! 41.04
      SUBROUTINE SBOT (ABRBOT  ,DEP2    ,ECOS    ,ESIN    ,         & ! 41.04
...
!
      ...
      USE VARS_WAVE, ONLY : AC2,SBF,WK,DS_INCR
...
!
      REAL(rkind)     AKN    ,XDUM   ,KD     ,SBOTEO,FACB  ,&
     &         CFW    ,FW     ,CURR   ,UC    ,ABRBOT,&
     &         ADUM   ,CDUM   ,DDUM
      REAL(rkind)     CFBOT(MSC)
      REAL(rkind)     DSP    ,ETOT   ,EEX    ,EEY   ,EAD
      REAL(rkind)     S      ,D      ,PHI    ,THETA , &                         ! 41.51
     &         DAST   ,THETAC ,RIPH   ,RIPW                              ! 41.51
!
      LOGICAL  VARFR
!
!      REAL(rkind)     AC2(MDC,MSC,MCGRD)        ,&                             ! 41.04
...
!                                              4 uFW          16 A**2
          ...
          ELSE
            FW = 0.3
          ENDIF
          CFBOT =  UBOT(KCGRD(1)) * FW / (SQRT(2.) * GRAV_W)
...
!          if nonstationary and first time step, roughness is based on grain
!          size (assumes no ripples)
              AKN = 2.5*D
...
!          if stationary and first iteration, roughness is based on grain size
!          (assumes no ripples)
              AKN = 2.5*D
              IF ( (AKN/ABRBOT).LT.0.63 ) THEN
!                friction factor based on Swart formula
...
!             roughness coefficient calculation incorporating ripple height and
!             wavelength
...
!          case of sheet flow, ripples are flattened
           ...
           ELSE
!          immobile seabed case: zero concentration and friction based on grain
!          size
...
!        PRINT *,'SBOT : CFBOT =',CFBOT
!
!       *** test output ***
!
        IF (TESTFL .AND. ITEST.GE.60) THEN
          WRITE (PRTEST, 910) IBOT, KCGRD(1),iplg(KCGRD(1)), DEP2(KCGRD(1)), CFBOT(1)
! 910      FORMAT (' SBOT :IBOT INDX DEP CFBOT:', 2I5, 2E12.4)
 910      FORMAT (' SBOT :IBOT INDX INDX_GL DEP CFBOT:', 3I8, 2E12.4)
        ENDIF
...
!
             ...
             ! JL: Please, check again and again 
              ...
              COST = SPCDIR(ID,2)
              SINT = SPCDIR(ID,3)
!              SBF(1,KCGRD(1)) = SBF(1,KCGRD(1))+               &
!     &         SINT*(WK(IS,KCGRD(1))/SPCSIG(IS))*              &
!     &         (-SBOTEO*AC2(ID,IS,KCGRD(1)))*DS_INCR(IS)*DDIR
!              SBF(2,KCGRD(1)) = SBF(2,KCGRD(1))+               &
!     &         COST*(WK(IS,KCGRD(1))/SPCSIG(IS))*              &
!     &         (-SBOTEO*AC2(ID,IS,KCGRD(1)))*DS_INCR(IS)*DDIR
              SBF(1,KCGRD(1)) = SBF(1,KCGRD(1)) -             &
     &         COST*WK(IS,KCGRD(1))*                          &
     &         (SBOTEO*AC2(ID,IS,KCGRD(1)))*DS_INCR(IS)*DDIR
...
!**********************************************************************
!*  J.Lefevre Compute SBF                                             *
!**********************************************************************
#if 0
      SUBROUTINE COMPUTE_SBF(IDCMIN, IDCMAX, ISSTOP, DISSC1)

      USE SWCOMM3, ONLY: MSC,MDC,MDISP,DDIR,KCGRD,GRAV_W
!      USE SWCOMM4
      USE OCPCOMM4, ONLY: LTRACE
      USE M_GENARR, ONLY: SPCDIR

      USE VARS_WAVE, ONLY: AC2,WK,DS_INCR,SBF
      USE schism_glbl, ONLY : rkind,dkind,iplg
!
      IMPLICIT NONE

!  4. Argument variables
!
!     DISSC1 gives the dissipation coefficient distributed over the
!     spectral space in one point of the computational grid

      REAL(rkind) ::  DISSC1(MDC,MSC,MDISP)
...
!     IS      Counter for frequency steps
      ...
      REAL(rkind) ::  SINT,COST !, DSDD,SDSDD,ADISSIP,ACT1

      SAVE IENT
      DATA IENT/0/
      IF (LTRACE) CALL STRACE (IENT,'COMPUTE_SBF')

      IF (IPLG(KCGRD(1))==9216)THEN
        write(12,*)'SBF bfr m2/s',KCGRD(1)
        write(12,*) SBF(1,KCGRD(1)),SBF(2,KCGRD(1))
      ENDIF
      ...
      DO 101 IS = 1, ISSTOP
        ...
        DO 100 IDDUM = IDCMIN(IS), IDCMAX(IS)
          ...
          ! DISSC1(ID,IS,3)  : 3 is Dissipation coeff due to bottom friction 
          SBF(1,KCGRD(1)) = SBF(1,KCGRD(1)) - &
     &      COST*WK(IS,KCGRD(1))*DISSC1(ID,IS,3)*AC2(ID,IS,KCGRD(1))* &
     &      DS_INCR(IS)*DDIR
          SBF(2,KCGRD(1)) = SBF(2,KCGRD(1)) - &
     &      SINT*WK(IS,KCGRD(1))*DISSC1(ID,IS,3)*AC2(ID,IS,KCGRD(1))* &
     &      DS_INCR(IS)*DDIR
...
!                     the discontinuity at B = 0.3 is changed in a
!                     discontinuity
...
!
      REAL(rkind)    ETOT,  HM,  KTETA, QBLOC
!
...
!     &                  ISSTOP  ,DISSC0  ,DISSC1  )                      ! 40.67 40.61 30.81 30.21
     &                  IMATDA  ,IDCMIN  ,IDCMAX  ,                  &   ! 30.81
     &                  ISSTOP  ,DISSC0  ,DISSC1  ,PLWBRK )              ! JL change
!
!****************************************************************
!
      USE SWCOMM3                                                     !  40.41
      ...
      USE VARS_WAVE, ONLY: AC2,WK,DS_INCR,SBR
...
!     REAL(rkind)     AC2(MDC,MSC,MCGRD)   ,&
      REAL(rkind)     DISSC0(MDC,MSC,MDISP),&                                   ! 40.67
...
!
      REAL(rkind)     ETOT,  HM,  QB, SMEBRK, KMESPC, KTETA                     ! 41.47 30.81
!
!  5. Parameter variables
!
!  6. Local variables
...
!     SbrD    Derivative of source term for surf breaking (Sbr) to action
!     density
!
      INTEGER          ID,       IDDUM,   IENT,   IS
      REAL(rkind)             PP,       FAC,     EPTOT,  ETOT0, FMEAN, &
     &                 ECS(MDC), FMIN,    FMAX,   FRFAC(MSC)
      REAL(dkind)      BB,       DIS0,    SbrD,                 &
     &                 SURFA0,   SURFA1,  WS  ,                 &
     &                 TEMP1 ,   TEMP2
      REAL(rkind)             SwanIntgratSpc
...
!
     ...
     &              BB**(0.5*(PSURF(5)+1))/DBLE(SQRT(PI_W))
...
!#endif
      ...
      DO 101 IS = 1, ISSTOP
        ...
        !DSDD  = DDIR * FRINTF * SPCSIG(IS)
        !SDSDD = DSDD * SPCSIG(IS)

        DO 100 IDDUM = IDCMIN(IS), IDCMAX(IS)
          ID = MOD ( IDDUM - 1 + MDC , MDC ) + 1
          !  DSSBR(IS,ID)  = SURFA1   ! JL same wwm
          !   SSBR(IS,ID)  = SURFA0 * DBLE(AC2(ID,IS,KCGRD(1)))
...
!#ifdef SCHISM
          ...
          COST = SPCDIR(ID,2) 
          SINT = SPCDIR(ID,3) 
!          SBR(1,KCGRD(1)) = SBR(1,KCGRD(1)) - &
!     &     SINT*(WK(IS,KCGRD(1))/SPCSIG(IS))*REAL(DIS0)*DS_INCR(IS)*DDIR
!          SBR(2,KCGRD(1)) = SBR(2,KCGRD(1)) - &
!     &     COST*(WK(IS,KCGRD(1))/SPCSIG(IS))*REAL(DIS0)*DS_INCR(IS)*DDIR

! Same as in WWM ....
!          SBR(1,KCGRD(1)) = SBR(1,KCGRD(1)) - &
!     &     GRAV_W*COST*WK(IS,KCGRD(1))*REAL(DIS0)*DS_INCR(IS)*DDIR
!          SBR(2,KCGRD(1)) = SBR(2,KCGRD(1)) - &
!     &     GRAV_W*SINT*WK(IS,KCGRD(1))*REAL(DIS0)*DS_INCR(IS)*DDIR

!          SBR(1,KCGRD(1)) = SBR(1,KCGRD(1)) - &
!     &     GRAV_W*COST*(WK(IS,KCGRD(1))/SPCSIG(IS))*REAL(DIS0)*DS_INCR(IS)*DDIR
!          SBR(2,KCGRD(1)) = SBR(2,KCGRD(1)) - &
!     &     GRAV_W*SINT*(WK(IS,KCGRD(1))/SPCSIG(IS))*REAL(DIS0)*DS_INCR(IS)*DDIR

! pattern good cas DUCK94 !!!  mais pas GRAV_W n'apparait pas !!!
          SBR(1,KCGRD(1)) = SBR(1,KCGRD(1)) - &
     &     COST*WK(IS,KCGRD(1))*REAL(DIS0)*DS_INCR(IS)*DDIR
          SBR(2,KCGRD(1)) = SBR(2,KCGRD(1)) - &
     &     SINT*WK(IS,KCGRD(1))*REAL(DIS0)*DS_INCR(IS)*DDIR

!          ACT1 = AC2(ID,IS,KCGRD(1))
!          ADISSIP = ADISSIP + SDSDD*(REAL(DIS0))
!#endif
#endif
...
!      write(12,*)'ADISSIP m2/S',KCGRD(1) !,ADISSIP
...
!
 ...
 6020   FORMAT ('       : HM QB ETOT SMEBRK :',4E12.4)
        WRITE(PRINTF,6019) SBR(1,KCGRD(1)),SBR(2,KCGRD(1))
 6019   FORMAT ('Moment flux vector due to wv breaking (sbr):',2E12.4)
      END IF
!
!
...
!**********************************************************************
!*  J.Lefevre Compute SBR                                             *
!**********************************************************************
#if 0
      SUBROUTINE COMPUTE_SBR(IDCMIN, IDCMAX, ISSTOP, DISSC1)
...
!  4. Argument variables
!
!     DISSC0 gives the dissipation distributed over the
!     spectral space in one point of the computational grid
...
!     IS      Counter for frequency steps
      ...
      SAVE IENT
      DATA IENT/0/
      IF (LTRACE) CALL STRACE (IENT,'COMPUTE_SBR')

      IF (IPLG(KCGRD(1))==9216)THEN
        write(12,*)'ADISSIP bfr m2/s',KCGRD(1)
        write(12,*) SBR(1,KCGRD(1)),SBR(2,KCGRD(1))
      ENDIF
      ...
      DO 101 IS = 1, ISSTOP
        ...
        DO 100 IDDUM = IDCMIN(IS), IDCMAX(IS)
          ...
          ! DISSC0(ID,IS,2)  : 2 is Dissipation due to Wave Breaking
!          SBR(1,KCGRD(1)) = SBR(1,KCGRD(1)) - &
!     &     GRAV_W*COST*WK(IS,KCGRD(1))*DISSC0(ID,IS,2)*DS_INCR(IS)*DDIR
!          SBR(2,KCGRD(1)) = SBR(2,KCGRD(1)) - &
!     &     GRAV_W*SINT*WK(IS,KCGRD(1))*DISSC0(ID,IS,2)*DS_INCR(IS)*DDIR
          ! DISSC0(ID,IS,2)  : 2 is Dissipation due to Wave Breaking
          SBR(1,KCGRD(1)) = SBR(1,KCGRD(1)) - &
     &     GRAV_W*COST*WK(IS,KCGRD(1))*DISSC1(ID,IS,2)*AC2(ID,IS,KCGRD(1))* &
     &     DS_INCR(IS)*DDIR
          SBR(2,KCGRD(1)) = SBR(2,KCGRD(1)) - &
     &     GRAV_W*SINT*WK(IS,KCGRD(1))*DISSC1(ID,IS,2)*AC2(ID,IS,KCGRD(1))* &
     &     DS_INCR(IS)*DDIR
...
!     SUBROUTINE SWCAP  (SPCDIR  ,SPCSIG  ,KWAVE   ,AC2     , &         ! 40.02
      SUBROUTINE SWCAP  (SPCDIR  ,SPCSIG  ,KWAVE            , &         ! 40.02
     &                   IDCMIN  ,IDCMAX  ,ISSTOP  ,          &         ! 40.02
!     &                   ETOT    ,IMATDA  ,IMATRA  ,PLWCAP  , &         ! 40.02
     &                   ETOT    ,IMATDA  ,IMATRA  ,          &         ! 40.02 JL Change
     &                   CGO     ,UFRIC   ,                   &         ! 40.53
     &                   DEP2    ,DISSC1  ,DISSC0, PLWCAP  )            ! 40.67 40.61 40.12 JL Change
!
!****************************************************************
!
      USE SWCOMM3                                                       ! 40.41
...
!     IDCMIN: Counter that indicates the minimum direction that is propagated in
!     the sweep
!     IDCMAX: Counter that indicates the maximum direction that is propagated in
!     the sweep
!     IMATDA: The values at the diagonal of the matrix that is solved
!     numerically
!     IMATRA: The values at the right-hand side of the equation that is solved
!     numerically
!     ISSTOP: Maximum counter in frequency space that is propagated within a
!     sweep
!     KWAVE : Wavenumber
...
!      REAL, INTENT(IN)    :: AC2(MDC,MSC,MCGRD), 
      REAL(rkind), INTENT(IN)    :: DEP2(MCGRD)
      REAL(rkind), INTENT(IN)    :: ETOT
!     Changed ICMAX to MICMAX, since MICMAX doesn't vary over gridpoint   40.22
!      REAL(rkind), INTENT(IN)    :: KWAVE(MSC,MICMAX)                           ! 40.22
! JL Wrong ICMAX != MICMAX
      REAL(rkind), INTENT(IN)    :: KWAVE(MSC,ICMAX)
      REAL(rkind), INTENT(IN)    :: SPCDIR(MDC,6), SPCSIG(MSC)
      REAL(rkind), INTENT(OUT)   :: PLWCAP(MDC,MSC,NPTST)
      REAL(rkind), INTENT(INOUT) :: IMATDA(MDC,MSC), IMATRA(MDC,MSC)
      REAL(rkind), INTENT(INOUT) :: DISSC0(MDC,MSC,MDISP),DISSC1(MDC,MSC,MDISP) ! 40.67 40.12
      REAL(rkind), INTENT(IN)    :: UFRIC                                       ! 40.53
!      REAL(rkind), INTENT(IN)    :: CGO(MSC,MICMAX)                             ! 40.53
! JL Wrong ICMAX != MICMAX
      REAL(rkind), INTENT(IN)    :: CGO(MSC,ICMAX)
!
!  6. Local variables
!
...
!     expression
!     SIG0  : Average zero-crossing frequency used in the Longuet Higgins
!     expression
...
!
      REAL(rkind)              :: A, C_BJ, HM, HRMS, N1, N2
      REAL(rkind)              :: QB_WC, SIG0, STP_OV, STP_PM
!
      REAL(rkind), ALLOCATABLE :: C_K(:), C_LH(:), WCAP(:), WCIMPL(:)

      REAL(rkind)              :: B, P, BRKD, FBR                             ! 40.63 40.53
      REAL(rkind)              :: PWCAP1, PWCAP2, PWCAP9, PWCAP10, PWCAP11    ! 40.63
      REAL(rkind)              :: EF(MSC)                                     ! 40.53
!
!     8. REMARKS
!
!        alpha : PWCAP( 7)
...
!
      IF ((IWCAP.EQ.1).OR.(IWCAP.EQ.2).OR. &
...
!
      ENDIF
!
      IF ((IWCAP.EQ.4).OR. (IWCAP.EQ.5)    ) THEN
!
...
!
        ...
        CALL FRABRE(HM, ETOT, QB_WC, 1._rkind)
!
! Calculate C_BJ
!
        IF (HRMS.GE.HM) THEN
...
!                                                                         40.53
     ...
     &    (GRAV_W**(0.5)*KWAVE(IS,1)**(0.5)/SPCSIG(IS))**(PWCAP(10)/2-1) *&! 40.53
     &    GRAV_W**(0.5)*KWAVE(IS,1)**(0.5)                                &! 40.53
...
!      SUBROUTINE BRKPAR (BRCOEF  ,ECOS    ,ESIN    ,AC2     ,   &  ! 40.22
      SUBROUTINE BRKPAR (BRCOEF  ,ECOS    ,ESIN             ,   &  ! 40.22
...
!  4. Argument variables
!
      REAL(rkind), INTENT(OUT) :: BRCOEF    ! variable breaker coefficient       40.22
      REAL(rkind), INTENT(OUT) :: KTETA     ! number of directional partitions   41.47

!     SPCSIG: Relative frequencies in computational domain in sigma-space 30.72
!
      REAL(rkind), INTENT(IN)  :: SPCSIG(MSC)                                   ! 30.72
!     REAL(rkind), INTENT(IN)  :: AC2(MDC,MSC,MCGRD)   ! action densities        40.22
      REAL(rkind), INTENT(IN)  :: ECOS(MDC), ESIN(MDC) ! Cos and Sin of Theta    40.22
      REAL(rkind), INTENT(IN)  :: DEP2(MCGRD)          ! depths at grid points   40.22
      REAL(rkind), INTENT(IN)  :: BOTLV(MCGRD)         ! bottom depth            41.38
      REAL(rkind), INTENT(INOUT)  :: GAMBR(MCGRD)      ! breaker parameter       41.38
      REAL(rkind), INTENT(INOUT)  :: RESPL(MCGRD)      ! response length         41.38
!
!     RDX, RDY:  coefficients to obtain spatial derivatives               40.22
!      REAL(rkind), INTENT(IN)  :: RDX(MICMAX), RDY(MICMAX)                      ! 40.08
! JL Wrong RDX RDY dim = 2
      REAL(rkind), INTENT(IN)  :: RDX(2), RDY(2)
!     REAL(rkind), INTENT(IN)  :: KWAVE(MSC,MICMAX)                             ! 41.03
! JL Wrong MICMAX != ICMAX
      REAL(rkind), INTENT(IN)  :: KWAVE(MSC,ICMAX) 
      REAL(rkind), INTENT(IN)  :: FDIR ! represents first spectral direction     41.38
...
!
      REAL(rkind)  :: ETOTS ,EEX   ,EEY   ,               &
     &         EAD   ,SIGMA1,COSDIR,SINDIR,DDDX  , &                    !  40.22
     &         DDDY  ,DDDS  ,DETOT                                     !  40.22
      REAL(rkind)  :: EMAX, ETD, KP, KPD                               !  41.03
      REAL(rkind)  :: DSPR  ! directional spread in radians                    !  41.38
      REAL(rkind)  :: FAC1, FAC2                                       !  41.47

! vars related with FVCOM grid
!      INTEGER :: J,I1,I2,IG
!      REAL(SP) :: F1
!# if defined (SPHERICAL)
!      REAL(DP) XTMP,XTMP1
...
!
      IF (ISURF .EQ. 1 ) THEN                                          !  41.38 41.03
...
!
     ...
     &         + RDY(2) * (BOTLV(KCGRD(1)) - BOTLV(KCGRD(3)))
         ...
         DDDS = -1._rkind * ( DDDX * COSDIR + DDDY * SINDIR )

         IF ( TESTFL .AND. ITEST .GE. 40 ) THEN
          WRITE(PRINTF,580) KCGRD(1),SINDIR,COSDIR,RDX(1),RDX(2),&
     &         RDY(1),RDY(2)
 580      FORMAT ('BRKPAR: point nr,sindir,cosdir,rdx1,rdx2,rdy1,rdy2:',&
     &          I7,6(1X,E12.4))
         END IF
...
!
         ...
         IF ( (1./ABS(DDDS)) .LT. 10._rkind ) DDDS = 0.1_rkind
!
...
!
         IF ( FAC1.GT.0._rkind) THEN
            IF ( FAC2.GT.0._rkind ) THEN
               IF ( FAC1.GT.5._rkind*FAC2 ) THEN
                  BRCOEF = FAC1
               ELSE
...
!
         IF ( ETOTS .GT. 0._rkind ) THEN
            COSDIR = EEX / ETOTS
            SINDIR = EEY / ETOTS
            FAC1   = MIN( 1._rkind, SQRT(COSDIR**2+SINDIR**2) )
            DSPR   = SQRT(2._rkind-2._rkind*FAC1)
         ELSE
            DSPR   = 0._rkind
         ENDIF
!
         KTETA = MAX(1._rkind,DSPR/((PSURF(15)/180._rkind)*PI_W))
      ELSE
         KTETA = 1._rkind
      ENDIF
!
...
!
      IF ( TESTFL .AND. ITEST .GE. 40 ) THEN
        WRITE(PRINTF,600) KCGRD(1), ATAN2(SINDIR,COSDIR)*180./PI_W, &
...
!
      ...
      REAL(rkind)    A, B, C, D, KD, KVEGH, LAYPRT, SINHK, SLAYH,         &
...
!  4. Argument variables
      ...
      REAL(rkind)             :: IMATDA(1:MDC,1:MSC)         ! main diagonal of matrix
      REAL(rkind), INTENT(IN) :: KWAVE(1:MSC,1:ICMAX)        ! wave number
      REAL(rkind)             :: DISSC1(1:MDC,1:MSC,1:MDISP) ! total dissipation in spectral domain
      REAL(rkind), INTENT(IN) :: TURBV2(1:MCGRD)             ! turbulent viscosity at current time
      REAL(rkind), INTENT(IN) :: DEP2(1:MCGRD)               ! water depth
      REAL(rkind)             :: PLTURB(MDC,MSC,NPTST)
...
!  5. Local variables
      ...
      REAL(rkind) :: CVISC       ! dissipation coefficient
      REAL(rkind) :: VISCLOC     ! local turbulent viscosity
      REAL(rkind) :: XKD         ! dimensionless depth
...
!
      REAL(rkind)             :: IMATDA(1:MDC,1:MSC)
      REAL(rkind), INTENT(IN) :: CGMUD(1:MSC,1:ICMAX)
      REAL(rkind), INTENT(IN) :: DMW(1:MSC,1:ICMAX)
      REAL(rkind), INTENT(IN) :: KMUD(1:MSC,1:ICMAX)
      REAL(rkind)             :: DISSC1(1:MDC,1:MSC,1:MDISP)
      REAL(rkind), INTENT(IN) :: DEP2(1:MCGRD)
      REAL(rkind)             :: PLMUD(MDC,MSC,NPTST)
