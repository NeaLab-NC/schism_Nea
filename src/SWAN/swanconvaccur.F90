...
subroutine SwanConvAccur ( accur, hscurr, tmcurr, delhs, deltm, xytst, spcsig,      ivlow, ivup )
!
!   --|-----------------------------------------------------------|--
...
!
    ...
    !
    real(rkind), intent(out)                           :: accur  ! percentage of active vertices in which required accuracy has been reached
...
!
    ...
    real(rkind)                                  :: hsoval   ! required accuracy with respect to overall error in wave height
    real(rkind)                                  :: hsprev   ! wave height at previous iteration level
    real(rkind)                                  :: hsrel    ! required accuracy with respect to relative error in wave height
    real(rkind)                                  :: hsmean   ! space-averaged wave height at previous iteration level
    real(rkind)                                  :: hsmeant  ! hsmean variable for calling thread
    ...
    real(rkind)                                  :: tmmean   ! space-averaged mean period at previous iteration level
    real(rkind)                                  :: tmmeant  ! tmmean variable for calling thread
    real(rkind)                                  :: tmabs    ! absolute difference in mean period between last 2 iterations
    real(rkind)                                  :: tmoval   ! required accuracy with respect to overall error in mean period
    real(rkind)                                  :: tmprev   ! mean period at previous iteration level
    real(rkind)                                  :: tmrel    ! required accuracy with respect to relative error in mean period
    !
!   For Parallel run :
    REAL(dkind), DIMENSION(3) :: SBUF,RBUF1               ! For Sum/Max/Min of quantitie over partition
...
!
    ...
    !
    ! # if defined (MULTIPROCESSOR)
    IF(.NOT.SERIAL) call MPI_REDUCE(SBUF,RBUF1,3,MPI_DOUBLE,MPI_SUM,0,comm,ierr)

    nwetp = NINT(RBUF1(1))
    hsmean= RBUF1(2)
    tmmean= RBUF1(3)
    !
    hsmean = hsmean/DBLE(nwetp)
    tmmean = tmmean/DBLE(nwetp)
!    PRINT *,'space-averaged wave height',  hsmean
!    PRINT *,'space-averaged mean period', tmmean
    !$omp end single
    !
    ! calculate a set of accuracy parameters based on relative and overall accuracy measures for Hs and Tm
    !
!    PRINT *,'calculate a set of accuracy parameters based on relative and overall accuracy'
...
!          PRINT *,' hscurr at ivert',ivert,hscurr(ivert)
...
!          PRINT *,' tmcurr at ivert',ivert,tmcurr(ivert)
          !
...
!          PRINT *,' required Accu hsrel vs hsoval :',hsrel,hsoval
          !
...
!    PRINT *,' npacc nwetp',npacc,nwetp
    !
    ! perform global reductions in parallel run
    !
    SBUF = 0.0_dkind ; RBUF1 = 0.0_dkind

    RBUF1(1) = MyREAL(npacc)
    SBUF(1)  = MyREAL(npacc)
    !
    ! Sum over partition
    !
    ! # if defined (MULTIPROCESSOR)
    IF(.NOT.SERIAL) call MPI_REDUCE(SBUF,RBUF1,1,rtype,MPI_SUM,0,comm,ierr)
    ...
    !$omp barrier
    !$omp single
    !accur = MyREAL(npacc)*100./MyREAL(nwetp)
    accur = ceiling(DBLE(npacc)*10000./DBLE(nwetp))/100.
    !$omp end single
    !
