...
!   Argument variables
!
    real(rkind), dimension(MSC,ICMAX), intent(out) :: cgo    ! group velocity
    real(rkind), dimension(nverts), intent(in)     :: dep2   ! water depth at current time level
    real(rkind), dimension(MSC,ICMAX), intent(out) :: dmw    ! mud dissipation rate
    real(rkind), dimension(MSC,ICMAX), intent(out) :: kwave  ! wave number
    real(rkind), dimension(nverts), intent(in)     :: mudl2  ! mud thickness at current time level
...
!
    ...
    real(rkind)                 :: dm       ! local mud layer
    real(rkind), dimension(MSC) :: n        ! ratio of group and phase velocity
    real(rkind), dimension(MSC) :: nd       ! derivative of N with respect to depth
!
!   Structure
!
