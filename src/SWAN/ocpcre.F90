...
!
      ...
      !print*,KONT,' ',NAAM,' ',DRSTA,RDBL,'final',R
...
!****************************************************************
!                                                               *
      SUBROUTINE SCAN_INLOGC (NAAM, L, KONT, LSTA)
!                                                               *
!****************************************************************
!
...
!
      INPFIL = "./INPUT"
      ISCAN = SCAN_SWINPUT(INPFIL,NAAM,LVAL = LTMP)
      IF(ISCAN == 0)THEN
        L = LTMP
      ELSE
        IF(KONT == 'STA')THEN
        ...
        ELSE IF(KONT == 'REQ')THEN
          ...
          CALL parallel_abort('SCAN_INLOGC ABORT')
        END IF
      END IF
...
!JQI   END IF
      ...
      RETURN
      END SUBROUTINE SCAN_INLOGC

 
!****************************************************************
!                                                               *
      SUBROUTINE SCAN_INREAL (NAAM, R, KONT, RSTA)
!                                                               *
!****************************************************************
!
      USE schism_glbl, ONLY : errmsg
      ...
      USE OCPCOMM3                                                        
      USE OCPCOMM4 
      USE schism_glbl, ONLY:skind,dkind,rkind
      USE SWSCAN                                                      
!
      IMPLICIT NONE
!
...
!
      ...
      CHGVAL = .FALSE.
      ISCAN = SCAN_SWINPUT(INPFIL,NAAM,FSCAL = FNTMP)

      !WRITE(PRINTF,*)'SCAN_INREAL FNTMP',FNTMP
      ...
      ELSE
       ...
       ELSE IF(KONT == 'REQ')THEN  
        WRITE(PRINTF,*)'ERROR READING ',NAAM,': ',ISCAN
        CALL parallel_abort('SCAN_INREAL')
       END IF
      END IF
...
!JQI   END IF        
      ...
      !WRITE(PRINTF,*)'SCAN_INREAL R',R

      RETURN
      END SUBROUTINE SCAN_INREAL
 
!****************************************************************
!                                                               *
      SUBROUTINE SCAN_ININTG (NAAM, IV, KONT, ISTA)
!                                                               *
!****************************************************************
!
      USE schism_glbl, ONLY : errmsg
...
!JQIJQI   END IF        
      ...
      ISCAN = SCAN_SWINPUT(INPFIL,NAAM,ISCAL = INTMP)
      IF(ISCAN == 0)THEN
        IV = INTMP
      ELSE
        IF(KONT == 'STA')THEN
        ...
        ELSE IF(KONT == 'REQ')THEN    
          WRITE(PRINTF,*)'ERROR READING ',NAAM,': ',ISCAN
          CALL parallel_abort('SCAN_ININTG ABORT')
        END IF
      END IF    
      
      RETURN
      END SUBROUTINE SCAN_ININTG
 
!****************************************************************
!                                                               *
      SUBROUTINE SCAN_INCSTR (NAAM, C, KONT, CSTA)
!                                                               *
!****************************************************************
!
      USE schism_glbl, ONLY : errmsg
...
!
      INPFIL = "./INPUT"
      ISCAN = SCAN_SWINPUT(INPFIL,NAAM,CVAL = CHTMP)
      IF(ISCAN == 0)THEN
        C = TRIM(CHTMP)
      ELSE
        IF(KONT == 'STA')THEN
        ...
        ELSE IF(KONT == 'REQ')THEN  
         WRITE(PRINTF,*)'ERROR READING ',NAAM,': ',ISCAN
         CALL parallel_abort('SCAN_INCSTR ABORT')
        END IF
      END IF  
...
!JQI   END IF        
      ...
      RETURN
      END SUBROUTINE SCAN_INCSTR

!****************************************************************
!                                                               *
      SUBROUTINE SCAN_INCTIM (IOPTIM, NAAM, RV, KONT, RSTA)
!                                                               *
!****************************************************************
!
     USE schism_glbl, ONLY : errmsg,rkind,dkind
     USE schism_msgp, ONLY : parallel_abort
...
!     NAAM   : name of the variable according to the user manual
!     KONT   : What to do with the variable?
!              ='REQ'; error message if no value is found in the input
!              file
!              ='UNC'; If no value, then variable will not be changed
!              ='STA'; If no value, then variable will get default value
!              ='RQI'; Variable may not have the value of RSTA
...
!JQI     ELSE IF(KONT == 'REQ')THEN
       WRITE(PRINTF,*)'ERROR READING ',NAAM,': ',ISCAN
       CALL parallel_abort('SCAN_INCTIM ABORT')
!JQI     END IF
      END IF
...
!==============================================================================!
      ...
      SUBROUTINE GET_VALUE(LNUM,NUMCHAR,TEXT_LINE,VARNAME,VARTYPE,&
     &                     LOGVAL,STRINGVAL,REALVAL,INTVAL,NVAL)
...
!==============================================================================!
      ...
      REAL(dkind), INTENT(INOUT) :: REALVAL(150)
...
!
      ...
      IF(EQLOC == 0) then
        ...
        !&'Header comment lines must start with "!", Data lines must ', &
        !&'contain "="',&
        !&'DATA LINE '//ERRSTRING//': This often occurs if the header', &
        !&' variable is missing!'
        !DBG:
        !WRITE (PRINTF,*) ' **INFO GET_VALUE: next'
        VARTYPE = "no data"
        VARNAME = "no data"
        RETURN
      ENDIF
...
!
      ...
      IF(LVARVAL == 0) then
         WRITE (PRINTF,*) ' ** WARNING : ',&
     &   'IN DATA PARAMETER FILE VARIABLE'//VARNAME// &
     &   '; LINE'//ERRSTRING//' HAS NO ASSOCIATED VALUE'
        VARTYPE = "no data"
        VARNAME = "no data"
        RETURN
      END IF
