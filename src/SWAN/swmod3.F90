...
! physical constants
!
    real(rkind) sqrtg   ! square root of gravity
    real(rkind) gsq     ! square of gravity
    real(rkind) nu      ! kinematic viscosity of water
!
    real(rkind) d_water ! density of water
    real(rkind) d_air   ! density of air

    real(rkind) trshdep ! treshold depth (=DEPMIN as given by SWAN)
!
! mathematical constants
!
    real(rkind) pih    ! pi/2
    real(rkind) dera   ! conversion from degrees to radians
    real(rkind) rade   ! conversion from radians to degrees
    real(rkind) expmin ! min argument for exp. function to avoid underflow
    real(rkind) expmax ! max argument for exp. function to avoid overflow
    real(rkind) sqrt2  ! square root of 2 ~ 1.41
!
    contains
!
!------------------------------------------------------------------------------
...
!
    ...
    pih  = 0.5_rkind*PI_W
    dera = PI_W/180.0_rkind
    rade = 180.0_rkind/PI_W
!
    expmin = -20.0_rkind
    expmax =  20.0_rkind
!
...
!------------------------------------------------------------------------------
    ...
    real(rkind), intent(in)  :: grav_w   !  gravitational acceleration (m/s^2)
...
!
       do iter=1,maxit
          ...
          s = sqrt(fm**2-fl*fh)
          if(s == 0.) goto 9000
          xnew = xm+(xm-xl)*(sign(1._rkind,fl-fh)*fm/s)
!
!           if(luprint>0) write(luprint,'(a,4e13.5)')
...
!
!-----------------------------------------------------------------------------!
    real(rkind) function z_wnumb(w,d,grav_w)
!-----------------------------------------------------------------------------!
!
!   +-------+    ALKYON Hydraulic Consultancy & Research
!   |       |    Gerbrant van Vledder
...
!-------------------------------------------------------------------------
      ...
      real(rkind), intent(in)     :: grav_w          ! Gravitational acceleration
...
!    08/08/2002  Upgrade to release 4.0
!    13/08/2002  Indexing in terms of integers and real(rkind) weights
...
!! \A
!!    real(rkind) x_kfunc                 ! real(rkind) function to compute wave number
!! \Z
!
...
!
!-------------------------------------------------------------------------------
!     Convert real(rkind) indices to integer indexing and real(rkind) weights
!
...
!
      ...
      real(rkind) diold           ! 'real' old number of indices between succeeding lumped bins
      real(rkind) dinew           ! 'real' new number of indices between succeeding lumped bins
!
!!    real(rkind) x_disper        ! evaluate dispersion relation
...
!
      ...
      real(rkind) rpar                  ! real(rkind) parameter
!--------------------------------------------------------------------------------
!
...
! functions used
!
!!    real(rkind) x_kfunc      ! function to invert computation of wieghts
!------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
      real(rkind) function x_cple(k1x,k1y,k2x,k2y,k3x,k3y,k4x,k4y,iq_cple,       &
                           depth,grav_w)
!------------------------------------------------------------------------------
!
!   +-------+    ALKYON Hydraulic Consultancy & Research
...
! real(rkind) function x_locus1 moved to before subroutine q_locpos because of compilation problems
! real(rkind) function x_locus2 moved to before subroutine q_locpos because of compilation problems
...
!------------------------------------------------------------------------------
      real(rkind) function xc_webb(k1x,k1y,k2x,k2y,k3x,k3y,k4x,k4y,grav_w)
!------------------------------------------------------------------------------
!
!   +-------+    ALKYON Hydraulic Consultancy & Research
!   |       |    Gerbrant van Vledder
