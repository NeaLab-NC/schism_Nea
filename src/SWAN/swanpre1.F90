...
! 12/2018 JLEFEVRE Update
      ...
      USE schism_glbl, ONLY : skind,rkind,pi,nws,np_global,np,npa,nea
      USE schism_glbl, ONLY : elnode,dp,ics,xnd,ynd,isonb_w,xlon,ylat  !,rearth
      USE schism_glbl, ONLY : errmsg,nope_global,nond_global,iond_global
      USE schism_glbl, ONLY : iplg,wtiminc,neta_global,ipgl
      USE schism_glbl, ONLY : msc2,mdc2 !,IOBCN_GL_W,IOBCN_W,I_OBC_N_W,I_OBC_GL_W
      USE schism_glbl, ONLY : rearth_eq
      USE misc_modules


! 12/2018 JLEFEVRE Update end
...
!   !30.01, Feb. 99: Introducing STPNOW
...
!
      ...
      REAL(rkind)       :: TRCF,HGT,SLP,BK,OGAM,OBET,REF0,XP,YP
      REAL(rkind)       :: ALTMP,FRLOW,FRHIG,GAMMA,TMPDIR,VALX,VALY
      REAL*8            :: DIFF
      REAL(rkind)              :: DEGCNV
      REAL(rkind)              :: HH, CC, DD
      INTEGER           :: NN
...
!
      ...
      REAL(rkind)           :: POWN, DUM                                !40.31 40.18
      REAL(rkind)           :: FD1, FD2, FD3, FD4                       !40.31 40.28
      REAL(rkind), ALLOCATABLE :: RLAMBDA(:)                            !40.17

      CHARACTER*6  QOVSNM                                               !40.87
      CHARACTER*40 QOVLNM                                               !40.87
      REAL(rkind)         QR(9)                                         !40.87
      REAL*8       DVAL                                                 !40.87
...
!JL      CHARACTER PSNAME *8, PNAME *8, COMPUT *(*), PTYPE *1, DTTIWR *18    !40.00
      CHARACTER PSNAME *8, PNAME *8, PTYPE *1, DTTIWR *18               !40.00
...
! SWAN in SCHISM : local vars 
      ...
      LOGICAL :: INP_BOT,INP_CUR,INP_CUR_STAT,INP_CUR_SERI,INP_WI,    &
                 INP_WI_STAT,                                         &
                 INP_WI_SERI,INP_FR,INP_FR_STAT,INP_FR_SERI,INP_WLEV, &
                 INP_WLEV_STAT,INP_WLEV_SERI

!      CHARACTER(LEN=80) :: PROJJ,SET,MODE,COORDINATES,BOUNDARY,       &
!                 CONSTANT,FRIC_FORM
      CHARACTER(LEN=2) :: BOUNDARY
      LOGICAL :: CHECK, KEY_SCGRID, KEY_SCBOU, KEY_SCCOMP
      REAL(rkind), ALLOCATABLE :: UXB_GL(:,:),UYB_GL(:,:), &
                                  FRIC_GL(:,:), WLEVL_GL(:,:)
!   INTEGER :: I,IGR2,IGRD,IGR1,J,JJ,NUMCOR,I1,NCNT
      INTEGER :: I1, IGR1, istat, NCNT
      INTEGER :: K
!   INTEGER :: ITRAS
      INTEGER, ALLOCATABLE :: TEMP1(:),TEMP2(:)
      INTEGER :: nb_OBC_seg, nb_INT_vert, nb_OBC_vert, nb_LND_vert

      REAL(rkind) :: WIND_X, WIND_Y
      CHARACTER(LEN=6)  UNIT
! End SWAN in SCHISM : local vars

! DBG
      CHARACTER(LEN=120) FILEDBG
      INTEGER :: ILPOS

      CALL STRACE (IENT, 'SWREAD')

!      print *,'SWREAD : Reading '//TRIM(FILENM)

! JL : NML section to scan differently, by using 
! JL ! the FVCOM NML scanning method, not using the SWAN one
      KEY_SCGRID = .TRUE.
      KEY_SCBOU  = .TRUE.
      KEY_SCCOMP = .TRUE.
!
...
! ============================================================
!
!JL      IF (KEYWIS ('STOP')) THEN
!JL        IF (RUNMADE) THEN
!JL          COMPUT = 'STOP'
!JL        ELSE
!JL          WRITE (PRINTF,*)' ** No computation requested **'
!JL          COMPUT = 'NOCO'
!JL        ENDIF
!JL        RETURN
!JL      ENDIF
...
!     -----------------------------------------------------------------------
!
! 03/2017 JLEFEVRE : defining boundary conditions
!                    before grid definition to fill
!                    vmark
!
...
!     See schism_init.F90 
! Note :
!  isonb_w(i) can be = 1 in absence of solid boundary !!
!==============================================

!JL : THE FOLLOWING SECTION With KEYWORDS SCBOU IS NOT
!JL ! RELEVANT WITH THE SWAN NML SCANNING METHOD
!JL : we manage to skip that following section, by using  RDINIT
!JL : (see opcrce;F)

!      WRITE(PRINTF,*) 'swread dbg 1'

      IF (KEYWIS ('SCBOU')) THEN
       !CALL MSGERR (1, 'Keyword SCBOU indicates aliens inputs, bypass')
       CALL RDINIT
!      CALL NWLINE
       GO TO 100
      ENDIF

!      WRITE(PRINTF,*) 'swread dbg 2'


      IF(KEY_SCBOU) THEN

       KEY_SCBOU = .FALSE. !First and last visit

       CALL SCAN_INCSTR('SCBOU BOUNDARY',BOUNDARY,'REQ',' ')

       IF(BOUNDARY /= 'default')THEN

!----------------Determine Number of Nodes on Outer Boundary-------------------!
!
!        WRITE(PRINTF,*) 'Set nb of Nodes on Outer Boundary..'
        IOBCN_GL_W = neta_global  
        IOBCN_W = 0
!        WRITE(PRINTF,*)'IOBCN_GL_W',IOBCN_GL_W

        IF(IOBCN_GL_W.GT.0)THEN

!------------Read in Open Boundary Nodes --------------------------------------!
...
!         GLOBAL
          ...
          NCNT = 0
          DO K=1,nope_global
           NCNT = NCNT + nond_global(K)
          ENDDO
          IF(IOBCN_GL_W.NE.NCNT) THEN
            write(errmsg,*) 'Something wrong in counting OBC nodes'
            call parallel_abort(errmsg)
          END IF

          IF(myrank.EQ.0) &
           WRITE(PRINTF,*) 'LOCAL OPEN BOUNDARY NODES, Ok to start'  

          I=0
          DO K=1,nope_global
           DO J=1,nond_global(K)
           I=I+1
           I_OBC_GL_W(I)=iond_global(K,J)
           !WRITE(PRINTF,*) I_OBC_GL_W(I)
!          Make Sure It Is In Global Domain
           IF((I_OBC_GL_W(I) > np_global))THEN
           write(errmsg,*)'OPEN BOUNDARY NODE NUMBER',I,'IS NOT IN THE',&
     &    'GLOBAL DOMAIN. CHECK INPUT FILE AND ENSURE OPEN BOUNDARY '  ,&
     &    'NODES <=',np_global
           call parallel_abort(errmsg)
           END IF
           ENDDO
          ...
          IF(myrank.EQ.0) &
           WRITE(PRINTF,*) 'LOCAL OPEN BOUNDARY NODES, PARALLELIZE'
...
!         Fill LOCAL array
          ...
          ELSE !// Parallel
            ...
            IF(myrank.EQ.0) &
             WRITE(PRINTF,*) 'SET UP LOCAL OPEN BOUNDARY NODES' 

            DO I=1,IOBCN_GL_W
             I1 = I_OBC_GL_W(I)
           
!            IF(myrank.EQ.11) print*,'testing ',I1

 
             IF (ipgl(I1)%id.gt.0.and.ipgl(I1)%rank.EQ.myrank) THEN
!              IF(myrank.EQ.11) print*,'yes',I1
              NCNT = NCNT + 1
              TEMP1(NCNT) = ipgl(I1)%id !iplg(I1)
!             IF (ipgl(I1)%rank.EQ.myrank) THEN
!             NCNT = NCNT + 1
!             TEMP1(NCNT) = ipgl(I1)%id !iplg(I1)
             END IF
            END DO
...
!adjust ISONB_W according to SCHISM
! already done in Hydro/schism_init.F90

!        ISONB_W = ISONB

!        DO I=1,npa
!           IF(ISONB(I)/=0) ISONB_W(I)=1 !separate interior points and outer boundary
!        ENDDO
!
!        DO I=1,IOBCN_W
!           I1=I_OBC_N_W(I)
!           ISONB_W(I1)=2                !separate open boundary from outer boundary
!        ENDDO

        END IF !!IOBCN_GL_W > 0

!        CALL SWBOUN ( )
!        print*,'after SWBOUN NESTING=',NESTING
!        deplaced after section CGRID 

       ELSE  !! not BOUNDARY /= 'default'
...
!      WRITE(PRINTF,*) 'swread dbg 6'
      ...
      IF (KEYWIS ('SET')) THEN
         ...
         CALL INREAL ('GRAV',   GRAV_W, 'UNC', 0.)
         CALL INREAL ('RHO',    RHO_W,  'UNC', 0.)
...
!
         ...
         IF (CHGVAL) THEN
           ...
           PWTAIL(3) = PWTAIL(1) + 1.
         ENDIF
         CALL INREAL ('FROUDMAX', PNUMS(18), 'UNC', 0._rkind)                 !30.50
         CALL INREAL ('SORT'    , asort    , 'UNC', 0._rkind)                 !41.48
...
!        so not documented
         ...
         if ( asort.gt.-999. ) then                                     !41.48
            asort = DEGCNV (asort)
            ALTMP = asort / 360._rkind
            asort = 2 * PI_W * (ALTMP - NINT(ALTMP))
         endif
         GOTO 100
      ENDIF
!
...
!
        ...
        IF (KEYWIS('UNSTRUC')) THEN
          ...
          YPC   = 0._rkind                                                    !40.80
          ALPC  = 0._rkind                                                    !40.80
          XCLEN = 0._rkind                                                    !40.80
          YCLEN = 0._rkind                                                    !40.80
        ENDIF

        IF (OPTG.NE.5) THEN                                             !40.95
          CALL MSGERR(4,"CGRID : UNSTRUCTURED KeyWord not found ?")
          RETURN
        ENDIF

        CALL INREAL('ALPC',ALPC,'UNC',0._rkind)
!
!         ALPC is made to be between -PI and PI
!
          ALTMP = ALPC / 360._rkind
          ALPC = 2* PI_W * (ALTMP - NINT(ALTMP))
          CVLEFT = .TRUE.                                               !40.00

!        WRITE(PRINTF,*) 'swread dbg 101'

        CALL INKEYW ('STA', 'CIR')                                      !20.67
        IF (KEYWIS('SEC')) THEN
          FULCIR = .FALSE.
          CALL INREAL ('DIR1', SPDIR1, 'REQ', 0._rkind)                       !20.43
          CALL INREAL ('DIR2', SPDIR2, 'REQ', 0._rkind)                       !20.56
...
!
        CALL INREAL('FLOW',FRLOW,'STA',-999._rkind)                           !40.31
        CALL INREAL('FHIGH',FRHIG,'STA',-999._rkind)                          !40.31
        CALL ININTG('MSC',MSS,'STA',-999)                               !40.31

        WRITE (PRINTF,*)'FLOW',FRLOW
        WRITE (PRINTF,*)'FHIGH',FRHIG
        WRITE (PRINTF,*)'MSC',MSS
...
!         purpose: prevent problem with grids under 45 degrees          !40.13
          SPDIR1 = ALPC + 0.5_rkind * DDIR                              !30.72 40.13
        ELSE
          IF (BNAUT) THEN                                               !30.70
!           swap values of SPDIR1 and SPDIR2, and transform             !30.70
            TMPDIR = SPDIR1                                             !30.70
            SPDIR1 = 180._rkind + DNORTH - SPDIR2                       !30.70
            SPDIR2 = 180._rkind + DNORTH - TMPDIR                       !30.70
          ENDIF                                                         !30.70
          SPDIR1 = SPDIR1 * PI_W / 180._rkind                           !30.50
          SPDIR2 = SPDIR2 * PI_W / 180._rkind                           !30.50
          IF (SPDIR2.LT.SPDIR1) SPDIR2 = SPDIR2 + 2*pi_W                !20.56
        ...
        ! Check out (JL, SCHISM) 
        IF(msc2.NE.MSC) THEN
           write(errmsg,*)'PLEASE, CHECKOUT INPUT and param.in files',&
     &    'SCHISM msc2=',msc2,' but SWAN msc=',MSC
           call parallel_abort(errmsg)
        ENDIF

        IF(mdc2.NE.MDC) THEN
           write(errmsg,*)'PLEASE, CHECKOUT INPUT and param.in files',&
     &    'SCHISM mdc2=',mdc2,' but SWAN mdc=',MDC
           call parallel_abort(errmsg)
        ENDIF
        ! End Check out (JL, SCHISM)
!
...
!
 ...
 6051     FORMAT (' MDC: ',I6,' MSC: ',I6, ' DDIR: ', F6.3)             !40.80
        ENDIF
!
!JL        IF(.NOT.ALLOCATED(XCGRID)) ALLOCATE(XCGRID(MXC,MYC))            !40.31
!JL        IF(.NOT.ALLOCATED(YCGRID)) ALLOCATE(YCGRID(MXC,MYC))            !40.31
!
        IF(.NOT.ALLOCATED(XCGRID)) ALLOCATE(XCGRID(npa))
        IF(.NOT.ALLOCATED(YCGRID)) ALLOCATE(YCGRID(npa))

!
!       *** the computational grid is included in output data  ***      !40.31
!JL      IF ((MXC.GT.0) .AND. (MYC.GT.0) ) THEN                          !40.31
        IF(MCGRD > 0)THEN
           ALLOCATE(OPSTMP)                                             !40.31
           OPSTMP%PSNAME = 'COMPGRID'                                   !40.31
!JL           IF (OPTG .EQ. 1) THEN                                         ! 40.31
!JL               OPSTMP%PSTYPE = 'F'                                        ! 40.31
!JL               OPSTMP%OPR(1) = XPC                                        ! 40.31
!JL               OPSTMP%OPR(2) = YPC                                        ! 40.31
!JL               OPSTMP%OPR(3) = XCLEN                                      ! 40.31
!JL               OPSTMP%OPR(4) = YCLEN                                      ! 40.31
!JL               OPSTMP%OPR(5) = ALPC                                       ! 40.31
!JL            ELSE IF (OPTG.EQ.3) THEN                                      ! 40.80 40.31
!JL               OPSTMP%PSTYPE = 'H'                                        ! 40.31
!JL               OPSTMP%OPR(1) = FLOAT(MXC-1)                               ! 40.31
!JL               OPSTMP%OPR(2) = FLOAT(MYC-1)                               ! 40.31
!JL               OPSTMP%OPR(3) = 0.                                         ! 40.31
!JL               OPSTMP%OPR(4) = 0.                                         ! 40.31
!JL               OPSTMP%OPR(5) = ALPC                                       ! 40.31
!JL            ENDIF                                                         ! 40.31
!JL            OPSTMP%OPI(1) = MXC                                           ! 40.31
!JL            OPSTMP%OPI(2) = MYC                                           ! 40.31
!JL            ALLOCATE(OPSTMP%XP(0))                                       !40.31
!JL            ALLOCATE(OPSTMP%YP(0))                                       !40.31
!JL            NULLIFY(OPSTMP%NEXTOPS)                                      !40.31
!JL            IF ( .NOT.LOPS ) THEN                                        !40.31
!JL              FOPS = OPSTMP                                             !40.31
!JL              COPS => FOPS                                              !40.31
!JL              LOPS = .TRUE.                                             !40.31
!JL            ELSE                                                         !40.31
!JL              COPS%NEXTOPS => OPSTMP                                    !40.31
!JL              COPS => OPSTMP                                            !40.31
!JL            END IF                                                       !40.31
!JL            ENDIF

           IF (OPTG .EQ. 5) THEN
                OPSTMP%PSTYPE = 'F'
                OPSTMP%OPR(1) = 0.0!VXMIN       !XPC
                OPSTMP%OPR(2) = 0.0!VYMIN       !YPC
...
! ---------------- CGRID  (Continue)----------------------------------------
!
! SWAN in SCHISM :    unstructured grid,  Update 12/2018 JEROME
!
! Convert mesh Topology from SCHISM to SWAN
!
! ---------------------------------------------------------------------------
!
! Set the mesh grid
! -----------------
! Set, allocate and Populate several SWAN Mesh Topology Objects
! useful later in SwanCompUnstruc.F

        IF(myrank.EQ.0) WRITE(PRINTF,*)  &
        'unstructured grid, Allocation Begin vert, cell and face'
...
! -----------
        ...
        IF(INP_BOT)THEN
          ...
          IF(.NOT.ALLOCATED(DEPTH)) ALLOCATE(DEPTH(npa))
          ! Depth is transfered from the FVCOM bathymetric field H
          DO I = 1,nverts
            DEPTH(I) = dp(I)
          END DO
          ! bottom topography s now set , so                 40.80
          LOGCOM(3) = .TRUE.                               ! 40.80
          LEDS(1)   = 2                                    ! 40.80
        ENDIF

! coordinates of computational points in the Swan Unstructured grid
        IF(.NOT.ALLOCATED(xcugrd)) ALLOCATE(xcugrd(npa))
        IF(.NOT.ALLOCATED(ycugrd)) ALLOCATE(ycugrd(npa))
        xcugrd = 0.0_rkind
        ycugrd = 0.0_rkind
        ...
        !if(ics==1) then
         IF(KSPHER ==0)THEN
          DO I = 1,nverts
           xcugrd(I) = xnd(I);  ycugrd(I) = ynd(I);
          END DO
         ENDIF

         IF(KSPHER ==1)THEN
          DO I = 1,nverts
           ! in schsim xlon are in radians, convert back to degree..
           xcugrd(I) = xlon(I)*180.0_rkind/PI_W
           ycugrd(I) = ylat(I)*180.0_rkind/PI_W
          END DO
         ENDIF

        !endif           

! Mesh topology in the Swan Unstructured grid
       IF(.NOT.ALLOCATED(kvertc)) ALLOCATE(kvertc(3,ncells))
         kvertc = 0
         DO I = 1, ncells
          kvertc(1,I) = elnode(1,I)
          kvertc(2,I) = elnode(2,I)
          kvertc(3,I) = elnode(3,I)
         ENDDO

! VMARK will serve later to populate %att(VMARKER)
! VMARK = 0 interior
! VMARK = 1 node where OBC is applied
! VMARK = +999 ghost vertices
! Remind : 
!     isonb_w(i)=0:  node in the interior computational domain               
!     isonb_w(i)=1:  node on the solid boundary
!     isonb_w(i)=2:  node on the open boundary
       ...
       IF(.NOT.ALLOCATED(vmark)) ALLOCATE(vmark(nverts))
       vmark = 0
       nb_INT_vert = 0
       nb_OBC_vert = 0
       nb_LND_vert = 0
       DO I = 1,nverts
        IF(ISONB_W(I)==0) nb_INT_vert = nb_INT_vert +1
        IF(MAXVAL(ISONB_W).EQ.1)THEN
          IF(ISONB_W(I)==1) nb_OBC_vert = nb_OBC_vert +1
          IF(ISONB_W(I)==1) vmark(I) = 1
        ELSEIF(MAXVAL(ISONB_W).GE.2)THEN
          IF(ISONB_W(I)==1) nb_LND_vert = nb_LND_vert +1
          IF(ISONB_W(I)==1) vmark(I) = 1
          IF(ISONB_W(I)==2) nb_OBC_vert = nb_OBC_vert +1
          IF(ISONB_W(I)==2) vmark(I) = 2
        ENDIF
      ! Mark ghost vertices, interior only
        IF(ISONB_W(I)==0) THEN
          IF(I>np) THEN
            vmark(I)=999
          ENDIF
...
!DBG
      ...
      IF(.NOT.SERIAL) THEN
         ...
         write(38,*)'I,INDEX_GL,vmark'
         DO I = 1,nverts
          write(38,*)I,iplg(I),vmark(I)
         ENDDO
         CLOSE(38) 
      ENDIF
#endif
!END DBG
      ...
      ! for sake of convenience, set MCGRD to nverts (for allocating AC1 and
      ! COMPDA)
      !
       MCGRD   = nverts  ! = npa

      !   End SwanInitCompGrid details :

!       WRITE(PRINTF,*)'Before SwanCreateEdges'

       CALL SWANCREATEEDGES                                          !40.80

      !  SwanCreateEdges details:
      !
      !   Generates edge-based data structure
      !
      !   Faces of triangles are computed from elements and stored in Swan data
      !   structure
      !
      !   return kvertf(2,nfaces)
      !
      !  End SwanCreateEdges details :
      !
      ! ----------------------------------------------------------------------

!       WRITE(PRINTF,*)'Before SwanGridTopology'

      !   CALL SwanGridTopology                                         40.80
      !
      !   SwanGridTopology details :
      !
      ...
      !
      !   JLEFEVRE : SwanGridTopology NOT CALLED, PERFORM ALLOCATIONS BELOW NOW
      !
      !   allocate arrays vert, cell and face
      !
       ALLOCATE(gridobject%vert_grid(nverts), stat = istat)
       IF ( istat == 0 ) ALLOCATE(gridobject%cell_grid(ncells), &
        stat = istat)
       IF ( istat == 0 ) ALLOCATE(gridobject%face_grid(nfaces), &
        stat = istat)
       IF ( istat /= 0 ) THEN
        WRITE(PRINTF,*)'Allocation problem in swanpre,          & 
     &   gridobject strucure array vert, cell or face'
        RETURN
       ENDIF
      !
      ! setup the vertices
      ...
      !
      ! End SwanGridTopology details
      !    WRITE(PRINTF,*)'After SwanGridTopology'
      !
      ! END unstructured grid,  Update 12/2018 JEROME
      ! ---------------------------------------------------------------------


       IF(OPTG.EQ.5) THEN                                              !JL
          WRITE (PRINTF,6052) nb_INT_vert
          WRITE (PRINTF,6053) nb_OBC_vert
          WRITE (PRINTF,6054) nb_LND_vert
 6052     FORMAT (' INTERIOR VERTICES: ',I6)
 6053     FORMAT (' OBC VERTICES     : ',I6)
 6054     FORMAT (' LAND VERTICES    : ',I6)
       ENDIF


       ! JL : MSC, MDC are defined. Time to allocate others array for OBC
!       print*,'before SWBOUN NESTING=',NESTING
       IF(BOUNDARY /= 'default') THEN
        CALL SWBOUN ( )
!        print*,'after SWBOUN NESTING=',NESTING
       ENDIF
...
!     ============================================================
!
!      WRITE(PRINTF,*) 'swread dbg 400'

      IF (KEYWIS ('TEST')) THEN
        !WRITE(PRINTF,*) 'swread dbg 410'
        CALL ININTG ('ITEST' , ITEST , 'STA', 30)
!       statements restructured:
        IF (ITEST.GE.30) THEN
          IF (ERRPTS.EQ.0.AND.myrank.EQ.0) THEN                           !40.95 40.30 40.13
            ERRPTS = 16
            OPEN (ERRPTS, FILE='ERRPTS',&
     &            STATUS='UNKNOWN', FORM='FORMATTED')
          ENDIF                                                         !40.13
        ENDIF
        !WRITE(PRINTF,*) 'swread dbg 420'
        ...
        ELSE
          LTRACE =.FALSE.
        ENDIF
        !WRITE(PRINTF,*) 'swread dbg 430'
        CALL INKEYW ('STA', ' ')
        IF (KEYWIS('POI')) THEN
          NPTST  = 0
          MPTST  = 50
          IPP    = 2                                                    !40.80
          !WRITE(PRINTF,*) 'swread dbg 450'
...
!      WRITE(PRINTF,*) 'swread dbg 1200'
      ...
      IF (KEYWIS ('COORD')) THEN                                        !33.09
        ...
        ELSE IF (KEYWIS ('SPHE')) THEN                                  !33.09
          KSPHER = 1                                                    !33.09
          CALL INREAL ('REARTH', REARTH2, 'UNC', 0.)                    !33.09

#if defined USE_DUCK94 || defined USE_PLANARBEACH
          REARTH2 = rearth_eq 
#endif
          LENDEG = REARTH2 * PI_W / 180._rkind                          !33.09
...
! JL : this stuff below will be reinitialized later 
! See  section --- Replace previous definitions (DTW, RDTIM etc from SWREAD() 
! in swmain sbr SWMAIN_SETUP
          CALL IGNORE ('NONST')
          TINIC = IFLBEG(IGRD)
          IF(TINIC < TIMCO)THEN
          ...
          CALL ININTV ('DELTC', DTW, 'REQ', 0.)                         ! 40.03
          NSTATC = 1                                                    ! 40.00
...
!JL     IF (NSTATM.GT.0) CHTIME = DTTIWR(ITMOPT, TIMCO)                 !40.00
        NCOMPT = NCOMPT + 1                                             !40.41
!JL     IF (NCOMPT.GT.300) CALL MSGERR (2, &                            !40.41
!JL  &                  'No more than 300 COMPUTE commands are allowed')!40.41
...
!
        ...
        IF (KEYWIS ('TRANS1') .OR. KEYWIS ('TRFREQ')) THEN              !41.65
          ...
          CURRFT => FRSTFT
 403      CALL INREAL ('TRCOEF',TRCF,'REP',-1._rkind)
          IF ( TRCF.NE.-1. ) THEN
        ...
        ELSE IF (KEYWIS ('TRANS2')) THEN                                !41.65
          ...
          NULLIFY(FRSTFT%NEXTFT)
          CURRFT => FRSTFT
 404      CALL INREAL ('TRCOEF',TRCF,'REP',-1._rkind)
          IF ( TRCF.NE.-1. ) THEN
             IF ( TRCF.LT.0. .OR. TRCF.GT.1. ) THEN
                CALL MSGERR(3,'Transmission coefficient is not allowed')
                CALL MSGERR(3,'to be greater than 1 or smaller than 0!')
        ...
        ELSE IF (KEYWIS ('TRANS')) THEN
          ITRAS = 0
          CALL INREAL ('TRCOEF', TRCF, 'REQ', 0._rkind)
        ...
        ELSE IF (KEYWIS ('DAM')) THEN
          ...
          IF (KEYWIS ('DANG')) THEN                                     !40.66
             ITRAS = 2                                                  !40.66
             CALL INREAL ('HGT'  , HGT , 'REQ', 0._rkind)                     !40.66
             CALL INREAL ('SLOPE', SLP , 'REQ', 0._rkind)                     !40.66
             CALL INREAL ('BK'   , BK  , 'REQ', 0._rkind)                     !40.66
          ...
          ELSE                                                          !40.66
             ...
             ITRAS = 1
             CALL INREAL ('HGT'  , HGT , 'REQ', 0._rkind  )                   !30.80
             CALL INREAL ('ALPHA', OGAM, 'STA', 2.6_rkind )                   !30.60
             CALL INREAL ('BETA' , OBET, 'STA', 0.15_rkind)                   !30.60
...
!
        ...
        IF (KEYWIS ('REFL')) THEN                                       !40.09
          ...
          ENDIF                                                         !40.09
          CALL INREAL ('REFLC', REF0, 'STA', 1._rkind)                        !40.09
...
!                                                                       !40.18
          ...
          IF (KEYWIS('RDIFF')) THEN                                     !40.18
            ...
            CALL INREAL ('POWN', POWN, 'REQ', 1._rkind)                       !40.18
...
!
          ...
          IF (KEYWIS('RFD')) THEN                                       !40.28
            LRFRD = 1                                                   !40.31 40.28
            CALL INREAL ('FD1', FD1, 'REQ', 0.565_rkind)                      !40.28
            CALL INREAL ('FD2', FD2, 'REQ', 0.75_rkind)                       !40.28
            CALL INREAL ('FD3', FD3, 'REQ', -21._rkind)                       !40.28
            CALL INREAL ('FD4', FD4, 'REQ', 0.066_rkind)                      !40.28
...
!      WRITE(PRINTF,*) 'swread dbg 1500'

!JL switch off (I see later)
#if 1
      IF (KEYWIS ('INIT')) THEN                                           !30.61
!JQI     CALL INITVA( AC2, SPCSIG, SPCDIR, KGRPNT )                        !40.80 40.31
!JQI     IF (STPNOW()) RETURN                                              !34.01
...
!     and INIT can be useful also in stationary mode                      40.13
        ...
        IF (KEYWIS('PAR')) THEN
          print*,'To Do parametric'
          !return
        ELSEIF (KEYWIS('ZERO')) THEN
          print*,'To Do ZERO'
          !rreturn
        ELSE
...
! ============================================================================
!
! JL MUTE, implemented in schism_step.F
#if 0
...
!JL : THE FOLLOWING SECTION With KEYWORDS SCGRID IS NOT
!JL ! RELEVANT WITH THE SWAN OCEAN PACK NML READING METHOD
!JL : we manage to skip that following section, by using  RDINIT
!JL : (see opcrce;F)
! the following Input are used (no action)

!      WRITE(PRINTF,*) 'swread dbg 1700'

      IF (KEYWIS ('SCGRID')) THEN
!       CALL MSGERR (1, 'Keyword SCGRID indicates aliens inputs, bypass')
       CALL RDINIT
!   CALL NWLINE
       GO TO 100
      ENDIF

!JL : We use the SCHISM namelist method to get key/values
!JL : this is the first visit, Turn KEY_INPGRID OFF
!      WRITE(PRINTF,*) 'swread dbg 1800'
...
!
! Alien CURRENT ... may add or replace schism current output
!
...
!       ICUR = 0
!# else
        ICUR = 1  ! indicates presence of currents, default
!# endif
!# endif
       CALL SCAN_INLOGC('SCGRID INP_CUR',INP_CUR,'REQ','T')

!      PRINT *,'INP_CUR',INP_CUR

       IF(INP_CUR)THEN

         IF(myrank.EQ.0) &
         WRITE(PRINTF,*)'INP_CUR = TRUE, are-you sure to ', &
     &                  'fill UXB,UYB with alien data?'
         IGRD = 1
         IGR1 = 2
         IGR2 = 3
         ICUR = 1
         ...
         CALL SCAN_INCSTR('SCGRID INP_CUR_NAME',INP_CUR_NAME,'REQ',' ')
         
         PRINT *,'INP_CUR_NAME',INP_CUR_NAME

         INQUIRE(FILE=TRIM(INP_CUR_NAME),EXIST=CHECK)
         IF(CHECK)THEN
          OPEN(1,FILE=TRIM(INP_CUR_NAME), & 
     &         STATUS='OLD',FORM='UNFORMATTED')
         ELSE
          WRITE(PRINTF,*) TRIM(INP_CUR_NAME), ' DOES NOT EXIST.'
          RETURN
         ENDIF

         CALL SCAN_INLOGC('SCGRID INP_CUR_STAT',INP_CUR_STAT,'REQ','T')
         
         PRINT *,'INP_CUR_STAT',INP_CUR_STAT

         IF(.NOT. INP_CUR_STAT)THEN
          IF(NSTATM == 0) CALL MSGERR(3,                               &
     &   'In stationary mode,the current field also have to be         & 
     &     stationary')
          NSTATM = 1
          CALL SCAN_ININTG('SCGRID CUR_TBEGINP',IFLBEG(IGRD),'REQ',0)
          CALL SCAN_ININTG('SCGRID CUR_DELTINP',IFLINT(IGRD),'REQ',0)
          CALL SCAN_ININTG('SCGRID CUR_TENDINP',IFLEND(IGRD),'REQ',0)
          PRINT *,'CUR_TBEGINP',IFLBEG(IGRD)
          PRINT *,'CUR_DELTINP',IFLINT(IGRD)
          PRINT *,'CUR_TENDINP',IFLEND(IGRD)
          ...
          CALL SCAN_INLOGC('SCGRID INP_CUR_SERI',INP_CUR_SERI,'REQ','T')
          PRINT *,'INP_CUR_SERI',INP_CUR_SERI

          IF(INP_CUR_SERI)THEN
           INP_CUR_NTIME = (IFLEND(IGRD)-IFLBEG(IGRD)+1)/IFLINT(IGRD)
           ALLOCATE(UXB_GL(np_global,INP_CUR_NTIME)); UXB_GL = 0.0
           ALLOCATE(UYB_GL(np_global,INP_CUR_NTIME)); UYB_GL = 0.0

           ALLOCATE(UXB(npa,INP_CUR_NTIME));  UXB = 0.0
           ALLOCATE(UYB(npa,INP_CUR_NTIME));  UYB = 0.0

           DO J=1,INP_CUR_NTIME
            READ(1) (UXB_GL(I,J),UYB_GL(I,J),I=1,np_global)
           END DO
          ELSE
           INP_CUR_NTIME = 1
           ALLOCATE(UXB_GL(np_global,1)); UXB_GL = 0.0
           ALLOCATE(UYB_GL(np_global,1)); UYB_GL = 0.0

           ALLOCATE(UXB(npa,1)); UXB = 0.0
           ALLOCATE(UYB(npa,1)); UYB = 0.0

           READ(1) (UXB_GL(I,1),UYB_GL(I,1),I=1,np_global)
          END IF
...
!#  if defined (MULTIPROCESSOR)
        IF(.NOT.SERIAL)THEN
         DO J=1,INP_CUR_NTIME
          DO I=1,npa
           UXB(I,J) = UXB_GL(iplg(I),J)
           UYB(I,J) = UYB_GL(iplg(I),J)
          END DO
         END DO
        ENDIF
!#  endif
      ...
      IF(.NOT.ALLOCATED(UXB)) ALLOCATE(UXB(npa,1)); UXB = 0._rkind
      IF(.NOT.ALLOCATED(UYB)) ALLOCATE(UYB(npa,1)); UYB = 0._rkind

!
! WIND : we tell that wind are provided by schism
!
      VARWI  = .FALSE.
      INP_WI = .FALSE.
      INP_WI_SERI = .FALSE.
...
! Jerome quick Fix
      ...
      END IF
!
! Alien FRICTION .. may add or replace schism friction outputs...
!
      CALL SCAN_INLOGC('SCGRID INP_FR',INP_FR,'REQ','T')

      IF(INP_FR)THEN

        WRITE(PRINTF,*)'INP_FR = TRUE, are-you sure to ', &
     &                  'fill FRIC with alien data?'
        ...
        CALL SCAN_INCSTR('SCGRID INP_FR_NAME',INP_FR_NAME,'REQ',' ')
        PRINT *,'INP_FR_NAME',INP_FR_NAME
        INQUIRE(FILE=TRIM(INP_FR_NAME),EXIST=CHECK)
        IF(CHECK)THEN
          OPEN(1,FILE=TRIM(INP_FR_NAME),STATUS='OLD',FORM='UNFORMATTED')
        ELSE
          WRITE(PRINTF,*) TRIM(INP_FR_NAME), ' DOES NOT EXIST.'
          RETURN
        END IF

        CALL SCAN_INLOGC('SCGRID INP_FR_STAT',INP_FR_STAT,'REQ','T')
        PRINT *,'INP_FR_STAT',INP_FR_STAT
        IF(.NOT. INP_FR_STAT)THEN
          IF(NSTATM == 0) CALL MSGERR(3,                              &
     &     'In stationary mode, the friction field also have to be    & 
     &      stationary')
          NSTATM = 1
          CALL SCAN_ININTG('SCGRID FR_TBEGINP',IFLBEG(IGRD),'REQ',0)
          CALL SCAN_ININTG('SCGRID FR_DELTINP',IFLINT(IGRD),'REQ',0)
          CALL SCAN_ININTG('SCGRID FR_TENDINP',IFLEND(IGRD),'REQ',0)
          IFLDYN(IGRD) = 1
          IFLTIM(IGRD) = IFLBEG(IGRD)
          IF(IGR2 > 0)THEN
            IFLBEG(IGR2) = IFLBEG(IGRD)
          ...
          CALL SCAN_INLOGC('SCGRID INP_FR_SERI',INP_FR_SERI,'REQ','T')
          IF(INP_FR_SERI)THEN
            INP_FR_NTIME = (IFLEND(IGRD)-IFLBEG(IGRD)+1)/IFLINT(IGRD)
            ALLOCATE(FRIC_GL(np_global,INP_FR_NTIME)); FRIC_GL = 0._rkind
            ALLOCATE(FRIC(npa,INP_FR_NTIME));          FRIC    = 0._rkind
            DO J=1,INP_FR_NTIME
              READ(1) (FRIC_GL(I,J),I=1,np_global)
            END DO
          ELSE
            INP_FR_NTIME = 1
            ALLOCATE(FRIC_GL(np_global,1));     FRIC_GL = 0._rkind
            ALLOCATE(FRIC(npa,1));              FRIC    = 0._rkind
            READ(1) (FRIC_GL(I,1),I=1,np_global)
          ENDIF
        ELSE
          INP_FR_NTIME = 1
          ALLOCATE(FRIC_GL(np_global,1));       FRIC_GL = 0._rkind
...
!#    endif
      ...
      IF(.NOT.ALLOCATED(FRIC)) ALLOCATE(FRIC(npa,1)); FRIC = 0._rkind

!
! Alien WLEV ... may add or replace schism outputs
!
      CALL SCAN_INLOGC('SCGRID INP_WLEV',INP_WLEV,'REQ','T')
      IF(INP_WLEV)THEN

        IF(myrank.EQ.0) &
         WRITE(PRINTF,*)'INP_WLEV = TRUE, are-you sure to ', &
     &                  'fill VARWLV with alien data?'
        ...
        CALL SCAN_INCSTR('SCGRID INP_WLEV_NAME',INP_WLEV_NAME,'REQ',' ')
        INQUIRE(FILE=TRIM(INP_WLEV_NAME),EXIST=CHECK)
        IF(CHECK)THEN
         OPEN(1,FILE=TRIM(INP_WLEV_NAME),STATUS='OLD', & 
     &          FORM='UNFORMATTED')
        ELSE
         WRITE(PRINTF,*) TRIM(INP_WLEV_NAME), ' DOES NOT EXIST.'
         RETURN
        ENDIF

        CALL SCAN_INLOGC('SCGRID INP_WLEV_STAT',INP_WLEV_STAT,'REQ','T')
        IF(.NOT. INP_WLEV_STAT)THEN
          IF(NSTATM == 0) CALL MSGERR(3,                               &
              'In stationary mode, the water level also have to be     &
     &         stationary')
          NSTATM = 1
          CALL SCAN_ININTG('SCGRID WLEV_TBEGINP',IFLBEG(IGRD),'REQ',0)
          CALL SCAN_ININTG('SCGRID WLEV_DELTINP',IFLINT(IGRD),'REQ',0)
          CALL SCAN_ININTG('SCGRID WLEV_TENDINP',IFLEND(IGRD),'REQ',0)
          IFLDYN(IGRD) = 1
          IFLTIM(IGRD) = IFLBEG(IGRD)
          IF(IGR2 > 0)THEN
            IFLBEG(IGR2) = IFLBEG(IGRD)
         ...
         CALL SCAN_INLOGC('SCGRID INP_WLEV_SERI',INP_WLEV_SERI,'REQ','T')
          IF(INP_WLEV_SERI)THEN
            INP_WLEV_NTIME = (IFLEND(IGRD)-IFLBEG(IGRD)+1)/IFLINT(IGRD)
            ALLOCATE(WLEVL_GL(np_global,INP_WLEV_NTIME)); WLEVL_GL = 0._rkind
            ALLOCATE(WLEVL(npa,INP_WLEV_NTIME));     WLEVL    = 0._rkind
            DO J=1,INP_WLEV_NTIME
              READ(1) (WLEVL_GL(I,J),I=1,np_global)
            END DO
          ELSE
            INP_WLEV_NTIME = 1
            ALLOCATE(WLEVL_GL(np_global,1));    WLEVL_GL = 0._rkind
            ALLOCATE(WLEVL(npa,1));             WLEVL    = 0._rkind
            READ(1) (WLEVL_GL(I,1),I=1,np_global)
          ENDIF
        ELSE
          INP_WLEV_NTIME = 1
          ALLOCATE(WLEVL_GL(np_global,1));    WLEVL_GL = 0._rkind
        ...
        IF(SERIAL) WLEVL = WLEVL_GL

!#if defined (MULTIPROCESSOR)
        IF(.NOT.SERIAL)THEN
         DO J=1,INP_WLEV_NTIME
          DO I=1,npa
           WLEVL(I,J) = WLEVL_GL(iplg(I),J)
          END DO
         END DO
        ENDIF
!#endif
      ...
      IF(.NOT.ALLOCATED(WLEVL)) ALLOCATE(WLEVL(npa,1)); WLEVL = 0.0
...
!JL Use Fake Value Now
        WIND_X = 0._rkind;  WIND_Y = 0._rkind

        U10  = SQRT(WIND_X*WIND_X+WIND_Y*WIND_Y)
        WDIP = ATAN2(WIND_Y,WIND_X)
        WDIP = WDIP*180._rkind/PI_W
!
...
! ============================================================
!
!      WRITE(PRINTF,*) 'swread dbg 2000'

      IF (KEYWIS ('LIM')) THEN                                          !40.16
        CALL INREAL('URSELL',PTRIAD(3),'UNC',0._rkind)                        !40.16
        CALL INREAL('QB'    ,PNUMS(28),'UNC',0._rkind)                        !40.16
        GOTO 100                                                        !40.16
      ENDIF                                                             !40.16
!
!     ------------------------------------------------------------------
...
!       *** accuracy and criterion to terminate the iteration ***
        IF (KEYWIS ('STOPC')) THEN                                      !40.41
          PNUMS(21) = 1.                                                !40.41
          CALL INREAL ('DABS'   , PNUMS(2) , 'STA', 0.00_rkind)         !40.41
          CALL INREAL ('DREL'   , PNUMS(1) , 'STA', 0.01_rkind)         !40.41
          CALL INREAL ('CURVAT' , PNUMS(15), 'STA', 0.005_rkind)        !40.41
          CALL INREAL ('NPNTS'  , PNUMS(4) , 'UNC', 0._rkind)           !40.41
          CALL INREAL ('DTABS'  , PNUMS(3) , 'STA', 1000._rkind)        !40.93
          CALL INREAL ('CURVT'  , PNUMS(16), 'STA', 1000._rkind)        !40.93
          CALL INKEYW ('STA', 'STAT')                                   !40.41
          IF (KEYWIS ('STAT')) THEN                                     !40.41
            CALL ININTG ('MXITST' , MXITST   , 'STA', 50)               !40.41
            CALL INREAL ('ALFA'   , PNUMS(30), 'UNC', 0._rkind)         !40.41
          ...
          ENDIF                                                         !40.41
          CALL INREAL ('LIMITER', PNUMS(20), 'UNC', 0._rkind)           !40.41
        ELSE IF (KEYWIS ('ACCUR')) THEN
          PNUMS(21) = 0.                                                !40.41
          CALL INREAL ('DREL'   , PNUMS(1) , 'UNC', 0._rkind)
          CALL INREAL ('DHOVAL' , PNUMS(15), 'UNC', 0._rkind)                 !30.82
          CALL INREAL ('DTOVAL' , PNUMS(16), 'UNC', 0._rkind)                 !30.82
          CALL INREAL ('NPNTS'  , PNUMS(4) , 'UNC', 0._rkind)
          CALL INKEYW ('STA', 'STAT')                                   !40.03
          IF (KEYWIS ('STAT')) THEN                                     !40.03
            CALL ININTG ('MXITST' , MXITST   , 'UNC', 0 )               !40.80 40.03
            CALL INREAL ('ALFA'   , PNUMS(30), 'UNC', 0._rkind)               !40.23
          ...
          ENDIF
          CALL INREAL ('LIMITER', PNUMS(20), 'UNC', 0._rkind)                 !30.70
        END IF
!
...
!
        CALL INKEYW ('STA','  ')
        IF (KEYWIS ('DIR')) THEN
          CALL INREAL ('CDD'    , PNUMS(6) , 'UNC', 0._rkind)
          CALL INREAL ('CDLIM'  , PNUMS(17), 'UNC', 0._rkind)                 !30.80
...
!
        ...
        IF (KEYWIS ('REFRL')) THEN
          PNUMS(29) = 1.
          CALL INREAL ('FRLIM', PNUMS(26) ,'UNC', 0._rkind)
          CALL INREAL ('POWER', PNUMS(27), 'UNC', 0._rkind)
        ENDIF
!
...
!         ***  This is the default option   PNUMS(8) = 1.  ***
          PNUMS(8) = 1.
          CALL INREAL ('CSS'  , PNUMS (7), 'UNC', 0._rkind)
          CALL INREAL ('EPS1' , PNUMS(11), 'UNC', 0._rkind)
          CALL INREAL ('EPS2' , PNUMS(12), 'UNC', 0._rkind)
          CALL INREAL ('OUTP' , PNUMS(13), 'UNC', 0._rkind)
          CALL INREAL ('NITER', PNUMS(14), 'UNC', 0._rkind)
        ELSE IF (KEYWIS('SIGEX') .OR. KEYWIS('EXP')) THEN               !30.20
!
!         *** 2) explicit scheme ***
          PNUMS(8) = 2.
          CALL INREAL ('CSS'  , PNUMS (7), 'UNC', 0._rkind)                   !41.07
          CALL INREAL ('CFL'  , PNUMS(19), 'UNC', 0._rkind)
!
        ELSE IF (KEYWIS ('FIL')) THEN
!         *** 3) explicit scheme -> filter the spectrum  ***
          PNUMS(8) = 3.
          CALL INREAL ('DIFFC'  , PNUMS(9), 'UNC', 0._rkind)
!
        END IF
!
...
!
        ...
        IF (KEYWIS ('CT')) THEN
           PNUMS(35) = 1.
           CALL INREAL ('CFL', PNUMS(36) ,'STA', 0.9_rkind)
        ENDIF
!
...
!
        ...
        IF (KEYWIS ('CS')) THEN
           PNUMS(33) = 1.
           CALL INREAL ('CFL', PNUMS(34) ,'STA', 0.9_rkind)
        ENDIF
!
...
!         *** Settings for the setup  ***                               !30.82
          CALL INREAL ('EPS2' , PNUMS(23), 'UNC', 0._rkind)                   !30.82
          CALL INREAL ('OUTP' , PNUMS(24), 'UNC', 0._rkind)                   !30.82
          CALL INREAL ('NITER', PNUMS(25), 'UNC', 0._rkind)                   !30.82
        ENDIF                                                           !30.82
        GOTO 100
      ENDIF
!
...
!      WRITE(PRINTF,*) 'swread dbg 4000'
     ...
     & 'setup will not be compute correctly with spherical coordinates')!40.13
        ...
        ENDIF                                                           !40.80
        LSETUP = 1                                                      !32.02
        CALL INREAL ('SUPCOR', PSETUP(2), 'UNC', 0._rkind)                    !30.82
...
! =============================================================         !40.21
!                                                                       !40.21
!      WRITE(PRINTF,*) 'swread dbg 5000'

      IF (KEYWIS ('DIFFR')) THEN                                        !40.21
        CALL ININTG ('IDIFFR', IDIFFR, 'STA', 1)                        !40.21
        CALL INREAL ('SMPAR', PDIFFR(1), 'STA', 0.0_rkind)                    !40.21
        CALL INREAL ('SMNUM', PDIFFR(2), 'STA', 0._rkind)                     !40.21
        CALL INREAL ('CGMOD', PDIFFR(3), 'STA', 1._rkind)                     !40.21
        GOTO 100                                                        !40.21
      ENDIF                                                             !40.21
!
!     ------------------------------------------------------------------
...
!
 ...
 70      CALL SVARTP (IVTYPE)                                           !40.87
         IF (IVTYPE.GE.1 .AND. IVTYPE.LE.NMOVAR) THEN                   !40.87
            ...
            CALL INREAL ('LEXP' , QR(1) , 'STA',  999._rkind )                !40.87
            CALL INREAL ('HEXP' , QR(2) , 'STA', -999._rkind )                !40.87
            CALL INREAL ('EXCV' , QR(3) , 'STA',  999._rkind )                !40.87
            CALL INCTIM (ITMOPT, 'REF', DVAL, 'STA', -9.99D2)           !40.87
            QR(4) = REAL(DVAL)                                          !40.87
            CALL INREAL ('POWER', QR(5) , 'STA', -999._rkind)                 !40.87
            CALL INREAL ('FSWELL', QR(6), 'STA', -999._rkind)                 !40.87
            CALL INREAL ('FMIN' , QR(8) , 'STA',  999._rkind )                !40.87
            CALL INREAL ('FMAX' , QR(9) , 'STA', -999._rkind )                !40.87
...
! ============================================================
      IF (KEYWIS('CICE')) THEN
        ...
        IF (KEYWIS('ADCICE')) THEN
          IICE=2
          CALL INREAL('WBICETH',WBICETH,'STA',70.0_rkind)
        ELSE
          CALL WRNKEY
        ENDIF
        GOTO 100
...
!
      IF (KEYWIS ('BRE')) THEN
        ...
        WRITE(PRINTF,*)'DBG1 ALPHA', PSURF(1),'GAMMA', PSURF(2)  
        WRITE(PRINTF,*)'DBG1 ISURF',ISURF
        IF (KEYWIS('CON')) THEN
          ISURF = 1
          CALL INREAL ('ALPHA', PSURF(1), 'STA', 1.0_rkind)
          CALL INREAL ('GAMMA', PSURF(2), 'STA', 0.73_rkind)
        ELSE IF (KEYWIS('VAR') .OR. KEYWIS('NEL')) THEN                 !41.03
          
          ISURF = 2
          CALL INREAL ('ALPHA',  PSURF(1), 'STA', 1.5_rkind)
          CALL INREAL ('GAMMIN', PSURF(4), 'STA', 0.55_rkind)
          CALL INREAL ('GAMMAX', PSURF(5), 'STA', 0.81_rkind)
          CALL INREAL ('GAMNEG', PSURF(6), 'STA', 0.73_rkind)
          CALL INREAL ('COEFF1', PSURF(7), 'STA', 0.88_rkind)
          CALL INREAL ('COEFF2', PSURF(8), 'STA', 0.012_rkind)
        ELSE IF (KEYWIS('RUE')) THEN                                    !41.03
          ISURF = 3
          CALL INREAL ('ALPHA', PSURF(1), 'STA', 1.0_rkind)
          CALL INREAL ('A'    , PSURF(4), 'STA', 0.76_rkind)
          CALL INREAL ('B'    , PSURF(5), 'STA', 0.29_rkind)
        ELSE IF (KEYWIS('TG')) THEN                                     !41.03
          ISURF = 4
          CALL INREAL ('ALPHA',  PSURF(1), 'STA', 1.0_rkind)
          CALL INREAL ('GAMMA',  PSURF(4), 'STA', 0.42_rkind)
          CALL INREAL ('POWN' ,  PSURF(5), 'STA', 4.0_rkind)
        ELSE IF (KEYWIS('BKD')) THEN                                    !41.38
          ISURF = 6                                                     !41.38
          CALL INREAL ('ALPHA' , PSURF(1) ,'STA', 1.00_rkind)                 !41.47
          CALL INREAL ('GAMMA0', PSURF(4) ,'STA', 0.54_rkind)                 !41.47
          CALL INREAL ('A1'    , PSURF(5) ,'STA', 7.59_rkind)                 !41.47
          CALL INREAL ('A2'    , PSURF(6) ,'STA',-8.06_rkind)                 !41.47
          CALL INREAL ('A3'    , PSURF(7) ,'STA', 8.09_rkind)                 !41.47
        ENDIF
!
        CALL INKEYW ('STA', '  ')                                       !41.47
        IF (KEYWIS ('DIR')) THEN                                        !41.47
           IDISRF = 1                                                   !41.47
           CALL INREAL ('SPREAD', PSURF(15), 'STA', 15._rkind)                !41.47
        END IF                                                          !41.47
!
        CALL INKEYW ('STA', '  ')                                       !41.06
        IF (KEYWIS ('FREQD')) THEN                                      !41.06
           IFRSRF = 1                                                   !41.06
           CALL INREAL ('POWER', PSURF(16), 'STA', 2.0_rkind)                 !41.06
           CALL INREAL ('FMIN' , PSURF(17), 'STA', 0.0_rkind)                 !41.06
           CALL INREAL ('FMAX' , PSURF(18), 'STA', 1000._rkind)               !41.06
        END IF                                                          !41.06

        
        WRITE(PRINTF,*)'DBG2 ALPHA', PSURF(1),'GAMMA', PSURF(2)
        WRITE(PRINTF,*)'DBG2 ISURF',ISURF
...
! =============================================================
!
!        | -> KOMen   [cds2] [stpm] [powst] [delta] [powk]              !30.00
!        |
...
!         error checking in case user has already selected Babanin physics
          ...
          IWCAP = 1
          WCAPSET = .TRUE.                                              !40.88
          CALL INREAL ('CDS2',  PWCAP(1),  'UNC', 0._rkind)                   !20.73
          CALL INREAL ('STPM',  PWCAP(2),  'UNC', 0._rkind)                   !20.73
          CALL INREAL ('POWST', PWCAP(9),  'UNC', 0._rkind)                   !30.00
          CALL INREAL ('DELTA', PWCAP(10), 'UNC', 0._rkind)                   !30.00
          CALL INREAL ('POWK',  PWCAP(11), 'UNC', 0._rkind)                   !30.00
        ELSE IF ( KEYWIS ('JANS')) THEN
!         *** whitecapping according to Janssen (1989, 1991) ***
          IWCAP = 2
          CALL INREAL ('CDS1',  PWCAP(3), 'UNC', 0._rkind)                    !20.73
          CALL INREAL ('DELTA', PWCAP(4), 'UNC', 0._rkind)
!
...
!         whitecapping routines                                         !40.02
          ...
          JCHARN = MCMVAR+5                                               !JL for Air/sea coupling
!          MCMVAR = MCMVAR+4                                             !30.22
          MCMVAR = MCMVAR+5
          ALOCMP = .TRUE.                                               !40.97
          CALL INREAL ('PWTAIL', PWTAIL(1), 'STA', 5._rkind)                  !30.70
...
!         *** whitecapping according to Longuett Higgins ***
          IWCAP = 3
          CALL INREAL ('CFLHIG', PWCAP(5), 'UNC', 0._rkind)
        ELSE IF ( KEYWIS ('BJ')) THEN
!         *** whitecapping according to Battjes/Janssen formulation ***
          IWCAP = 4
          CALL INREAL ('BJSTP' , PWCAP(6), 'UNC', 0._rkind)
          CALL INREAL ('BJALF' , PWCAP(7), 'UNC', 0._rkind)
...
!         *** and Battjes/Janssen                                    ***
          ...
          CALL INREAL ('BJSTP' , PWCAP(6), 'UNC', 0._rkind)
          CALL INREAL ('BJALF' , PWCAP(7), 'UNC', 0._rkind)
          CALL INREAL ('KCONV' , PWCAP(8), 'UNC', 0._rkind)
        ELSE IF ( KEYWIS ('AB')) THEN                                   !40.53
!         *** whitecapping according to Alves and Banner (2003) ***     !40.53
          IWCAP = 7                                                     !40.53
          CALL INREAL ('CDS2',  PWCAP(1),  'STA', 5.0E-5_rkind)               !40.53
          CALL INREAL ('BR',    PWCAP(12), 'STA', 1.75E-3_rkind)              !40.53
          CALL INREAL ('P0',    PWCAP(10), 'STA', 4._rkind)                   !40.53
          CALL INREAL ('POWST', PWCAP(9),  'STA', 0._rkind)                   !40.53
          CALL INREAL ('POWK',  PWCAP(11), 'STA', 0._rkind)                   !40.53
        ELSE
          CALL WRNKEY
        ENDIF
        GOTO 100
      ...
      IF (KEYWIS ('FRIC')) THEN
        ...
        IF (KEYWIS('JON')) THEN
          ...
          IF (KEYWIS('VAR')) THEN                                       !41.04
            IBOT = 4
            CALL INREAL ('CFJ1', PBOT(6), 'STA', 0.038_rkind)
            CALL INREAL ('CFJ2', PBOT(7), 'STA', 0.067_rkind)
            CALL INREAL ('DSP1', PBOT(8), 'STA', 10._rkind)
            CALL INREAL ('DSP2', PBOT(9), 'STA', 30._rkind)
          ELSE
            CALL IGNORE ('CON')
            IBOT = 1
            CALL INREAL('CFJON',PBOT(3),'UNC',0._rkind)
          ENDIF
        ELSE IF (KEYWIS('COLL')) THEN                                   !20.68
          IBOT = 2
          CALL INREAL('CFW',PBOT(2),'UNC',0._rkind)
          CALL INREAL('CFC',PBOT(1),'UNC',0._rkind)
        ELSE IF (KEYWIS('MAD')) THEN
          IBOT = 3
          CALL INREAL('KN',PBOT(5),'UNC',0._rkind)
          JFRC2  = MCMVAR+1                                      ! JLEFEVRE Add KN from SCHISM
          MCMVAR = MCMVAR+1
          JFRC3  = MCMVAR+1                                      ! JLEFEVRE Add KN from SCHISM
          MCMVAR = MCMVAR+1
        ELSE IF (KEYWIS('RIP')) THEN                                    !41.51
          IBOT = 5
          CALL INREAL('S',PBOT(6),'UNC',0._rkind)
          CALL INREAL('D',PBOT(7),'UNC',0._rkind)
          JFRC2  = MCMVAR+1
          MCMVAR = MCMVAR+1
          ALOCMP = .TRUE.
        ELSE
...
!
      IF (KEYWIS ('MUD')) THEN
        IMUD = 1
        CALL INREAL('LAYER',PMUD(1),'UNC',0._rkind)
        CALL INREAL('RHOM' ,PMUD(2),'UNC',0._rkind)
        CALL INREAL('VISCM',PMUD(3),'UNC',0._rkind)
        CALL INREAL('RHOW' ,PMUD(4),'UNC',0._rkind)
        CALL INREAL('VISCW',PMUD(5),'UNC',0._rkind)
        GOTO 100
      ENDIF
!
!   ------------------------------------------------------------------
...
!   ------------------------------------------------------------------
      IF (KEYWIS ('VEGE')) THEN
        ...
        NULLIFY(FRSTV%NEXTV)
        CURRV => FRSTV
 303    CALL INREAL ('HEIGHT',HH,'REP',-1._rkind)
        IF ( HH.NE.-1. ) THEN
           IF ( HH.LT.0. ) THEN
              CALL MSGERR (2,'height is negative')
              HH = 0.
           END IF
           CALL INREAL ('DIAMTR',DD,'REQ', 0._rkind)
           IF ( DD.LT.0. ) THEN
           ...
           IF ( NN.LE.0 ) THEN
              ...
              NN = 1
           END IF
           CALL INREAL ('DRAG'  ,CC,'REQ', 0._rkind)
           IF ( CC.LT.0. ) THEN
...
! =================================================================
      ...
      IF (KEYWIS ('TURB')) THEN                                         !40.35
        ITURBV = 1
        CALL INREAL ('CTB',  PTURBV(1), 'STA', 0.01_rkind)                    !40.35
        CALL INKEYW ('STA', ' ')
        IF (KEYWIS('CUR')) THEN
          CALL INREAL ('TBCUR', PTURBV(2), 'STA', 0.004_rkind)
          IF (VARTUR) THEN
...
!       *** first generation wind model ***
        CALL INREAL ('CF10', PWIND(1), 'UNC', 0._rkind)
        CALL INREAL ('CF20', PWIND(2), 'UNC', 0._rkind)
        CALL INREAL ('CF30', PWIND(3),'UNC',0._rkind)
        CALL INREAL ('CF40', PWIND(4),'UNC',0._rkind)
        CALL INREAL ('EDMLPM', PWIND(10), 'UNC', 0._rkind)
        CALL INREAL ('CDRAG',  PWIND(11), 'UNC', 0._rkind)
        CALL INREAL ('UMIN', PWIND(12), 'UNC', 0._rkind)
        CALL INREAL ('CFPM',  PWIND(13), 'UNC', 0._rkind)
...
!       *** set value for the windparameters ***
        ...
        CALL INREAL ('CF30', PWIND(3),'UNC',0._rkind)
        CALL INREAL ('CF40', PWIND(4),'UNC',0._rkind)
        CALL INREAL ('CF50', PWIND(5),'UNC',0._rkind)
        CALL INREAL ('CF60', PWIND(6), 'UNC', 0._rkind)
        CALL INREAL ('EDMLPM', PWIND(10), 'UNC', 0._rkind)
        CALL INREAL ('CDRAG',  PWIND(11), 'UNC', 0._rkind)
        CALL INREAL ('UMIN', PWIND(12), 'UNC', 0._rkind)
        CALL INREAL ('CFPM',  PWIND(13), 'UNC', 0._rkind)
...
!             Babanin, Young, Tsagareli, Ardhuin and others
          ...
          WCAPSET = .TRUE.
          IWCAP = 8
          CALL INREAL ('A1SDS', A1SDS, 'REQ', 0._rkind)
          CALL INREAL ('A2SDS', A2SDS, 'REQ', 0._rkind)
          CALL INREAL ('P1SDS', P1SDS, 'REQ', 0._rkind)
          CALL INREAL ('P2SDS', P2SDS, 'REQ', 0._rkind)
          ROGERS = .TRUE. ! legacy swell dissipation
          IF(SWELLSET) CALL MSGERR(4,'swell dissipation is already set')
          SWELLSET = .TRUE.
          CALL INREAL ('CDSV'   , CDSV   , 'REQ', 0._rkind)
          CALL INREAL ('FESWELL', FESWELL, 'REQ', 0._rkind)
          CALL INKEYW ('REQ', ' ')
          IF (KEYWIS ('UP')) THEN
             UPWARDS = .TRUE.
...
!             Babanin, Young, Tsagareli, Ardhuin and others
          IWCAP = 8
          CALL INREAL ('A1SDS', A1SDS, 'UNC', 0._rkind)
          CALL INREAL ('A2SDS', A2SDS, 'UNC', 0._rkind)
          CALL INREAL ('P1SDS', P1SDS, 'STA', 4._rkind)
          CALL INREAL ('P2SDS', P2SDS, 'STA', 4._rkind)
          CALL INKEYW ('STA', 'UP')
          IF (KEYWIS ('DOWN')) THEN
             UPWARDS = .FALSE.
          ELSE
...
!            **** applies only when TRUE10 is not used        ****
             CALL INREAL ( 'WINDSCALING', WNDSCL, 'UNC', 0._rkind )
          ENDIF
          CALL INKEYW ('STA', ' ')
          IF (KEYWIS ('DEB')) THEN
             CALL INREAL ( 'CDFAC', CDFAC, 'REQ', 1._rkind )
          ENDIF
        ELSE
...
!         *** whitecapping according to Komen et al. (1984) ***
        ...
        IF (KEYWIS('AGROW')) THEN                                       !7/MAR
          CALL INREAL ('A',PWIND(31),'STA',0.0015_rkind)
        ELSE                                                            !30.60
...
!  ------------------------------------------------------------------   !40.88
      IF (KEYWIS('SSWELL')) THEN
         ...
         IF (KEYWIS ('ROG')) THEN
            ROGERS = .TRUE.
            CALL INREAL ('CDSV'   , CDSV   , 'STA', 1.2_rkind)
            CALL INREAL ('FESWELL', FESWELL, 'REQ', 0._rkind )
         ...
         ELSE IF (KEYWIS ('ZIE')) THEN
            ZIEGER = .TRUE.
            CALL INREAL ('B1', B1Z, 'STA', 0.00025_rkind)
...
!   ------------------------------------------------------------------  !40.88
      IF (KEYWIS ('NEG')) THEN
         CALL INREAL('RDCOEF',RDCOEF,'REQ',0._rkind)
         GOTO 100
      ENDIF
!     ----------------------------------------------------------------  !40.17
...
!     ----------------------------------------------------------------  !40.17
      ...
      IF (KEYWIS('MDIA')) THEN                                          !40.17
        ...
        IF (KEYWIS('LAM')) THEN                                         !40.17
          ...
          ILAMBDA = 0                                                   !40.17
          DO WHILE (MORE)                                               !40.17
            CALL INREAL('LAMBDA',RLAMBDA(ILAMBDA+1),'STA',-1._rkind)          !40.17
        ...
        IF (KEYWIS('CNL4_12')) THEN                                     !40.17
          DO ICNL4=1,MDIA                                               !40.17
            CALL INREAL('CNL4_1',CNL4_1(ICNL4),'REQ',0._rkind)                !40.17
            CALL INREAL('CNL4_2',CNL4_2(ICNL4),'REQ',0._rkind)                !40.17
          END DO                                                        !40.17
        ELSEIF (KEYWIS('CNL4')) THEN                                    !40.17
          DO ICNL4=1,MDIA                                               !40.17
            CALL INREAL('CNL4',CNL4_1(ICNL4),'REQ',0._rkind)                  !40.17
          END DO                                                        !40.17
          CNL4_2 = CNL4_1                                               !40.17
        ELSE                                                            !40.17
          CALL WRNKEY                                                   !40.17
...
!           *** first generation wind model ***
            ...
            CALL INREAL ('CF50', PWIND(5),'UNC',0._rkind)
            CALL INREAL ('CF60', PWIND(6), 'UNC', 0._rkind)
            CALL INREAL ('CF70', PWIND(7), 'UNC', 0._rkind)
            CALL INREAL ('CF80', PWIND(8), 'UNC', 0._rkind)
            CALL INREAL ('RHOAW', PWIND(9), 'UNC', 0._rkind)
            CALL INREAL ('EDMLPM', PWIND(10), 'UNC', 0._rkind)
            CALL INREAL ('CDRAG',  PWIND(11), 'UNC', 0._rkind)
            CALL INREAL ('UMIN', PWIND(12), 'UNC', 0._rkind)
            CALL INREAL ('CFPM',  PWIND(13), 'UNC', 0._rkind)
...
! ===================================================================
!
!  QUADrupl [iquad] [lambda] [cnl4] [csh1] [csh2] [csh3]                !30.00
!
! ===================================================================
!
        CALL ININTG ('IQUAD', IQUAD, 'UNC',  0)
        CALL INREAL ('LAMBDA', PQUAD(1), 'UNC', 0.0_rkind)                    !30.00
        CALL INREAL ('CNL4', PQUAD(2), 'UNC', 0.0_rkind)                      !30.00
        CALL INREAL ('CSH1', PQUAD(3), 'UNC', 0.0_rkind)                      !30.00
        CALL INREAL ('CSH2', PQUAD(4), 'UNC', 0.0_rkind)                      !30.00
        CALL INREAL ('CSH3', PQUAD(5), 'UNC', 0.0_rkind)                      !30.00
        GOTO 100
      ENDIF
!
!     ------------------------------------------------------------------
...
!
      IF (KEYWIS ('TRI')) THEN
        ...
        IF (ITRIAD.EQ.1) THEN
           CALL INREAL ('TRFAC', PTRIAD(1), 'STA', 0.8_rkind)                 !41.44
           CALL INREAL ('CUTFR', PTRIAD(2), 'STA', 2.5_rkind)                 !40.61 40.56 30.82
        ELSEIF (ITRIAD.EQ.2) THEN
           CALL INREAL ('TRFAC', PTRIAD(1), 'STA', 0.90_rkind)                !41.46
           CALL INREAL ('A'    , PTRIAD(6), 'STA', 0.95_rkind)                !41.46
           CALL INREAL ('B'    , PTRIAD(7), 'STA', 0.0_rkind )                !41.46
           CALL INREAL ('P'    , PTRIAD(8), 'STA', 0.0_rkind )
        ELSEIF (ITRIAD.EQ.5) THEN
           CALL INREAL ('TRFAC', PTRIAD(1), 'STA', 0.35_rkind)                !41.45
           CALL INREAL ('CUTFR', PTRIAD(2), 'STA', 1.33333333_rkind)
        ELSEIF (ITRIAD.EQ.11) THEN
!          original LTA (before version 41.01)
           CALL INREAL ('TRFAC', PTRIAD(1), 'STA', 0.05_rkind)
           CALL INREAL ('CUTFR', PTRIAD(2), 'STA', 2.5_rkind)
        ENDIF
        CALL INREAL ('URCRIT', PTRIAD(4), 'UNC', 0.0_rkind)                   !40.13
        CALL INREAL ('URSLIM', PTRIAD(5), 'STA', 0.01_rkind)                  !40.41 40.23 40.13
        GOTO 100
      ENDIF
!
!     ------------------------------------------------------------------
...
!
! JL Skip that
!JL   CALL SPROUT(FOUND, DEPTH, WLEVL)                                   !40.31
...
!       default values changed                                          !30.60
        CALL INREAL ('STAGRX',STAGRX,'STA', 0._rkind)                         !30.60
        CALL INREAL ('STAGRY',STAGRY,'STA', 0._rkind)                         !30.60
...
!
     ...
     &                 'UNC', 0., 0.)                                   !30.20
          CALL INREAL ('ALPINP',ALPG(IGRID1),'UNC',0._rkind)                  !30.20
        ELSE                                                            !40.13
!         [xpinp] and [ypinp] are required in case of spherical coordinates
          CALL READXY ('XPINP', 'YPINP', XPG(IGRID1), YPG(IGRID1), &
     &                 'REQ', 0., 0.)                                   !40.13
          CALL INREAL ('ALPINP',ALPG(IGRID1),'STA',0._rkind)                  !40.13
        ENDIF                                                           !40.13
!       ---------  ALPG(IGRID1) is always between -PI and PI   ---------
        ALTMP = ALPG(IGRID1)/360.
        ALPG(IGRID1)  = 2 *pi * (ALTMP - NINT(ALTMP))
...
!
      ...
      IF (KEYWIS ('EXC')) THEN                                          !30.60
        CALL INREAL ('EXCVAL', EXCFLD(IGRID1), 'REQ', 0._rkind)               !30.60
...
! 13. Source text
!
      REAL(rkind), ALLOCATABLE :: TARR(:)                                      !40.31
...
!
      CALL INREAL ('FAC', IFLFAC(IGR1), 'STA', 1._rkind)
!
      IF (IGR2.GT.0) THEN
        VECTOR = .TRUE.
...
!     FRINTF is the frequency integration factor (=df/f)                !20.35
      FRINTF = MyLOG(SHIG/SLOW) / FLOAT(MSC-1)                           !20.35
...
!
      ...
      REAL(rkind)       AC2(MDC,MSC,MCGRD)                                     !40.31
      ...
      REAL(rkind)    ACTMP(MDC), DIRTMP(MDC), ACLOC(MDC,MSC)                   !41.42 40.41
...
!       initial state defined by wave parameters
     ...
     &        'command INIT should follow READ BOT or READ UNSTRUC')    !40.80
        ...
        IF (SPPARM(2).GT.(2*pi_w/SPCSIG(1))) THEN
          CALL MSGERR (2, &
     &          'Inc. freq. lower than lowest spectral freq.')
          WRITE(PRINTF,11) 1./SPPARM(2),' < ',SPCSIG(1)/(2*pi_w), &
     &                     ' lowest'
        ENDIF
        IF (SPPARM(2).LT.(2*pi/SPCSIG(MSC))) THEN
          CALL MSGERR (2, &
     &          'Inc. freq. higher than highest spectral freq.')
          WRITE(PRINTF,11) 1./SPPARM(2),' > ',SPCSIG(MSC)/(2*pi_w), &
     &                     'highest'
        ENDIF
 11     FORMAT(' Inc. freq. = ',F9.5, A3, F9.5, '=',A7,' freq')
        CALL INREAL('DIR',  SPPARM(3), 'REQ', 0.)
        IF (DSHAPE.EQ.1) THEN
          CALL INREAL('DD', SPPARM(4), 'STA', 30._rkind)
        ELSE
          CALL INREAL('DD', SPPARM(4), 'STA', 2._rkind)
        ENDIF
