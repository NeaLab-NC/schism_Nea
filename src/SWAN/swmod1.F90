...
!     SWSCAN             contains functions SCAN_SWINPUT,
!                        READ_FLOAT, READ_INT used in Ocean Pack 
!
   MODULE OCPCOMM1
!
!
...
!                 subroutines INREAL(rkind), ININTG, INCSTR, INCTIM
...
! ELREAL(rkind) [      ] last element read from user command, when real or double
!
...
!
   ...
   REAL(rkind)    AKPOWR,         ALCQ,        ALPQ,          COSCQ
   REAL(rkind)    COSPQ,          DXK,         DYK
   REAL(rkind)    OVEXCV(NMOVAR),              OVLEXP(NMOVAR)
   REAL(rkind)    OVLLIM(NMOVAR),              OVHEXP(NMOVAR)
   REAL(rkind)    OVULIM(NMOVAR),              SINCQ,         SINPQ
   REAL(rkind)    XPQ,            XQLEN,       XQP,           YPQ
   REAL(rkind)    YQLEN,          YQP,         OUTPAR(MOUTPA)
!
!  8. Subroutines and functions used
!
!     ---
...
! grid(11): vegetation: number of plants per square meter
! grid(12): field containing turbulent viscosity
! grid(13): field containing fluid mud layer
!
...
!
   ...
   INTEGER MXG(NUMGRD), MYG(NUMGRD), NWAMN, LWDATE
   INTEGER OPTG,        NBFILS,      NBSPEC,      NBGRPT
   REAL(rkind)    ALPG(NUMGRD),             COSPG(NUMGRD)
   REAL(rkind)    COSVC,       COSWC,       DXG(NUMGRD)
   REAL(rkind)    DYG(NUMGRD), EXCFLD(NUMGRD)
   REAL(rkind)    RDTIM,       SINPG(NUMGRD)
   REAL(rkind)    SINVC,       SINWC,       STAGX(NUMGRD)
   REAL(rkind)    STAGY(NUMGRD),            XOFFS
   REAL(rkind)    XPG(NUMGRD), YOFFS,       YPG(NUMGRD)
...
!
   ...
   REAL(dkind)  IFLBEG(NUMGRD), IFLINT(NUMGRD), IFLEND(NUMGRD),      &
                IFLTIM(NUMGRD)
   REAL(rkind)         IFLFAC(NUMGRD)
...
!              =MCMVAR+4, for command WCAP JANS ... (add JCDRAG, JTAUW, JUSTAR,
!              JZEL)
...
! MTSVAR  [ 11] within array TESTDA
      ...
      INTEGER             JCHARN  ! JL for CHARNOCK
...
!
      ...
      INTEGER             NGRBND
      REAL(rkind)                ALCP,        ALPC,        COSPC,       DDIR
      REAL(rkind)                DX,          DXRP,        DY,          DYRP
      REAL(rkind)                FRINTF,      FRINTH,      SHIG,        SINPC
      REAL(rkind)                SLOW,        SPDIR1,      SPDIR2,      XCLEN
      REAL(rkind)                XCP,         XPC,         YCLEN,       YCP
      REAL(rkind)                YPC
      REAL(rkind)                XCGMIN,      XCGMAX,      YCGMIN,      YCGMAX
      LOGICAL             FULCIR
...
!
!      REAL(rkind) CASTD, CDCAPi
!      REAL(rkind) DEGRAD,      DNORTH,      GRAV_W,        PI_W
!      REAL(rkind) PI2_W,         PWTAIL(10),  RHO_W,         WLEV
!
      REAL(rkind) CASTD,       CDCAP,       USCAP
      REAL(rkind) DEGRAD,      DNORTH,      GRAV_W,        PI_W
      REAL(rkind) PI2_W,   PWTAIL(10),  RHO_W,         WLEV
!
...
!CTGA 111003:  Full ice implementation.  Parameter read in swanpre1.ftn
      INTEGER             IICE
      REAL(rkind)                DEPMIN,      PBOT(MBOT),  PNUMS(MNUMS)
      REAL(rkind)                PSETUP(MSETUP),           PSHAPE(MSHAPE)
      REAL(rkind)                PSURF(MSURF),             PTRIAD(MTRIAD)
      REAL(rkind)                PWCAP(MWCAP),             PWIND(MWIND)
      REAL(rkind)                SIGMAG
      REAL(rkind)                SPPARM(MSPPAR),SY0,       U10
      REAL(rkind)                WDIC,          WDIP,      HSRERR
      REAL(rkind)                PQUAD(MQUAD)
      REAL(rkind)                RCOMPT(50,5)
      REAL(rkind)                PDIFFR(MDIFFR)                                  
      REAL(rkind)                PMUD(MMUD)                                     ! 40.59
      REAL(rkind)                PTURBV(MTURBV)                                 ! 40.35
!CTGA 111003:  Full ice implementation.  Parameter read in swanpre1.ftn
      REAL(rkind)                WBICETH
      LOGICAL             ACUPDA
...
! parameters for Babanin physics and swell, see Rogers et al (JTECH, 2012)
      REAL(rkind)                A1SDS, A2SDS, P1SDS, P2SDS, CDSV, FESWELL      ! 40.88
      REAL(rkind)                RDCOEF, B1Z, WNDSCL, CDFAC                     ! 40.88
      LOGICAL             UPWARDS, VECTOR_TAU, TRUE_U10                  ! 40.88
      LOGICAL             ROGERS, ZIEGER, ARDHUIN                        ! 40.88
      REAL(rkind)                FPI                                            ! 40.88
...
!
   ...
   INTEGER PROPFL
   REAL(rkind)    WAVAGE,    REARTH2,    LENDEG  !Jianzhong Ge
!   REAL(rkind)    WAVAGE,    LENDEG
   REAL(rkind)    COSLAT(10)
...
!
   ...
   MODULE TIMECOMM
   USE schism_glbl, ONLY : dkind,rkind
!
!
!   --|-----------------------------------------------------------|--
!     | Delft University of Technology                            |
...
!
   ...
   REAL(rkind)     BEGBOU, TIMERB, TINTBO
!TIMG!
...
!==============================================================================|
   MODULE SWSCAN
     ...
     USE schism_glbl, ONLY : skind,rkind,dkind,errmsg
     ...
     CONTAINS 
            ...
            !------------------------------------------------------------------!
            ! SCAN_SWINPUT  :  Scan a SWAN INPUT File for a Variable             !
            ! -----------------------------------------------------------------!

      FUNCTION SCAN_SWINPUT(FNAME,VNAME,ISCAL,FSCAL, &
     &                 IVEC,FVEC,CVEC,NSZE,CVAL,LVAL)
...
#      endif
       ...
       END IF
       IF(NREP > 4)THEN 
         MSGSTR = 'CANNOT HAVE > 4 LINE CONTINUATIONS'
         WRITE(errmsg,*) MSGSTR
...
!----IF VARNAME MATCHES, PROCESS VARIABLE AND ERROR-CHECK----------------------!
 ...
 20   CLOSE(INPUTF)
      ...
      RETURN
      END FUNCTION SCAN_SWINPUT

!==============================================================================|

!----------------- FUNCTION READ_FLOAT ----------------------

      REAL(dkind) FUNCTION READ_FLOAT(ITEM,IERR)
...
!----------------- FUNCTION READ_INT ----------------------
      ...
      REAL(skind) FUNCTION READ_INT(ITEM,IERR)
