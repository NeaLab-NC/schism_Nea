...
!
     ...
     &            MSCMAX,MDCMAX                                         ! 34.00
!
      REAL(rkind)        SNLC1 ,LAMM2 ,LAMP2 ,DELTH3,                     &    ! 40.17 34.00
...
!
      ...
      IF (ITEST .GE. 2 .AND. myrank.EQ.0) WRITE(PRINTF,*)'sbr. FAC4WW'
...
!
      IF (ITEST .GE. 2 .AND. myrank.EQ.0) &
         WRITE(PRINTF,*)'*** Range of calculations ***'
...
!
      IF (ITEST .GE. 2  .AND. myrank.EQ.0) & 
         WRITE(PRINTF,*)'*** quadratic interpolation **'
...
!
      DO 100 IS=1, MSC
        AF11(IS) = ( SPCSIG(IS) / ( 2. * PI_W ) )**11                  ! 30.72
 100  CONTINUE
!
!     *** compute the radian frequency for the IS = MSC+1, ISHGH ***
!
      FREQ   = SPCSIG(MSC) / ( 2. * PI_W )                             ! 30.72
...
!
 ...
 900 FORMAT ( ' ** Error : array bounds and maxima in subr FAC4WW, ',&
     ...
     &         /,'            SMI,SMA : ',2I4, '   DMI,DMA : ',2I4)
      ENDIF
!
      IF (ITEST .GE. 40 .AND. myrank.EQ.0) THEN
        RADE = 360.0 / ( 2. * PI_W )
        WRITE(PRINTF,*)
...
!
      REAL(rkind)      X      ,X2     ,CONS   ,FACTOR ,SNLCS1 ,SNLCS2 ,SNLCS3,&
...
!      REAL(rkind)      AC2(MDC,MSC,MCGRD)                    ,&
     ...
     &          WWAWG(*)                              ,&
     &          WWSWG(*)
      REAL(rkind) :: REDC0 (MDC,MSC,MREDS)                                     ! 40.85
      REAL(rkind) :: REDC1 (MDC,MSC,MREDS)                                     ! 40.85
!
      INTEGER   IDCMIN(MSC)        ,&
     &          IDCMAX(MSC)        ,&
     &          WWINT(*)
...
!
          IF(myrank.EQ.0 .AND. ITEST.GE.100 .AND. TESTFL) THEN
...
!
          IF(myrank.EQ.0 .AND. ITEST.GE.90 .AND. TESTFL) THEN
...
!     *** test output ***
!
      IF (myrank.EQ.0 .AND. ITEST .GE. 50 .AND. TESTFL) THEN
        WRITE(PRINTF,*)
 ...
 9020   FORMAT (' SNLC1  X  X2  CONS    :',4E12.4)
        WRITE(PRINTF,9021) DEP2(KCGRD(1)),KMESPC, FACHFR, PI_W
...
!     *** test output ***
!
      IF (myrank.EQ.0 .AND. ITEST .GE. 40 .AND. TESTFL) THEN
...
!
      REAL(rkind)    DAL1, DAL2, DAL3, FACHFR, KMESPC, SNLC1                   ! 40.17
!      REAL(rkind)    AC2(MDC,MSC,MCGRD)
      REAL(rkind)    DEP2(MCGRD)
...
!
     ...
     &          ISCLW   ,ISCHG
!
      REAL(rkind)      X       ,X2      ,CONS    ,FACTOR  ,SNLCS2  ,         &
...
!     WWAWG : weight coefficients for the quadruplet interactions
!
      REAL(rkind)    DAL1, DAL2, DAL3, FACHFR, KMESPC, SNLC1
!      REAL(rkind)    AC2(MDC,MSC,MCGRD)
      REAL(rkind)    DEP2(MCGRD)
      REAL(rkind)    MEMNL4(MDC,MSC,MCGRD)
...
!  6. Local variables
!
      REAL(rkind), ALLOCATABLE :: SA1(:,:), SA2(:,:), SFNL(:,:), UE(:,:)
!
!  7. Common blocks used
!
!
...
!       *** value source term in every bin ***
!
        IF(myrank.EQ.0 .AND. ITEST.GE. 150 ) THEN
          DO I=1, MSC
            DO J=1, MDC
...
!
      REAL(rkind)      X       ,X2      ,CONS    ,FACTOR  ,SNLCS1  ,SNLCS2  , &
     &          SNLCS3  ,E00     ,EP1     ,EM1     ,EP2     ,EM2     , &
     &          SA1A    ,SA1B    ,SA2A    ,SA2B    ,                   &
     &          JACOBI  ,SIGPI
!
...
!     *** value source term in every bin ***
!
      IF ( myrank.EQ.0 .AND. ITEST.GE.150 .AND. TESTFL ) THEN
         DO I=1, MSC
            DO J=1, MDC
               WRITE(PRINTF,2006) I,J,MEMNL4(J,I,KCGRD(1)),SFNL(I,J), &
     &                            SPCSIG(I)
...
!      SUBROUTINE FILNL3 (IDCMIN  ,IDCMAX  ,IMATRA  ,IMATDA  ,AC2     ,& ! 40.23
      SUBROUTINE FILNL3 (IDCMIN  ,IDCMAX  ,IMATRA  ,IMATDA           ,& ! 40.23
...
!                                                      interactions
      REAL(rkind)   , INTENT(IN) :: ASWAN(NDIR,NSIG,NGRID)  ! action density spectrum as a
!                                                    ! function of (sigma,dir)
      REAL(rkind)   , INTENT(IN) :: SIGMA(NSIG)             ! Intrinsic frequencies
      REAL(rkind)   , INTENT(IN) :: DIR(NDIR,6)             ! directions in radians (when second index =1)
      REAL(rkind)   , INTENT(IN) :: DEPTH(NGRID)            ! depth array
      ...
      REAL(rkind)   , INTENT(OUT):: SNL(NDIR,NSIG,NGRID)    ! nonlinear quadruplet interaction computed with
...
!
      REAL(rkind) AQUAD(NSIG,NDIR)        ! action density spectrum A(sigma,dir)
      REAL(rkind) XNL(NSIG,NDIR)          ! transfer rate dA/dt(sigma,dir)
      REAL(rkind) DIAG(NSIG,NDIR)         ! diagonal term (dXnl/dA)
      REAL(rkind) DIRR(NDIR)              ! single array with directions in radians
!------------------------------------------------------------------------------
!
!     --- initialisations
!
...
!
      INTEGER ICC
!
      REAL(rkind) :: W1, W2, W12
      REAL(rkind) :: K1, K2, K12
!
      REAL(rkind) :: DEP2(MCGRD)
!
...
!         to avoid NaN when W2*W12=0=K2*K12, adapt product of K2 and K12
          ...
          R     =  (0.5 + ((W2*W12)/(GRAV_W*DEP*KPROD))) * (K2 + K12)**2
!
          S     = (-2./GRAV_W) * (  (GRAV_W*DEP*K1)          &
     &                          + (B2*GRAV_W*DEP_3*K1**3)  &
     &                          - (B3*DEP_2*K1*W1**2)   )
!
      ENDIF
!
...
!      SUBROUTINE COAPP (AC2, SPCSIG, DEP2, IDCMIN, IDCMAX, INTPL, SX, &
      SUBROUTINE COAPP (      SPCSIG, DEP2, IDCMIN, IDCMAX, INTPL, SX, &
...
!
      REAL(rkind), INTENT(OUT) :: SX(MSC,MSC,2)
      REAL(rkind), INTENT(OUT) :: KX(MSC,MSC,2)
      REAL(rkind), INTENT(OUT) :: CGX(MSC,MSC,2)
      REAL(rkind), INTENT(OUT) :: CX(MSC,MSC,2)
      REAL(rkind), INTENT(OUT) :: E(MDC,MSC)
      REAL(rkind), INTENT(OUT) :: EI(MDC,MSC)
      REAL(rkind), INTENT(OUT) :: EX(MDC,MSC,MSC,2)
      REAL(rkind), INTENT(OUT) :: EIX(MDC,MSC,MSC,2)
      REAL(rkind), INTENT(OUT) :: WX(MSC,MSC,2)
!
!     Values from common
!
...
!
      ...
      REAL(rkind)    PEXC, PSUM   , PWDTH  , PAVL(2), PFAC(2)
      REAL(rkind)    XIS , XISLN
      REAL(rkind)    FX(MSC,MSC,2)
      REAL(rkind)    D   , TMP(4)
!
!
!  7. SUBROUTINES USED
...
!
      ...
      DO II = 1, MDC
         DO IS = 1, MSC
            E(II,IS) = AC2(II,IS,KCGRD(1)) *2. * PI_W * SPCSIG(IS)
         ENDDO
      ENDDO
!
...
!      SUBROUTINE SWLTA ( AC2   , DEP2  , CGO   , SPCSIG,        &
      SUBROUTINE SWLTA (         DEP2  , CGO   , SPCSIG,        &
...
!
     ...
     &        ISP, ISP1
      REAL(rkind)    AUX1, AUX2,BIPH, C0, CM, DEP, DEP_2, DEP_3, E0, ED0, EDM,&
     &        EM, FT, RINT, SIGPI, SINBPH, STRI, WISM, WISM1, WISP,    &
     &        WISP1, W0, WM, WN0, WNM, XIS, XISLN
      REAL(rkind), ALLOCATABLE :: E(:), ED(:), SA(:,:)
!
!  9. Subroutines calling
!
!     SOURCE
...
!       --- calculate biphase
!
        BIPH   = (0.5*PI_W)*(TANH(PTRIAD(4)/URSELL(KCGRD(1)))-1.)
        SINBPH = ABS( SIN(BIPH) )
!
!       --- calculate integral of energy density over directions
!
        ED(:) = SUM(AC2(:,:,KCGRD(1)),DIM=1)*2.*PI_W*SPCSIG(:) * DDIR ! 41.44
!
        DO II = IDDLOW, IDDTOP
           ID = MOD ( II - 1 + MDC , MDC ) + 1
!
...
!
           DO IS = 1, MSC
              E(IS) = AC2(ID,IS,KCGRD(1)) * 2. * PI_W * SPCSIG(IS)
           END DO
!
           DO IS = 1, ISMAX
...
!
 ...
 2000    FORMAT (' SWLTA: KCGRD ISMAX  :',2I4)
         WRITE(PRINTF,2001) GRAV_W, DEP, DEP_2, DEP_3
...
!      SUBROUTINE SWSPB ( AC2   , DEP2  , CGO   , SPCSIG,        &
      SUBROUTINE SWSPB (         DEP2  , CGO   , SPCSIG,        &
     &                   KWAVE , IMATRA, IMATDA, REDC0 , REDC1 ,&
     &                   IDDLOW, IDDTOP, ISSTOP, IDCMIN, IDCMAX,&
     &                   PLTRI )
!
...
!      REAL(rkind) :: KWAVE(MSC,MICMAX)
      REAL(rkind) :: KWAVE(MSC,ICMAX)
      REAL(rkind) :: PLTRI(MDC,MSC,NPTST)
      REAL(rkind) :: REDC0(MDC,MSC,MREDS)
      REAL(rkind) :: REDC1(MDC,MSC,MREDS)
!
!     Values from common
!     MDC       : Size of array in theta-direction
!     MSC       : Size of array in sigma-direction
...
!
      REAL(rkind) :: KP, K
!
      REAL(rkind) :: K1, K2, K12, W1, W2, W12, C1, C2, C12
      REAL(rkind) :: V0, R0, S0 , MU, DK
      REAL(rkind) :: R1, R2, R3 , S1, S2, S3
      REAL(rkind) :: I1, I2, I3 , EE1, EE2, EE3, CC1, CC2, CC3
!
      REAL(rkind) :: SIGPI, STRI
!
!     --- for use of COAPP
      REAL(rkind)    SX(MSC,MSC,2)
      REAL(rkind)    KX(MSC,MSC,2)
      REAL(rkind)    CGX(MSC,MSC,2)
      REAL(rkind)    CX(MSC,MSC,2)
      REAL(rkind)    E2(MDC,MSC)
      REAL(rkind)    EI(MDC,MSC)
      REAL(rkind)    EX(MDC,MSC,MSC,2)
      REAL(rkind)    EIX(MDC,MSC,MSC,2)
      REAL(rkind)    WX(MSC,MSC,2)
!
      REAL(rkind), ALLOCATABLE :: CONTS(:,:)   !sum contribution
      REAL(rkind), ALLOCATABLE :: CONTD(:,:)   !difference contribution
!
!  7. SUBROUTINES USED
!
...
!      CALL COAPP(AC2,SPCSIG,DEP2,IDCMIN,IDCMAX,1,SX, &
      CALL COAPP(    SPCSIG,DEP2,IDCMIN,IDCMAX,1,SX, &
...
!
      DO IS = 1, ISSTOP
          SIGPI = SPCSIG(IS) * 2. * PI_W ! Factor to convert back to N(sigma)
          DO II = IDCMIN(IS), IDCMAX(IS)
              ID = MOD ( II - 1 + MDC , MDC ) + 1
              !
...
!  4. Argument variables
      ...
      REAL(rkind), INTENT(IN)      :: ACLOC(1:MDC,1:MSC)                       ! local action density spectrum
      REAL(rkind), INTENT(IN)      :: DEPLOC                                   ! Depth at gridpoint ix,iy (obtained from SWANCOM1)
      REAL(rkind), INTENT(IN)      :: SPCSIG(1:MSC)                            ! Relative frequencies in computational domain in sigma-space
      REAL(rkind)                  :: KWAVE(1:MSC)                             ! Wave number in stencil points
      ...
      REAL(rkind)                  :: IMATDA(1:MDC,1:MSC)                      ! IMATDA: Diagonal of matrix
      REAL(rkind)                  :: IMATRA(1:MDC,1:MSC)                      ! IMATRA: Right hand vector of matrix
      REAL(rkind), INTENT(IN)      :: CGO(1:MSC)                               ! Group velocities in stencil points
      REAL(rkind), INTENT(IN)      :: KMESPC                                   ! Mean wave number of the spectrum
      REAL(rkind), INTENT(IN)      :: ETOT                                     !
      REAL(rkind), INTENT(IN)      :: SMEBRK                                   !
...
!  6. Local variables
      ...
      REAL(rkind)              :: SIG1, SIG2, SIG3                             ! frequencies of 3 components
      REAL(rkind)              :: E1, E2, E3                                   ! energy densities of 2 components
      REAL(rkind)              :: KW1, KW2, KW3                                ! wave numbers of 3 components
      REAL(rkind)              :: CG1, CG2                                     ! wave group velocities at interacting frequencies
      REAL(rkind)              :: RS3, SS                                      ! aux. var. for determining SIG3
      REAL(rkind)              :: DSIG                                         ! frequency increment
      REAL(rkind)              :: SIGMEAN                                      ! mean freq of the 3 components
      REAL(rkind)              :: KMEAN                                        ! mean wave number
      REAL(rkind)              :: SDIA1, SDIA2                                 ! source term to diagonal
      REAL(rkind)              :: SRHS
      REAL(rkind)              :: DISPC                                        ! dispersion coefficient
      REAL(rkind)              :: BETA
      REAL(rkind)              :: SINABS
      REAL(rkind)              :: BIPH
      REAL(rkind)              :: URSLOC                                       ! auxiliary coefficients
!
      REAL(rkind)              :: ED1, ED2, ED3                                ! integration of energy density over all directions
      REAL(rkind)              :: SDIA1B, SDIA2B
      REAL(rkind)              :: F(2)
      INTEGER           :: ISHAP
      REAL(rkind)              :: SMEAN
      REAL(rkind)              :: SHAP(2)                                      ! coefficient for shape
!
      REAL(rkind), ALLOCATABLE :: ED(:)                                        ! integration of energy density over all directions
...
!
      ISHAP = 0
!
      URSLOC = (GRAV_W*2.*SQRT(ETOT))/(SQRT(2.)*SMEBRK**2*DEPLOC**2)
      BIPH   = 0.5 * PI_W * (TANH(PTRIAD(4)/URSLOC) - 1.)
      SINABS = ABS(SIN(BIPH))
...
!
              IF (ISHAP.EQ.1) THEN
                 SHAP(1) = EXP(-1.25*((SIG1/(2.*PI_W))/0.4)**(-4))
                 SHAP(2) = EXP(-1.25*((SIG2/(2.*PI_W))/0.4)**(-4))
                 !
                 SDIA1  = SDIA1  / SHAP(1)
                 SDIA2  = SDIA2  / SHAP(2)
