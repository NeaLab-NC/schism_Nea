...
! Jerome back to original
!      REAL     AC2(MDC,MSC,0:MT)            ,                    &
      REAL(rkind)  IMATRA(MDC,MSC)              ,                    &
	       IMATLA(MDC,MSC)              ,                    &
	       IMATDA(MDC,MSC)              ,                    &
	       IMATUA(MDC,MSC)              ,                    &
	       IMAT5L(MDC,MSC)              ,                    &
...
!      print *,'SOLPRE : vertice (KCGRD(1)):',KCGRD(1)
!      print *,'SOLPRE : AC2 before, max:',MAXVAL(AC2(:,:,KCGRD(1)))

!     IF(KCGRD(1).EQ.78) THEN
!         PRINT *,'BEFORE'
!         PRINT *,'IMATDA(18,:)', IMATDA(18,:)
!      ENDIF
...
!               IF(AC2(ID,IS,KCGRD(1))/=AC2(ID,IS,KCGRD(1))) &
!     &         WRITE(PRINTF,*)'NAN in AC2 ID,IS INDEX',ID,IS,KCGRD(1)
...
!        DO ID = 1, MDC
!
!               IF(IMATDA(ID,IS)/=IMATDA(ID,IS)) &
!     &         WRITE(PRINTF,*)'NAN in IMATDA ID,IS',ID,IS
!
!        END DO
!      END DO
...
!      print *,'SOLPRE : AC2 after, max:',MAXVAL(AC2(:,:,KCGRD(1)))

! JL Skip now
# if 1
      IF ( TESTFL .AND. ITEST .GE. 70 ) THEN
!       WRITE (PRINTF,6120) IXCGRD(1)+MXF-2, IYCGRD(1)+MYF-2              
        WRITE (PRINTF,6120) KCGRD(1),iplg(KCGRD(1))
! 6120   FORMAT (' SOLPRE: Matrix values for point:', 2I5)
 6120   FORMAT (' SOLPRE: Matrix values for point (global):', 2I7)
        WRITE (PRINTF,6121)
...
!      print *,'SOLMAT : vertice (KCGRD(1)):',KCGRD(1)
!      print *,'SOLMAT : AC2 before, max:',MAXVAL(AC2(:,:,KCGRD(1)))

!      IF(KCGRD(1).EQ.79) THEN      
!        PRINT *,'SOLMAT bef. IMATDA(:,1:5)', IMATDA(:,1:5)
!      ENDIF
...
!
        DO 100 IDDUM = (ID_MIN+1), ID_MAX                                 !20.43
          ...
          WP   = IMATDA(IDM1,IS)                                          !20.43
          IF ( ABS(WP) .LE. 1.E-20 ) THEN
            TEMP = IMATLA(ID,IS) / SIGN( 1.E-20_rkind , WP)
          ELSE
            TEMP = IMATLA(ID,IS) / WP
          END IF


!          IF(KCGRD(1).EQ.79) THEN
!             PRINT *,'IDM1, IMATDA(IDM1,IS)',IDM1, IMATDA(IDM1,IS)
!             PRINT *,'TEMP, IMATUA(IDM1,IS)',TEMP, IMATUA(IDM1,IS)
!             PRINT *,'TEMP, IMATRA(IDM1,IS)',TEMP, IMATRA(IDM1,IS)
!          ENDIF
...
!
        ...
        WP = IMATDA(ID,IS)
        IF ( ABS(WP) .LE. 1.E-20 ) THEN
          TEMP = SIGN (1.E-20_rkind , WP)
        ELSE
          TEMP = WP
        END IF

!        IF(KCGRD(1).EQ.79) THEN
!         PRINT *,',SOLMAT ID  IMATDA',ID,WP
!         PRINT *,',SOLMAT TEMP ID_MAX',TEMP
!        ENDIF
...
!
      DO IS = 1, MSC
        DO ID = 1, MDC
          IMATRA(ID,IS) = 0.0_rkind
          IMATDA(ID,IS) = 0.0_rkind
        ENDDO
      ENDDO
!
      IF ( TESTFL .AND. ITEST.GE. 40 ) THEN

        WRITE (PRINTF,6020) KCGRD(1),iplg(KCGRD(1))
        !WRITE (PRINTF,6020) IXCGRD(1)+MXF-2, IYCGRD(1)+MYF-2
 6020   FORMAT(' SOLMAT: point :',2I7)
        WRITE(PRINTF,*)
      END IF
!
...
!
      REAL(rkind)     SP     ,TEMP   ,CORMAT ,TEMP1                             ! 40.23
!
...
!
          DO 100 IDDUM = (IDLOW+1 ), IDTOP
            ...
            IF ( ABS(SP) .LE. 1.E-20 ) THEN
              TEMP = IMATLA(ID,IS) / SIGN( 1.E-20_rkind , SP)
              TEMP1 = CORMAT / SIGN(1.E-20_rkind , SP)
            ELSE
...
!
          SP = IMATDA(IDTOP,IS)
          IF ( ABS(SP) .LE. 1.E-20_rkind ) THEN
            TEMP = SIGN (1.E-20_rkind , SP)
          ELSE
            TEMP = SP
          END IF
!
...
!      SUBROUTINE SWSIP ( AC2   , IMATDA, IMATRA, IMATLA, IMATUA,     &
      SUBROUTINE SWSIP (         IMATDA, IMATRA, IMATLA, IMATUA,     &
...
!
      ...
      REAL(rkind)    REPS
!      REAL    AC2(MDC,MSC,0:MT),                         &
      REAL(rkind)    IMATDA(MDC,MSC), IMATRA(MDC,MSC),           &
...
!
      REAL(rkind),parameter :: ZERO = 0._rkind
      REAL(rkind),parameter :: ONE = 1._rkind
      REAL(rkind),parameter :: ALFA  = ZERO
      REAL(rkind),parameter :: SMALL = 1.E-15
!
!  6. Local variables
!
...
!
      ...
      REAL(rkind)    BNORM, EPSLIN, P1, P2, P3, RNORM, RNRM0, UEPS
      REAL(rkind)    RES(MDC,MSC)   , CMATDA(MDC,MSC),                 &
...
!     --- construct L and U matrices (stored in CMAT[xx])
      ...
      CMATDA(ID,IS) = ONE/(IMATDA(ID,IS)+SMALL)
      ...
      DO IDDUM = IDMIN(IS)+1, IDMAX(IS)
         ...
         CMAT5L(ID,IS) = IMAT5L(ID,IS)
         CMATLA(ID,IS) = IMATLA(ID,IS)/(ONE+P2)
      ...
      DO IS = 2, ISSTOP
         ...
         CMAT5L(ID,IS) = IMAT5L(ID,IS)/(ONE+P1)
         CMATLA(ID,IS) = IMATLA(ID,IS)
         P1 = P1*CMAT5L(ID,IS)
         P3 = IMATDA(ID,IS) + P1                        &
      ...
      EPSLIN = REPS*BNORM
      UEPS   = 1000._rkind*UNDFLW*BNORM
      IF ( EPSLIN.LT.UEPS .AND. BNORM.GT.ZERO ) THEN
         IF ( IAMOUT.GE.1 ) THEN
            WRITE (PRINTF,'(A)')                            &
...
! JL 
!      IF ( ICONV.EQ.0 ) THEN
!         WRITE(PRTEST,50) iplg(KCGRD(1))
! 50      FORMAT('SWSIP: no_conv INDEX_GL:',I7)
!      ENDIF
...
!     SUBROUTINE SWSOR ( AC2   , IMATDA, IMATRA, IMATLA, IMATUA, &
      SUBROUTINE SWSOR (          IMATDA, IMATRA, IMATLA, IMATUA, &
                         IMAT5L, IMAT6U, AC2OLD, REPS  , MAXIT , &
                         IAMOUT, INOCNV, IDDLOW, IDDTOP, ISSTOP, &
                         IDCMIN, IDCMAX )
!  (This subroutine has not been used and tested yet) 
...
!
      INTEGER ICONV, ID, IDINF, IDDL, IDDT, IDDUM, IDM, IDP, IENT,  &
              IS, ISINF, ISM, ISP, IT
      INTEGER IDMIN(MSC), IDMAX(MSC)
      REAL(rkind)    AC2I, RES, RESM, RESM0
      REAL(rkind)    LOPERI(MSC), UPPERI(MSC), INVMDA(MDC,MSC)
!
!  7. Common blocks used
!
!
...
!
      ...
      REAL(rkind)    ACS2, ACS3, HSREL ,HSABS, HSCURV, HSDIFO, TMABS,      &
              XMOM0, XMOM1
      LOGICAL LHEAD, TSTFL
!
!  7. Common blocks used
...
!
 ...
 7004 FORMAT(' Spatial step         : DX     ',E12.4,' DY    ',E12.4)
      WRITE(PRINTF,7104) EXP(MyLOG(SHIG/SLOW)/REAL(MSC-1))-1.,&           ! 40.41
     &                   DDIR/DEGRAD                                     ! 40.41
 7104 FORMAT(' Spectral bin         : df/f   ',E12.4,' DDIR  ',E12.4)    ! 40.41
      WRITE(PRINTF,7003) GRAV_W, RHO_W                                   ! 40.41
 7003 FORMAT(' Physical constants   : GRAV   ',E12.4,' RHO   ',E12.4)    ! 40.41
      WRITE(PRINTF,6003) REARTH2
 6003 FORMAT(' Earth Radius         : REARTH2',E12.4)                    ! JL
...
!
      IF ( (DYNDEP .OR. ICUR.EQ.1)) THEN
       IF(INT(PNUMS(8)).EQ.1 ) THEN       ! 40.41
 ...
 8223    FORMAT('                      : NITER  ',I12)
       ELSEIF(INT(PNUMS(8)).EQ.2 ) THEN
         WRITE(PRINTF,*) 'Solver is SIGEX'
         WRITE(PRINTF,2214) PNUMS(7), PNUMS(19)
 2214    FORMAT('                      : CSS    ',E12.4,'  CFL ',E12.4)
       ELSEIF(INT(PNUMS(8)).EQ.3 ) THEN
         WRITE(PRINTF,*) 'Solver is EXP'
         WRITE(PRINTF,2215) PNUMS(9)
 2215    FORMAT('                      : DIFFC  ',E12.4)
       ENDIF
      ENDIF
!
      IF (ICUR.GT.0) THEN
...
!
 ...
 7110   FORMAT(' Diffraction          : SMPAR  ',E12.4,&
     &         ' SMNUM ',I12)
      ...
      IF (IREFR.EQ.0) THEN
        WRITE (PRINTF, *) 'Refraction is off'
      ENDIF
!
...
!      SUBROUTINE SINTGRL(SPCDIR  ,KWAVE   ,AC2     ,                      40.02
      SUBROUTINE SINTGRL(SPCDIR  ,KWAVE   ,                  &   !  40.02
...
!     TMBOT : near bottom wave period                                     40.51
      ...
      REAL(rkind), INTENT(IN)     :: DEP2(MCGRD), BOTLV(MCGRD)
      REAL(rkind), INTENT(INOUT)  :: GAMBR(MCGRD), RESPL(MCGRD)                !  41.38
!     Changed ICMAX to MICMAX, since MICMAX doesn't vary over gridpoint   40.22
!      REAL, INTENT(IN)     :: KWAVE(MSC,MICMAX)                         !  40.22
! JL Wrong : ICMAX != MICMAX
      REAL(rkind), INTENT(IN)     :: KWAVE(MSC,ICMAX)
!      REAL, INTENT(IN)     :: RDX(MICMAX), RDY(MICMAX)                  !  40.08
! JL Wrong : rdx, rdy dimension = 2
      REAL(rkind), INTENT(IN)     :: RDX(2), RDY(2)
      REAL(rkind), INTENT(IN)     :: SPCDIR(MDC,6)
!
!      REAL, INTENT(IN OUT) :: AC2(MDC,MSC,MCGRD)
      REAL(rkind), INTENT(IN OUT) :: QB(MCGRD)
      REAL(rkind), INTENT(IN OUT) :: UBOT(MCGRD)
      REAL(rkind), INTENT(IN OUT) :: URSELL(MCGRD)
      REAL(rkind), INTENT(IN OUT) :: TMBOT(MCGRD)                              !  40.51
!
      REAL(rkind), INTENT(OUT)    :: ABRBOT, ETOT, HM, HS, QB_LOC
      REAL(rkind), INTENT(OUT)    :: AC2TOT, KMESPC, SMEBRK, KTETA
!
!  6. Local variables
!
!     IENT  : Number of entries in this subroutine
...
!
      REAL(rkind)              :: AB2, EMAX, UB2, ETOT_2, KLOC(1), ARR(1)
      REAL(rkind)              :: WH
      REAL(rkind)              :: FRINTF_X_DDIR
      REAL(rkind)              :: BRCOEF   ! variable breaking coefficient (calc. in BRKPAR)  40.22
!
      REAL(rkind)              :: ETOT_DSIG(MSC)                               !  40.22
      REAL(rkind)              :: ACTOT_DSIG(MSC), ETOT_DSHKD2_DSIG(MSC)       !  40.22
      REAL(rkind)              :: ETOT_DRK_DSIG(MSC), ETOT_SIG2_DSIG(MSC)      !  40.22
      REAL(rkind)              :: ETOT_K_DSIG(MSC), ETOT_SIG_DSIG(MSC)         !  40.22
      REAL(rkind)              :: ETOT_SIG2_DSHKD2_DSIG(MSC)                   !  40.22
      REAL(rkind)              :: ETOT_SIG4_DSIG(MSC)                          !  40.22
      REAL(rkind)              :: SINH_K_X_DEP_2(MSC)                          !  40.22
      REAL(rkind)              :: ETOT_SIG_2_DSIG(MSC)                         !  41.38
      REAL(rkind),PARAMETER  :: ZERO=0._rkind
...
!
      QB(KCGRD(1))   = 0._rkind
      ABRBOT         = 0.001_rkind
      UBOT(KCGRD(1)) = 0._rkind
      TMBOT(KCGRD(1))= 0._rkind
!
...
!
      EMAX = 0.25_rkind * ( PSURF(2) * DEP2(KCGRD(1)) )**2._rkind
!
!     --- reduce action density if necessary
!
      IF (ACUPDA .AND. ETOT .GT. EMAX .AND. ISURF .GE. 1) THEN          !  40.13
         AC2(:,:,KCGRD(1)) = MAX(0._rkind,(EMAX/ETOT)*AC2(:,:,KCGRD(1)))
!
         IF (TESTFL.AND.ITEST.GE.80) WRITE (PRTEST,7) DEP2(KCGRD(1)), EMAX, ETOT
...
!
!
        SINH_K_X_DEP_2(:)   = SINH(MIN(30._rkind,KWAVE(:,1)* &
     &                        DEP2(KCGRD(1))))**2._rkind
...
!
        IF (ETOT1  .GT. ZERO) SIGM01  = ETOT1 / ETOT
        IF (EKTOT  .GT. ZERO) KM01    = EKTOT / ETOT
        IF (ACTOT  .GT. ZERO) SIGM_10 = ETOT / ACTOT
        IF (EDRKTOT .GT. ZERO) THEN
          KM_WAM  = ( ETOT / EDRKTOT )**2._rkind
          SIGM_WAM = SQRT(GRAV_W*KM_WAM*TANH(KM_WAM*DEP2(KCGRD(1))))      !  41.47
        ENDIF
...
!
        IF ( UB2 .GT. ZERO) UBOT(KCGRD(1)) = SQRT ( UB2 )
        IF ( AB2 .GT. ZERO) ABRBOT = SQRT (2._rkind *  AB2)
        IF ( UB2 .GT. ZERO .AND. AB2 .GT. ZERO) &                        !  40.51
     &                             TMBOT(KCGRD(1)) = PI2_W*SQRT(AB2/UB2) ! 40.51

        IF (TESTFL .AND. ITEST.GE.40) THEN
         WRITE(PRTEST,855)iplg(KCGRD(1)),ETOT,HS,UB2,AB2, &
     &                    UBOT(KCGRD(1)), ABRBOT,TMBOT(KCGRD(1))
 855     FORMAT ('SINTGRL: INDEX_GL ETOT Hs UB2 AB2 UBOT ABRBOT TMBOT',&
     &           I7,7(1X, E11.4))
        END IF
...
!         PRINT *,'SINTGL BRKPAR not finished, sherical corr. todo'
   
!         CALL BRKPAR (BRCOEF, SPCDIR(1,2), SPCDIR(1,3), AC2,        &
         CALL BRKPAR (BRCOEF, SPCDIR(1,2), SPCDIR(1,3),             &
...
!       breaking disabled, assign very high value to HM
      ...
      IF (TESTFL .AND. ITEST.GE.40) THEN
         WRITE(PRTEST,866)iplg(KCGRD(1)),BRCOEF,DEP2(KCGRD(1)),&
     &         BOTLV(KCGRD(1)),GAMBR(KCGRD(1)),KTETA
 866     FORMAT ('SINTGRL: INDEX_GL BRCOEF DEP2 BOTLV GAMBR KTETA',&
     &           I7,5(1X, E11.4))
      END IF
...
!
        ELSEIF (ISURF.EQ.4) THEN
           WH = (2._rkind*SQRT(2._rkind*ETOT)/HM)**PSURF(5)
           WH = MIN(1.,WH)
           QB(KCGRD(1)) = WH
        ENDIF
!
...
!
     ...
     &           (IXCGRD(1).EQ.MXC .AND. IYCGRD(1).EQ.1) )) THEN        !  40.16
           URSELL(KCGRD(1)) = (GRAV_W*HS) /&
     &     (2._rkind*SQRT(2.)*SIGM01**2._rkind*DEP2(KCGRD(1))**2._rkind)
         ENDIF                                                          !  40.16
      ELSE                                                              !  40.41
        URSELL(KCGRD(1)) = 0._rkind                                     !  40.41
      END IF                                                            !  40.41
!
      QB_LOC = QB(KCGRD(1))
!
!     *** test output ***
!
      IF (TESTFL .AND. ITEST.GE.2) THEN
         WRITE(PRTEST, 901) ETOT, HS, SIGM_10, KM_WAM, ABRBOT,QB_LOC
 901     FORMAT (' SINTGRL: ETOT Hs Sigma K Aorb QB', 6(1X, E11.4))
      END IF
!
...
!   &                     ECOS       ,ESIN       ,AC2        ,& !JL : AC2 Global, spcdir give ecos esin
     &                     SPCDIR                             ,&
     &                     DEP2       ,IMATDA     ,IMATRA     ,&
     ...
     &                     MEMNL4     ,WWINT      ,WWAWG      ,&
!     &                     WWSWG      ,CGO        ,USTAR      ,&
     &                     WWSWG      ,CGO  ,USTAR, CHARN     ,&! JL ADD CHARNOCK output
!     &                     ZELEN      ,SPCDIR     ,ANYWND     ,& !JL change,spcdir at the begining
     &                     ZELEN      ,ANYWND     ,DMW        ,&
     ...
     &                     URSELL     ,ANYBIN     ,REFLSO     ,&        !  40.41 40.03
                                                   PLWNDA     ,&        ! JL change
     &                     PLWNDB     ,PLWCAP     ,PLBTFR     ,&        ! JL change
     &                     PLWBRK     ,PLNL4S     ,PLNL4D     ,&        ! JL change 
     &                     PLVEGT     ,PLTURB     ,PLMUD      ,&        ! ! JL change 40.59 40.35 40.55
     &                     PLTRI                               &
     &                                                        )         !  30.21
!
!****************************************************************
...
!      USE VARS_WAVE, ONLY : AC2,MSR !, MT
!      USE MOD_PAR, ONLY: NGID,NLID
...
!     JL Apr 2017 CHARN : Charnock Coefficient from SWIND4
!     
!  2. Purpose
!
...
!                     Janssen (1989,1990) wind input formulation
!     CHARN     2D    Charnock Coefficient from eq. Z0=CHARN*USTAR^2/G
!                     after iterative computation of USTAR using the
!                     wave growth rate following the parameterization of
!                     Janssen et al. (see SWIND4)   
!
...
!
     ...
     &         IDDTOP  ,IDDLOW  ,IX      ,IY
!
      REAL(rkind)     ABRBOT  ,ETOT    ,HM      ,QBLOC   ,ETOTW   ,&
     &         FPM     ,WIND10  ,THETAW  ,SMESPC  ,KMESPC  ,&
     &         SNLC1   ,FACHFR  ,DAL1    ,DAL2    ,DAL3    ,&
     &         UFRIC   ,SMEBRK  ,HS      ,XIS     ,KTETA
!
!      REAL(rkind)  :: AC2(MDC,MSC,MCGRD)                                       !  30.21
      REAL(rkind)  :: DEP2(MCGRD)
      REAL(rkind)  :: ALIMW(MDC,MSC)
      REAL(rkind)  :: IMATDA(MDC,MSC)
      REAL(rkind)  :: IMATRA(MDC,MSC)
!     Changed ICMAX to MICMAX, since MICMAX doesn't vary over gridpoint   40.22
!      REAL(rkind)  :: KWAVE(MSC,MICMAX)                                        !  40.22
!      REAL(rkind)  :: DMW(MSC,MICMAX)                                          !  40.59
! JL Wrong MICMAX !=ICMAX
      REAL(rkind)  :: KWAVE(MSC,ICMAX)
      REAL(rkind)  :: DMW(MSC,ICMAX)                                          
      REAL(rkind)  :: UBOT(MCGRD)
      REAL(rkind)  :: UX2(MCGRD)
      REAL(rkind)  :: UY2(MCGRD)
      ...
      REAL(rkind)  :: SFNL(MSC4MI:MSC4MA , MDC4MI:MDC4MA )
      REAL(rkind)  :: DSNL(MSC4MI:MSC4MA , MDC4MI:MDC4MA )
      REAL(rkind)  :: MEMNL4(MDC,MSC,MCGRD)
      REAL(rkind)  :: PLWNDA(MDC,MSC,NPTST)
      REAL(rkind)  :: PLWNDB(MDC,MSC,NPTST)
      REAL(rkind)  :: PLWCAP(MDC,MSC,NPTST)
      REAL(rkind)  :: PLBTFR(MDC,MSC,NPTST)
      REAL(rkind)  :: PLWBRK(MDC,MSC,NPTST)
      REAL(rkind)  :: PLNL4S(MDC,MSC,NPTST)
      REAL(rkind)  :: PLNL4D(MDC,MSC,NPTST)
      REAL(rkind)  :: PLVEGT(MDC,MSC,NPTST)                                    !  40.55
      REAL(rkind)  :: PLTURB(MDC,MSC,NPTST)                                    !  40.35
      REAL(rkind)  :: PLMUD (MDC,MSC,NPTST)                                    !  40.59
      REAL(rkind)  :: PLTRI(MDC,MSC,NPTST)
      REAL(rkind)  :: WWAWG(*)
      REAL(rkind)  :: WWSWG(*)
!     REAL(rkind)  :: CGO(MSC,MICMAX)                                          !  40.22
!JL MICMAX != ICMAX
      REAL(rkind)  :: CGO(MSC,ICMAX)
      REAL(rkind)  :: USTAR(MCGRD)
      REAL(rkind)  :: ZELEN(MCGRD)
      REAL(rkind)  :: CHARN(MCGRD)                                             ! JL
      REAL(rkind)  :: DISSC0(1:MDC,1:MSC,1:MDISP)                              !  40.67
      REAL(rkind)  :: DISSC1(1:MDC,1:MSC,1:MDISP)                              !  40.67
      REAL(rkind)  :: GENC0 (1:MDC,1:MSC,1:MGENR)                              !  40.85
      REAL(rkind)  :: GENC1 (1:MDC,1:MSC,1:MGENR)                              !  40.85
      REAL(rkind)  :: REDC0 (1:MDC,1:MSC,1:MREDS)                              !  40.85
      REAL(rkind)  :: REDC1 (1:MDC,1:MSC,1:MREDS)                              !  40.85
      REAL(rkind)  :: URSELL(MCGRD)                                            !  40.03
      REAL(rkind)  :: FRCOEF(MCGRD)                                            !  20.68
      REAL(rkind)  :: NPLA2(MCGRD)                                             !  40.55
      REAL(rkind)  :: TURBV2(MCGRD)                                            !  40.35
      REAL(rkind)  :: MUDL2(MCGRD)                                             !  40.59
      REAL(rkind)  :: REFLSO(MDC,MSC)                                          !  40.41
...
!
      DISSC0(1:MDC,1:MSC,1:MDISP) = 0._rkind                                  !  40.67
      DISSC1(1:MDC,1:MSC,1:MDISP) = 0._rkind                                  !  40.67
!
!     *** set all generation coeff at 0 ***
!
      GENC0(1:MDC,1:MSC,1:MGENR) = 0._rkind                                   !  40.85
      GENC1(1:MDC,1:MSC,1:MGENR) = 0._rkind                                   !  40.85
!
!     *** set all redistribution coeff at 0 ***
!
      REDC0(1:MDC,1:MSC,1:MREDS) = 0._rkind                                   !  40.85
      REDC1(1:MDC,1:MSC,1:MREDS) = 0._rkind                                   !  40.85
!
! JL set ECOS, ESIN (not passed through argin as before)
      ECOS(1:MDC) = SPCDIR(1:MDC,2)
      ESIN(1:MDC) = SPCDIR(1:MDC,3)
...
!       *** wave-bottom interactions ***
       ...
       !IF(TESTFL) THEN
       !PRINT *,'SOURCE : wave-bottom interactions'
       !PRINT *,'ABRBOT,DEP2,ECOS,ESIN',ABRBOT,DEP2 ,ECOS,ESIN
       !PRINT *,'ECOS',ECOS
       !PRINT *,'ESIN',ESIN
       !PRINT *,'ECOS SPCDIR(:,2)',SPCDIR(:,2)
       !PRINT *,'ESIN SPCDIR(:,3)',SPCDIR(:,3)
       !WRITE(PRINTF, *)'SOURCE isstop,hs',isstop,hs
       !WRITE(PRINTF, *)'SOURCE FPM',FPM
       !PRINT *,'SOURCE UBOT',UBOT
       !WRITE(PRTEST,*) 'SOURCE UFRIC',UFRIC
!                    PRINT *,'SOURCE idcmax , iddlow , iddtop , idwmin , idwmax',&
!                     idcmax , iddlow , iddtop , idwmin , idwmax
!                    PRINT *,'isstop , swtsda(1,1,1,JPWNDA)',&
!                    isstop , PLWNDA
       !PRINT *,'SOURCE : cgo ',cgo
       ! PRINT *,'SOURCE sIZE PLTRI',size(PLTRI)
       ! PRINT *,'SOURCE swtsda(1,1,1,JPTRI)',PLTRI
       !  PRINT *,'SOURCE ANYBIN',size(ANYBIN)
       !  PRINT *,ANYBIN
       !  PRINT *,'SOURCE REFLSO',size(REFLSO)
       !  PRINT *,REFLSO
       !ENDIF

!        CALL SBOT (ABRBOT   ,DEP2     ,ECOS     ,ESIN     ,AC2      ,&  ! 41.04
        CALL SBOT (ABRBOT   ,DEP2     ,ECOS     ,ESIN     ,          &  ! 41.04
     &             IMATDA   ,KWAVE    ,SPCSIG   ,UBOT     ,UX2      ,&  ! 30.72
     &             UY2      ,IDCMIN   ,IDCMAX   ,IT       ,ITER     ,&  ! 41.51
!     &             SWPDIR   ,PLBTFR   ,ISSTOP   ,DISSC1   ,VARFR    ,&  ! 41.51 40.67
!     &             FRCOEF   )
     &             SWPDIR            ,ISSTOP   ,DISSC1   ,VARFR    ,& !     ! 41.51 40.67
     &             FRCOEF ,PLBTFR  )

       !IF (ITEST.GE.140)
       ! DO ii=1,MSC
       !  DO jj=1,MDC
       !   IF(IMATDA(jj,ii)/=IMATDA(jj,ii))THEN
       !    WRITE(PRINTF,*)'SBOT NAN in imatda ID,IS,ivert',jj,ii,KCGRD(1)
...
!#ifdef SCHISM  JL MAr 2022
!         *** calculate Friction dissipation acceleration x/y
!        CALL COMPUTE_SBF(IDCMIN, IDCMAX, ISSTOP, DISSC1)
!#endif

      END IF

!      PRINT *,'SBOT : IMATDA ',IMATDA(:,1:5)
!
...
!
     ...
     &                KWAVE   ,DISSC1  ,PLTURB  )                       ! 40.35
       ...
       !   DO jj=1,MDC
       !    IF(IMATDA(jj,ii)/=IMATDA(jj,ii))THEN
       !    WRITE(PRINTF,*)'STURBV NAN in imatda ID,IS,ivert',jj,ii,KCGRD(1)
       !    call parallel_abort('stop')
       !    ENDIF
       !   ENDDO
       !  ENDDO
...
!       PRINT *,'SOURCE : surf breaking source term'!

          CALL SSURF (ETOT    ,HM      ,QBLOC   ,SMEBRK  ,KTETA   ,& ! 41.47
!     &                KMESPC  ,SPCSIG  ,AC2     ,IMATRA  ,         & ! 30.81
     &                KMESPC  ,SPCSIG  ,         IMATRA  ,         & ! 30.81
!     &                IMATDA  ,IDCMIN  ,IDCMAX  ,PLWBRK  ,         &
!     &                ISSTOP  ,DISSC0  ,DISSC1  )                   ! 40.67 40.61 30.21
     &                IMATDA  ,IDCMIN  ,IDCMAX  ,                  &
     &                ISSTOP  ,DISSC0  ,DISSC1  ,PLWBRK)             !     40.67 40.61 30.21
!

         ! DO ii=1,MSC
         !  DO jj=1,MDC
         !   IF(IMATDA(jj,ii)/=IMATDA(jj,ii))THEN
         !    WRITE(PRINTF,*)'SSURF NAN in imatda ID,IS,ivert',jj,ii,KCGRD(1)
         !    call parallel_abort('stop')
         !   ENDIF
         !  ENDDO
         ! ENDDO
...
!#ifdef SCHISM  JL MAr 2022
!         *** calculate compute wave breaking dissipation acceleration x/y
!         CALL COMPUTE_SBR(IDCMIN, IDCMAX, ISSTOP, DISSC1)
!#endif

      END IF

!      PRINT *,'SOURCE : IWIND =',IWIND
...
!        IF(MSR) WRITE(PRINTF,*)'before SWIND0 GENC0',GENC0
!        IF(MSR) WRITE(PRINTF,*)'before SWIND0 IMATRA',IMATRA
         CALL SWIND0 (SPCSIG ,SPCDIR ,UFRIC ,FPM ,ISSTOP,    &
     &                THETAW ,IMATRA ,GENC0 ,IDCMIN ,IDCMAX, &
     &                ANYWND, PLWNDA  )
        ENDIF
      ENDIF
!
...
!
!        IF(MSR) WRITE(PRINTF,*)'before SWIND3 GENC0',GENC0
!        IF(MSR) WRITE(PRINTF,*)'before SWIND3 IMATRA',IMATRA


!         IF(NGID(KCGRD(1)).EQ.48260) WRITE(PRINTF,*)'ITERATION',ITER
!         IF(NGID(KCGRD(1)).EQ.48260) WRITE(PRINTF,*)'Sweep Dir',SWPDIR
!         IF(NGID(KCGRD(1)).EQ.48260) &
!         WRITE(PRINTF,*)'before SWIND3 IMATRA',IMATRA(:,41)


!        WRITE(PRINTF,*)'before SWIND3 FPM ISSTOP',FPM     ,ISSTOP
!        WRITE(PRINTF,*)'before SWIND3 IDCMIN',IDCMIN
!        WRITE(PRINTF,*)'before SWIND3 IDCMAX',IDCMAX

!        WRITE(PRINTF,*)'before SWIND3 UFRIC ',UFRIC 
!        WRITE(PRINTF,*)'before SWIND3 KWAVE',KWAVE
        CALL SWIND3 (SPCSIG  ,SPCDIR, UFRIC, FPM, ISSTOP, THETAW  , &
                     KWAVE   ,IMATRA , &
!    &               IDCMIN  ,IDCMAX  ,AC2     ,UFRIC   ,       &
     &               GENC0, IDCMIN  ,IDCMAX  ,         &
!     &               FPM     ,PLWNDA  ,ISSTOP  ,SPCDIR  ,ANYWND  ) ! JL Change
     &               ANYWND, PLWNDA  )

!        IF(MSR) WRITE(PRINTF,*)'after SWIND3 GENC0',GENC0
!        IF(MSR) WRITE(PRINTF,*)'after SWIND3 IMATRA',IMATRA

!         IF(NGID(KCGRD(1)).EQ.48260) &
!         WRITE(PRINTF,*)'after SWIND3 IMATRA',IMATRA(:,41)
...
!
        CALL SWIND4  (IDWMIN  ,IDWMAX , SPCSIG ,WIND10 ,THETAW ,XIS ,&
     &                DDIR    ,KWAVE   ,IMATRA  ,GENC0   ,           &
!     &                IDCMIN  ,IDCMAX  ,AC2     ,UFRIC   ,           &
     &                IDCMIN  ,IDCMAX           ,UFRIC   ,           &
!     &                PLWNDA  ,ISSTOP  ,ITER    ,USTAR   ,ZELEN   ,  &
!     &                SPCDIR  ,ANYWND  ,IT      )
     &                ISSTOP  ,ITER    ,USTAR   ,ZELEN   , CHARN,     &
     &                SPCDIR  ,ANYWND  ,IT      ,PLWNDA  )
...
!     &               IMATRA  ,IDCMIN  ,IDCMAX  ,AC2,               &
     &               IMATRA  ,IDCMIN  ,IDCMAX  ,                   &
!     &               ANYWND  ,PLWNDA  , SPCDIR  ,GENC0             )
     &               ANYWND  , SPCDIR  ,GENC0 , PLWNDA      )
      ...
      ! DO jj=1,MDC
      !  IF(IMATDA(jj,ii)/=IMATDA(jj,ii))THEN
      !  WRITE(PRINTF,*)'SWIND NAN in imatda ID,IS,ivert,IWIND',jj,ii,KCGRD(1),IWIND
      !  call parallel_abort('stop')
      !  ENDIF
      ! ENDDO
      !ENDDO
...
!      PRINT *,'SOURCE : Calculate whitecapping source term'

      IF (IWCAP.GE.1) THEN
!      IF (IWCAP.LE.7) CALL SWCAP (SPCDIR  ,SPCSIG  ,KWAVE   ,AC2     ,& !  40.02

!         IF(NGID(KCGRD(1)).EQ.48260) &
!         WRITE(PRINTF,*)'before SWCAP IMATRA',IMATRA(:,41)


      IF (IWCAP.LE.7) &
         CALL SWCAP (SPCDIR  ,SPCSIG  ,KWAVE            ,& !  40.02
     &               IDCMIN  ,IDCMAX  ,ISSTOP  ,ETOT    ,& !  40.02
!     &              IMATDA  ,IMATRA  ,PLWCAP  ,CGO     ,& !  40.02
!     &              UFRIC  ,DEP2    ,DISSC1  ,DISSC0 )    !  40.53
     &               IMATDA  ,IMATRA  ,CGO              ,&  !  40.02
     &               UFRIC  ,DEP2    ,DISSC1  ,DISSC0, PLWCAP )     !  40.53
      ...
      ! DO jj=1,MDC
      !  IF(IMATDA(jj,ii)/=IMATDA(jj,ii))THEN
      !  WRITE(PRINTF,*)'SWCAP NAN in imatda ID,IS,ivert,IWCAP',jj,ii,KCGRD(1),IWCAP
      !  call parallel_abort('stop')
      !  ENDIF
      ! ENDDO
      !ENDDO
...
!         IF(NGID(KCGRD(1)).EQ.48260) &
!         WRITE(PRINTF,*)'after SWCAP IMATRA',IMATRA(:,41)
...
!      PRINT *,'SOURCE : compute nonlinear interactions'
!       PRINT *,'SOURCE : ITRIAD = ',ITRIAD   = 1
...
!              CALL SWLTA (          DEP2  , CGO   , SPCSIG,        &    ! 40.56
!     &                     KWAVE , IMATRA, IMATDA, REDC0 , REDC1 , &    ! 40.85
!     &                     IDDLOW, IDDTOP, ISSTOP, IDCMIN, IDCMAX, &
!!     &                     HS    , SMEBRK, PLTRI , URSELL )
!     &                     HS    , SMEBRK, URSELL, PLTRI )

!             IF(NGID(KCGRD(1)).EQ.48260) &
!              WRITE(PRINTF,*)'before SWLTA IMATRA',IMATRA(:,41)


              CALL SWLTA (         DEP2  , CGO   , SPCSIG,        & ! 40.56
     &                   KWAVE , IMATRA, IMATDA, REDC0 , REDC1 ,  & ! 40.56
     &                   IDDLOW, IDDTOP, ISSTOP, IDCMIN, IDCMAX,  & ! 40.85
     &                   HS    , SMEBRK, URSELL , PLTRI )

!             IF(NGID(KCGRD(1)).EQ.48260) &
!              WRITE(PRINTF,*)'after SWLTA IMATRA',IMATRA(:,41)

           ELSEIF (ITRIAD.EQ.2) THEN
!             SPB
!             CALL SWSPB (AC2   , DEP2  , CGO   , SPCSIG,        &
              CALL SWSPB (         DEP2  , CGO   , SPCSIG,        &
...
!       PRINT *,'SOURCE : compute quadruplet interactions'
!       PRINT *,'SOURCE : IQUAD = ',IQUAD  =2
...
!     &                 SFNL    ,DSNL    ,DEP2    ,AC2     ,IMATDA  ,&
     &                 SFNL    ,DSNL    ,DEP2    ,          IMATDA  ,&
     &                 IMATRA                    ,                   &  !  34.00
     &                 IDDLOW  ,IDDTOP  ,REDC0  ,REDC1, PLNL4S  ,PLNL4D)!  40.85 34.00
!
...
!             IF(NGID(KCGRD(1)).EQ.48260) &
!              WRITE(PRINTF,*)'before SWSNL2 IMATRA',IMATRA(:,41)
...
!     &                 DAL3    ,SFNL    ,DEP2    ,AC2     ,KMESPC  ,&
     &                 DAL3    ,SFNL    ,DEP2    ,         KMESPC  ,&
     &                          REDC0   ,REDC1   ,IMATDA  ,IMATRA  ,& !40.85 40.23 34.00
     &                 FACHFR  ,         IDCMIN  ,IDCMAX, PLNL4S  )  !34.00
!
!             IF(NGID(KCGRD(1)).EQ.48260) &
!              WRITE(PRINTF,*)'after SWSNL2 IMATRA',IMATRA(:,41)
...
!          CALL FILNL3 (IDCMIN  ,IDCMAX  ,IMATRA  ,IMATDA  ,AC2     ,&   !  40.23
          CALL FILNL3 (IDCMIN  ,IDCMAX  ,IMATRA  ,IMATDA  ,         &   !  40.23
     &                 MEMNL4  ,ISSTOP  ,REDC0   ,REDC1 ,PLNL4S  )    !  40.85 40.41
...
!
!JQI            CALL SWINTFXNL(AC2,SPCSIG,SPCDIR,MDC,MSC,MCGRD,         &   !  40.41
!     &                     DEP2,IQUAD,MEMNL4,KCGRD,ICMAX,IQERR)         !  40.41
!
          ENDIF                                                         !  40.41

!          PRINT *,'IQUAD IMATDA',IMATDA(:,1:5)
!
...
!          ENDIF                                                         !  40.41
!
!          PRINT *,'SOURCE : CALL FILNL3'

!          CALL FILNL3 (IDCMIN  ,IDCMAX  ,IMATRA  ,IMATDA  ,AC2     , &  !  40.41
          CALL FILNL3 (IDCMIN  ,IDCMAX  ,IMATRA  ,IMATDA  ,          &  !  40.41
     &                 MEMNL4  ,ISSTOP  ,REDC0   ,REDC1  ,PLNL4S )      !  40.85 40.41
!
!          IF (ITEST.GE.30) THEN                                         !  40.41
...
!         END DO                                                         !  40.41
      END IF                                                            !  40.41
!
!      PRINT *,'     End of the subroutine SOURCE'
...
!     SUBROUTINE PHILIM(AC2,AC2OLD,CGO,KWAVE,SPCSIG,ANYBIN,ISLMIN,NFLIM, &
      SUBROUTINE PHILIM(    AC2OLD,CGO,KWAVE,SPCSIG,ANYBIN,ISLMIN,NFLIM,&
     &                  QB_LOC)
!                                                                       *
!************************************************************************
!
...
!
      IF (MSC.GT.3) THEN
         DO IS=1,MSC
            DAC2MX=ABS((PNUMS(20)*0.0081_rkind)/&
     &             (2._rkind*SPCSIG(IS)*(KWAVE(IS,1)**3)*CGO(IS,1)))
            DO ID=1,MDC
               IF (ANYBIN(ID,IS) .AND.&
...
! 13. Source text
!
!      C_HJ = PI2_W**2*3.0*1.0E-7*DT*SPCSIG(MSC)
!      SPM_NOND = PI2_W * 5.6 * 1.0E-3
!      IF (MSC.GT.3) THEN
!         DO IS=1,MSC
!            UFRIC_VEL = MAX(USTAR(KCGRD(1)), GRAV_W*SPM_NOND/SPCSIG(IS))
...
!      SUBROUTINE RESCALE (AC2, ISSTOP, IDCMIN, IDCMAX, NRSCAL)
      SUBROUTINE RESCALE (     ISSTOP, IDCMIN, IDCMAX, NRSCAL)
!
!****************************************************************
...
!
      DO 180 IS = 1 , ISSTOP
        ...
        DO 160 IDDUM = IDCMIN(IS), IDCMAX(IS)
          ...
          IF ( AC2(ID,IS,KCGRD(1)) .LT. 0._rkind ) THEN
...
!
          DO 170 IDDUM = IDCMIN(IS), IDCMAX(IS)
            ID = MOD ( IDDUM - 1 + MDC , MDC ) + 1
            IF ( AC2(ID,IS,KCGRD(1)) .LT. 0._rkind) THEN
              AC2(ID,IS,KCGRD(1)) = 0._rkind
            END IF
            IF ( FACTOR .GE. 0._rkind ) THEN
...
# if 1
     ...
     &  WRITE (PRINTF, 171) KCGRD(1),iplg(KCGRD(1)), IS,&
     &  FACTOR , ATOT, ATOTP
 171    FORMAT('Rescale in Point INDEX_GL, Isig, Factor, ATOT, ATOTP:',&
     &  3I7, 3(1X,E11.4))
# endif
        ENDIF
 180  CONTINUE
