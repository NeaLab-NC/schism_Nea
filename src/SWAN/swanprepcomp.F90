...
!
    ...
    DO ivert = 1,nverts
      IF(MAXVAL(vmark)<2)THEN     ! Mean that OPEN OBC is flagged with value = 1
         IF(vmark(ivert)==1) vert(ivert)%atti(VBC) = 1
      ELSEIF(MAXVAL(vmark)>1)THEN ! Mean that OPEN OBC is flagged with value = 2 
         IF(vmark(ivert)==2) vert(ivert)%atti(VBC) = 1
      ENDIF
       ! ghost vertices are regarded as vertices with boundary condition
       if ( vmark(ivert) == 999 ) vert(ivert)%atti(VBC) = 1

       ! Debug, 2 mpi threads
       !IF(MSR)       WRITE(PRTEST,*) 'cpu',MYID,' v mk',NGID_X(ivert),vmark(ivert),NGID(ivert)
       !IF(MYID.EQ.2) WRITE(PRINTF,*) 'cpu',MYID,' v mk',NGID_X(ivert),vmark(ivert),NGID(ivert)
    ENDDO


    ! setup a vertex list
    !
    !PRINT *,'SwanPrepComp : call SwanVertlist'
...
!    if ( NUMOBS > 0 ) call SwanFindObstacles ( cross )
    if ( NUMOBS > 0 ) call SwanFindObstacles
    !
