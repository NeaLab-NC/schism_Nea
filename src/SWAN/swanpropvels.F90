...
!
    ...
    !
    real(rkind), dimension(MDC,MSC), intent(out)      :: cad    ! wave transport velocity in theta-direction
    real(rkind), dimension(MDC,MSC), intent(out)      :: cas    ! wave transport velocity in sigma-direction
    ...
    real(rkind), dimension(MSC,ICMAX), intent(in)     :: cgo    ! group velocity
    real(rkind), dimension(MDC), intent(in)           :: coscos ! help array containing cosine to power 2 of spectral directions
    real(rkind), dimension(nverts), intent(in)        :: dep1   ! water depth at previous time level
    real(rkind), dimension(nverts), intent(in)        :: dep2   ! water depth at current time level
    real(rkind), intent(in)                           :: dhdx   ! derivative of depth in x-direction
    real(rkind), intent(in)                           :: dhdy   ! derivative of depth in y-direction
    real(rkind), dimension(MSC), intent(in)           :: dkdx   ! derivative of wave number in x-direction
    real(rkind), dimension(MSC), intent(in)           :: dkdy   ! derivative of wave number in y-direction
    real(rkind), dimension(MDC), intent(in)           :: ecos   ! help array containing cosine of spectral directions
    real(rkind), dimension(MDC), intent(in)           :: esin   ! help array containing sine of spectral directions
    real(rkind), dimension(MSC,ICMAX), intent(in)     :: kwave  ! wave number
    real(rkind), dimension(MDC), intent(in)           :: sincos ! help array containing sine * cosine of spectral directions
    real(rkind), dimension(MDC), intent(in)           :: sinsin ! help array containing sine to power 2 of spectral directions
    real(rkind), dimension(nverts), intent(in)        :: ux2    ! ambient velocity in x-direction at current time level
    real(rkind), dimension(nverts), intent(in)        :: uy2    ! ambient velocity in y-direction at current time level
...
!
    ...
    !
    real(rkind)                                  :: alpha    ! upper limit of CFL restriction
    real(rkind), dimension(3)                    :: cd       ! coefficients for computing cad
    real(rkind), dimension(10)                   :: cs       ! coefficients for computing cas
    real(rkind), dimension(3)                    :: dloc     ! local depth at vertices
    real(rkind)                                  :: duxdx    ! derivative of ux2 to x
    real(rkind)                                  :: duxdy    ! derivative of ux2 to y
    real(rkind)                                  :: duydx    ! derivative of uy2 to x
    real(rkind)                                  :: duydy    ! derivative of uy2 to y
    real(rkind)                                  :: fac      ! a factor
    real(rkind)                                  :: fac2     ! another factor
    real(rkind)                                  :: frlim    ! frequency range in which limit on velocity in theta-direction is applied
    real(rkind)                                  :: kd       ! help variable, wave number times water depth
    real(rkind)                                  :: pp       ! power of the frequency dependent limiter on refraction
    !
...
!
    ...
    if ( IREFR /= 0 ) then
       ...
       if ( KSPHER.EQ.1 ) then
          !
          cd(1) = tan(DEGRAD*(vert(iv1)%attr(VERTY) + YOFFS)) / REARTH2
          !
          do id = 1, MDC
             cd(2) = cd(1) * ecos(id)
             do is = 1, MSC
